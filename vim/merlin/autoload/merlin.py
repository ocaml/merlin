        ...
        if msg:
            ...
            if re.search("Not_found", msg):
                ...
                if vim.eval('exists("b:merlin_incompatible_version")') == "0":
                    ...
                    vimprint(
                        "The version of merlin you're using doesn't support this version of ocaml"
                    )
                return None
            vimprint(msg)
...
def vim_codec():
    ...
    if sys.version_info >= (3, 0):
        ...
        encoding = vim.eval("&fileencoding") or vim.eval("&encoding") or "ascii"
        return ((lambda str: str.encode(encoding)), (lambda str: str.decode(encoding)))
...
def merlin_exec(args, input=""):
    ...
    try:
        ...
        vimprint(
            "Failed starting ocamlmerlin. Please ensure that ocamlmerlin binary is executable."
        )
        raise e
...
def command2(args, context=None, track_verbosity=None):
    ...
    cmdline = (
        ...
        + verbosity
        + concat_map(
            lambda ext: ("-extension", ext), vim_list_if_set("b:merlin_extensions")
        )
        + concat_map(lambda pkg: ("-I", pkg), vim_list_if_set("b:merlin_packages_path"))
        + concat_map(
            lambda dm: ("-dot-merlin", dm), vim_list_if_set("b:merlin_dot_merlins")
        )
        + log_errors
...
def command_locate(path, pos):
    try:
        ...
        if pos is None:
            ...
            if path is None:
                pos_or_err = command(
                    "locate", "-look-for", choice, "-position", fmtpos(pos)
                )
            else:
                pos_or_err = command(
                    "locate",
    ...
    try:
        ...
        if completions["context"] and completions["context"][0] == "application":
            ...
            if not base or base == suffix:
                for label in app["labels"]:
                    ...
                    if name.startswith(suffix):
                        ...
                        vim.command(
                            "let l:tmp = {'word':'%s','menu':'%s','info':'%s','kind':'%s'}"
                            % (
                                prep(name),
                                prep(label["name"] + ":" + label["type"]),
                                "",
                                "~",
                            )
                        )
            ...
            if (
                (
                    show_argtype == "always"
                    or (show_argtype == "several" and nb_entries > 1)
                )
                and (not suffix or atom_bound.match(suffix[0]))
                and app["argument_type"] != "'_a"
            ):
                vim.command(
                    "let l:tmp = {'word':'%s','abbr':'<type>','kind':':','menu':'%s','empty':1}"
...
def vim_expand_prefix(base, vimvar, kinds=[]):
    ...
    try:
        ...
        args = [
            "expand-prefix",
            "-position",
            fmtpos(vim.current.window.cursor),
            "-prefix",
            base,
        ] + kinds
        l = command2(args)
...
def vim_loclist(vimvar, ignore_warnings):
    ...
    for error in errors:
        ...
        vim.command(
            "let l:tmp = {'bufnr':%d,'lnum':%d,'col':%d,'end_lnum':%d,'end_col':%d,'vcol':0,'nr':%d,'pattern':'','text':'%s','type':'%s','valid':1}"
...
def vim_occurrences(vimvar):
    ...
    for pos in lst:
        lnum = pos["line"]
        lcol = pos["col"]
        if (lnum, lcol) <= (line, col):
            cursorpos = nr
        text = vim.current.buffer[lnum - 1]
        text = text.replace("'", "''")
        vim.command(
            "let l:tmp = {'bufnr':%d,'lnum':%d,'col':%d,'vcol':0,'nr':%d,'pattern':'','text':'%s','type':'I','valid':1}"
...
def vim_case_analysis():
    ...
    if enclosing_types == []:
        ...
        try:
            enclosing_types = command(
                "type-enclosing", "-position", fmtpos((to_line, to_col))
            )
            if enclosing_types != []:
                current_enclosing = 0
            else:
    ...
    try:
        result = command(
            "case-analysis", "-start", fmtpos(tmp["start"]), "-end", fmtpos(tmp["end"])
        )
        tmp = result[0]
        txt = result[1]
...
def type_enclosing_at_pos(to_line, to_col):
    ...
    try:
        enclosing_types = command2(
            ["type-enclosing", "-position", fmtpos((to_line, to_col)), "-index", "0"],
...
def vim_construct(depth):
    ...
    try:
        result = command(
            "construct", "-max-depth", depth, "-position", fmtpos((line, col))
        )
        loc = result[0]
        txts = result[1]
...
def easy_matcher_wide(start, stop):
    ...
    return "{0}{1}.*\\%{2}l\\%{3}c".format(
        startl, startc, stop["line"], stop["col"] + 1
    )
