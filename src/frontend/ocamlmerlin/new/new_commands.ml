...
let marg_completion_kind f =
  Marg.param "completion-kind" (function
    ...
    | str ->
      failwithf
        "expecting completion kind, got %S. \
         kind can be value, variant, constructor, \
         label, module or module-type" str
  )

let rec find_command name =
  function
  | [] -> raise Not_found
  | (Command (name', _, _, _, _) as command) :: xs ->
    if name = name' then command else find_command name xs

let run pipeline query =
  Logger.log ~section:"New_commands" ~title:"run(query)" "%a" Logger.json
    (fun () ->
    Query_json.dump query
  );
...
" ~default:(`Offset (-1), `Offset (-1)) (fun buffer ->
      function
    ...
    command "holes" ~spec:[]
      ...
      ~default:() (fun buffer () -> run buffer Query_protocol.Holes);
    command "construct"
      ~spec:
      [
...
inferior depth will not be returned." ~default:(`Offset (-1), None, None)
...
Otherwise, Merlin looks for the documentation for the entity under the cursor (at `-position`)."
      ...
      ~default:(None, `None) (fun buffer (ident, pos) ->
      match pos with
      | `None -> failwith "-position <pos> is mandatory"
      | #Msource.position as pos ->
...
`message` is the error description to be shown to the user."
      ~default:(true, true, true) (fun buffer (lexing, parsing, typing) ->
...
completion only completes an identifier (last part of a module path)."
    ...
    command "extension-list"
      ...
      ~doc:
      "List all known / currently enabled / currently disabled extensions \
       as a list of strings." ~default:`All (fun buffer status ->
    ...
    command "findlib-list"
      ...
      ~default:() (fun buffer () -> run buffer Query_protocol.Findlib_list);
    command "flags-list" ~spec:[]
      ~doc:
      "Returns supported compiler flags.\
The purpose of this command is to implement interactive completion of \
compiler settings in an IDE." ~default:() (fun _ () ->
...
" ~default:("", `None) (fun buffer (target, pos) ->
      match pos with
      | `None -> failwith "-position <pos> is mandatory"
      | #Msource.position as pos ->
    ...
    command "phrase"
      ...
      [
        ...
        arg "-position" "<position> Position to complete"
          (marg_position (fun pos (target, _pos) -> target, pos)) ]
      ~doc:
      "Returns the position of the next or previous phrase \
       (top-level definition or module definition)." ~default:(`Next, `None)
...
matching and prints the corresponding module name." ~default:[]
    ...
    command "locate"
      ...
      [
        ...
        optional "-look-for"
          ...
          (Marg.param "<interface|implementation>" (fun kind (prefix, pos, _) ->
             match kind with
...
of all occurrences in current buffer of the entity at the specified position."
      ~default:`None (fun buffer ->
      function
      | `None -> failwith "-identifier-at <pos> is mandatory"
      | `Ident_at pos -> run buffer (Query_protocol.Occurrences (`Ident_at pos))
    );
...
of the buffer." ~default:() (fun buffer () -> run buffer Query_protocol.Outline);
    ...
    command "search-by-polarity"
      ...
      ~default:("", `None) (fun buffer (query, pos) ->
      match pos with
      | `None -> failwith "-position <pos> is mandatory"
      | #Msource.position as pos ->
...
"
      ...
      [
        arg "-position" "<position> Position "
          (marg_position (fun pos _pos -> pos)) ] ~default:`None (fun buffer ->
      function
      | `None -> failwith "-position <pos> is mandatory"
...
```"
    ...
    command "type-expression"
      ~doc:
      "Returns the type of the expression when typechecked in the environment \
around the specified position."
      ...
      ~default:("", `None) (fun buffer (expr, pos) ->
      match pos with
      | `None -> failwith "-position <pos> is mandatory"
      | #Msource.position as pos ->
      ...
      `Assoc
        ...
        *)
        [
          "dot_merlins",
          `List
            begin match Mconfig.(config.merlin.config_path) with
      ...
      [
        arg "-what"
          "<source|parsetree|ppxed-source|ppxed-parsetree|typedtree\
           ...
           Information to dump ()" (Marg.param "string" (fun what _ -> what)) ]
