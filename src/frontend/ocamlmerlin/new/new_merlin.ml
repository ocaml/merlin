...
let commands_help () =
  ...
  List.iter (fun (New_commands.Command (name, doc, args, _, _)) ->
    ...
    List.iter print_desc descs; print_newline (); print_endline doc
...
let run =
  ...
  | "-version" :: _ ->
    ...
    0
  | "-vnum" :: _ -> Printf.printf "%s\n" My_config.version; 0
  | "-warn-help" :: _ -> Warnings.help_warnings (); 0
  | "-flags-help" :: _ -> Mconfig.document_arguments stdout; 0
  | "-commands-help" :: _ -> commands_help (); 0
  | query :: raw_args ->
    begin match New_commands.find_command query New_commands.all_commands with
    | exception Not_found ->
      prerr_endline ("Unknown command " ^ query ^ ".\n"); usage (); 1
    | New_commands.Command (_name, _doc, spec, command_args, command_action) ->
      (* Setup notifications *)
      let notifications = ref [] in
      Logger.with_notifications notifications @@
        (fun () ->
           (* Parse commandline *)
           match
             ...
             let (config, command_args) =
               let fails = ref [] in
               let (config, command_args) =
                 Mconfig.parse_arguments ~wd:(Sys.getcwd ()) ~warning:(fun w ->
                   fails := w :: !fails
               ...
               let config =
                 ...
                 Mconfig.(
                   { config with  merlin = { config.merlin with  failures } }
                 )
               in
               config, command_args
             in
                   ...
                   fun () ->
                     ...
                     let json =
                       let (class_, message) =
                         ...
                         match
                           Mpipeline.with_pipeline pipeline @@
                             fun () -> command_action pipeline command_args
                         with
                         | result -> "return", result
                         | exception (Failure str) -> "failure", `String str
                         | exception exn ->
                       ...
                       let timing =
                         ("clock", clock_time) ::
                           ("cpu", cpu_time) ::
                             ("query", cpu_time -. pipeline_time) :: timing
                       in
           ...
           | () -> 0
           | exception exn ->
             prerr_endline ("Exception: " ^ Printexc.to_string exn); 1)
    end
...
let run env wd args =
  ...
  let wd_msg =
    ...
    | Some wd ->
      (try Sys.chdir wd; Printf.sprintf "changed directory to %S" wd
       with
       | _ -> Printf.sprintf "cannot change working directory to %S" wd)
  in
  ...
  in
  Logger.with_log_file log_file ~sections @@
    fun () -> log ~title:"run" "%s" wd_msg; run args
