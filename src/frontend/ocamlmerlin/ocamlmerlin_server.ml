...
module Server = struct
  ...
  let process_client client =
    ...
    let close_with return_code =
      flush_all (); Os_ipc.context_close context ~return_code
    in
    match process_request client with
    | code -> close_with code
    | exception Exit -> close_with (-1); raise Exit
    | exception exn ->
  ...
  let rec loop merlinid server =
    ...
    | Some client ->
      let continue =
        match process_client client with exception Exit -> false | () -> true
      in
  ...
  let start socket_path socket_fd =
    ...
    | Some server ->
      loop (File_id.get Sys.executable_name) server; Os_ipc.server_close server
end
...
let main () =
  ...
  | "single" :: args ->
    exit (New_merlin.run ("PATH=" ^ Unix.getenv "PATH" ^ "\000") None args)
