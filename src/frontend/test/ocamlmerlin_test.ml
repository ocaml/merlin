...
let process ?(with_config=(fun x -> x)) ?for_completion filename text =
  ...
  let config =
    Mconfig.({ config with  query = { config.query with  filename } })
  in
...
=
  test filename (fun () ->
    ...
    let (failures, typer_errors) =
      Mtyper.with_typer (M.typer_result m) @@
        fun () -> Mconfig.((M.final_config m).merlin.failures), M.typer_errors m
    in
    let fmt_msg exn =
...
let assertf b fmt =
  if b then Printf.ikfprintf ignore () fmt else Printf.ksprintf failwith fmt
...
let tests =
  [
    group "misc"
      [ assert_errors "relaxed_external.ml" "external test : unit = \"bs\"";
        validate_output "occurrences.ml" "let foo _ = ()\nlet () = foo 4\n"
          (Query_protocol.Occurrences (`Ident_at (`Offset 5))) (fun locations ->
      ...
      [
        group "glob"
          (let glob_match ~pattern str =
             Glob.match_pattern (Glob.compile_pattern pattern) str
           in
           let should_match name ~pattern str =
             test name (fun () ->
               assertf (glob_match ~pattern str) "pattern %S should match %S"
                 pattern str
             )
           and shouldn't_match name ~pattern str =
             test name (fun () ->
               assertf (not (glob_match ~pattern str))
        ...
        group "shell"
          (let string_list =
           ...
           let assert_split i (str, expected) =
             test ("split_command-" ^ string_of_int i) @@
               fun () ->
                 let result = Shell.split_command str in
                 assertf (result = expected)
                   "Shell.split_command %S = %s, expecting %s" str
                   (string_list result) (string_list expected)
           in
...
let rec run_tests indent =
  function [] -> () | x :: xs -> run_test indent x; run_tests indent xs
...
and run_test indent =
  ...
  | Group (name, tests) ->
    Printf.printf "%s-> %s\n" indent name; run_tests (indent ^ "  ") tests
