...
module Id = struct
  ...
  let name = function Id id -> Ident.name id | String s -> s
  
  let unique_name =
    function Id id -> Ident.unique_toplevel_name id | String s -> s
...
let rec to_string ~name =
  function
  | [] | Applied_to _ :: _ -> invalid_arg "Namespaced_path.to_string"
  | Ident (id, ns) :: rest ->
    List.fold_left rest ~init:(name id ^ Namespace.to_tag_string ns)
      ~f:(fun acc elt ->
      match elt with
      | Ident (id, ns) ->
...
let head_exn = function [] -> invalid_arg "head" | x :: _ -> x
let head x = try Some (head_exn x) with Invalid_argument _ -> None

let peal_head_exn =
  function [] -> invalid_arg "peal_head_exn" | _head :: rest -> rest
