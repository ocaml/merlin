         ...
         try(* initialize v0: v0(i) = A(0)(i) = delete i chars from t *)
           ...
           for i = 0 to l1 - 1 do(* first edit distance is the deletion of i+1 elements from s *)
             ...
             for j = 0 to String.length key - 1 do
               ...
               (if
                  i > 0 &&
                    j > 0 && str.[i] = key.[j - 1] && str.[i - 1] = key.[j]
                then
...
let spell_index s1 =
  let cutoff =
    match String.length s1 with 0 -> 0 | 1 -> 0 | 2 -> 0 | 3 -> 1 | _ -> 2
  in
...
let filter path ts =
  ...
  let rec aux_ts ts =
    function [] -> [] | p0 :: ps -> List.filter_map ~f:(aux_t p0 ps) ts
