(** Provides type information around the cursor.
    ...
    The information comes from two sources: 1. enclosing AST nodes: we just
    retrieve the types in the typedtree 2. if the cursor is on an identifier, by
    typing it in the current environment
    ...
    - when there is a syntax or type error in that area, and we don't have a
      precise enough AST node for the position (i.e. we got a "recovered" node,
      of type ['a]).
    ...
    then we'll have one AST node covering the whole ident. But what we
    reconstruct gives us: [Foo.Bar], [Foo.Bar.Baz], [Foo.Bar.Baz.lol]; and we
    return the type for each of them. These are what we call "small enclosings".
