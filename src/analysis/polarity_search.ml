...
let build_query ~positive ~negative env =
  let prepare r l =
    ...
    let set, _ = Env.lookup_type ~loc:Location.none l env in
...
let directories ~global_modules env =
  ...
  List.fold_left ~f:(fun l name ->
      ...
      match Env.lookup_module_path ~loc:Location.none ~load:true lident env with
      | exception _ -> l
...
let execute_query query env dirs =
  ...
  let rec recurse acc (Trie (_, dir, children)) =
    match
      ignore (Env.lookup_module_path ~loc:Location.none ~load:true dir env);
      Lazy.force children
    with
    | children ->
