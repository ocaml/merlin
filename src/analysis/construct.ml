...
module Util = struct
  ...
  let predef_types =
    ...
    let () =
      ...
      in
      let ident s = Ast_helper.Exp.ident (Location.mknoloc (Longident.Lident s))
      in
      List.iter ~f:(fun (k, v) -> Hashtbl.add tbl k v)
        Parsetree.(
          [ Predef.path_int, constant (Pconst_integer ("0", None));
            ...
            Predef.path_bool, construct "true"; Predef.path_unit, construct "()";
  ...
  let type_to_string t =
    Printtyp.type_expr Format.str_formatter t; Format.flush_str_formatter ()
    ...
    let aux name path value_description acc =
      ...
      let rec check_type type_expr params =
        ...
        | Some snap ->
          ...
          Btype.backtrack snap; Some params
        | None ->
  ...
  let combinations l =
    List.fold_left l ~init:[ [] ] ~f:(fun acc_l choices_for_arg_i ->
      List.fold_left choices_for_arg_i ~init:[] ~f:(fun acc choice_arg_i ->
        let choices = List.map acc_l ~f:(fun l -> List.rev (choice_arg_i :: l))
        in
...
module Gen = struct
  ...
  let rec module_ env =
    ...
    | Mty_ident path ->
      (try
         ...
         match m.mtd_type with Some t -> module_ env t | None -> raise Not_found
       with
       | Not_found ->
  ...
  and structure_item env =
    ...
    | Sig_class_type (id, class_type_decl, _, _) ->
      let str =
        Format.asprintf
          "Construct does not handle class types yet. \
      Please replace this comment by [%s]'s definition." (Ident.name id)
      in
      Str.text [ Docstrings.docstring str Location.none ] |> List.hd
    | Sig_class (id, class_decl, _, _) ->
      let str =
        Format.asprintf
          "Construct does not handle classes yet. \
      Please replace this comment by [%s]'s definition." (Ident.name id)
      in
      Str.text [ Docstrings.docstring str Location.none ] |> List.hd
  ...
  let rec expression ~idents_table values_scope ~depth =
    ...
    let make_arg =
      let make_i n i =
        Hashtbl.replace idents_table n i; Printf.sprintf "%s_%i" n i
      in
      let uniq_name env n =
        ...
        | Not_found ->
          (try
             ...
             make_i n 0
           with
           | Not_found -> Hashtbl.add idents_table n 0; n)
      in
      fun env label ty ->
    ...
    let constructor env type_expr path constrs =
      ...
      let make_constr env path type_expr cstr_descr =
        ...
        | Some snap ->
          let lid =
            Util.prefix env ~env_check:Env.find_constructor_by_name path
          ...
          List.filter_map exps ~f:(fun exp ->
            ...
            (* For gadts not all combinations will be valid.
                ...
                can be typed. *)
            try Typecore.type_expression env exp |> ignore; Some exp
            with
            | _ -> None
          )
        | None -> []
      in
      List.map constrs ~f:(make_constr env path type_expr) |> Util.panache
    in
    let variant env typ row_desc =
      let fields =
        List.filter ~f:(fun (lbl, row_field) ->
    ...
    let record env typ path labels =
      ...
      Util.combinations lbl_exprs |>
        List.map ~f:(fun lbl_exprs ->
          let labels = List.map2 lbl_lids lbl_exprs ~f:(fun lid exp -> lid, exp)
          in
    ...
    fun env typ ->
      ...
      let constructed_from_type =
        ...
        | Tconstr (path, params, _) ->
          ...
          (try [ Hashtbl.find Util.predef_types path ]
           with
           | Not_found ->
        ...
        | Ttuple types ->
          let choices = List.map types ~f:(exp_or_hole env) |> Util.combinations
          in
        ...
        | Tpackage (path, lids, args) ->
          ...
          (try
             ...
             in
             let ast =
               Exp.constraint_ (Exp.pack (module_ env ty))
