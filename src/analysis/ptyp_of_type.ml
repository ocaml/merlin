...
and core_type type_expr =
  ...
  | Tvariant { row_fields; row_closed; row_name; _ } ->
    let field (label, row_field) =
      ...
      | Rpresent None | Reither (true, _, _, _) -> Rf.tag label true []
...
and extension_constructor id { ext_args; ext_ret_type; ext_attributes; _ } =
  Ast_helper.Te.decl ~attrs:ext_attributes
    ...
    ?res:(Option.map ~f:core_type ext_ret_type) (var_of_id id)
...
and constructor_declaration { cd_id; cd_args; cd_res; cd_attributes; _ } =
  Ast_helper.Type.constructor ~attrs:cd_attributes
    ~args:(constructor_arguments cd_args) ?res:(Option.map ~f:core_type cd_res) @@
    var_of_id cd_id

and type_declaration
    id
    { type_params; type_variance; type_manifest; type_kind; type_attributes;
      type_private; _ }
=
  let params =
...
and signature_item (str_item : Types.signature_item) =
  ...
  | Sig_typext (id, ext_constructor, _, _) ->
    let ext =
      Te.mk (Location.mknoloc @@ Longident.Lident (Ident.name id))
