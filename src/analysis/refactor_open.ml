...
(** [qual_or_unqual_path mode ~open_lident ~open_path node_path node_lid] if
    mode is `Unqualify - returns [node_lid] or [node_lid] with prefix
    [open_lident] cut off, whichever is shorter

    `Qualify - returns [node_path] with its prefix equal to [open_lident]

    Returns [None] if [node_lid] doesn't need changes.

    Note: by "prefix" we mean the leftmost consecutive part of a longident or a
    path. *)
...
let get_rewrites ~mode typer pos =
  ...
  | Some (open_path, open_lident, (_, node) :: _) ->
      let paths_and_lids =
        Browse_tree.all_occurrences_of_prefix open_path node
      in
