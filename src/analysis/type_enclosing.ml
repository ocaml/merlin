    ...
    | Class_field
        {
          cf_desc =
            Tcf_method
              (_,
               _,
               Tcfk_concrete (_, { exp_type = { desc = Tarrow (_, _, t, _) } }))
        }
      ->
      ret (Type (env, t))
    | Class_field
...
let from_reconstructed ~nodes ~cursor ~verbosity exprs =
  ...
  let include_lident = match node with Pattern _ -> false | _ -> true in
  let include_uident =
    match node with
    | Module_binding _
  ...
  let f =
    fun { Location.txt = source; loc } ->
      ...
      | _ ->
        ...
        | "" ->
          log ~title:"from_reconstructed" "no reconstructed identifier"; None
        | source when not include_lident && Char.is_lowercase source.[0] ->
          log ~title:"from_reconstructed" "skipping lident"; None
        | source when not include_uident && Char.is_uppercase source.[0] ->
          log ~title:"from_reconstructed" "skipping uident"; None
        | source ->
          (try
             let (ppf, to_string) = Format.to_string () in
