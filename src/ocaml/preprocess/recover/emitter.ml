...
module Codeconsing (S : Synthesis.S) (R : Recovery.S with module G = S.G) : sig(* Step 1: record all definitions *)
    ...
    | x :: xs as xxs ->
      (try !(Hashtbl.find normalized_actions xxs)
       ...
       | Not_found ->
         ...
         let xxs' = x' :: xs' in
         Hashtbl.add normalized_actions xxs (ref xxs'); xxs')
  
  and normalize_action =
    function
    | (Abort | Reduce _ | Shift _) as a -> a
    | Var v ->
      begin match normalize_actions (S.solution v) with
      | [ x ] -> x
      | xs -> Var (A xs)
      end
  ...
  let record_item root = roots := root :: !roots
    ...
    let rec get =
      ...
      | x :: xs ->
        ...
        (try
           ...
           incr r; v
         with
         | Not_found -> Hashtbl.add table xxs (ref 1, xxs); xxs)
    and get_one = function Var (A v) -> Var (A (get v)) | x -> x
    in
    Hashtbl.iter (fun _k v -> v := get !v) normalized_actions;
    (* Return counter *)
    fun v -> try !(fst (Hashtbl.find table v)) with Not_found -> 0
  
  type instr = Nil | Cons of instr paction * instr | Ref of int ref * instr
      ...
      | x :: xs as xxs ->
        (try Hashtbl.find table xxs
         ...
         | Not_found ->
           ...
           if counter xxs = 1 then
             value
           else
             (assert (not !frozen);
              let value = Ref (ref (-1), value) in
              values := value :: !values; Hashtbl.add table xxs value; value))
    in
    frozen, values, emit
  ...
  let normalize () =
    ...
    frozen := true; !values, (fun item -> emit (item_to_actions item))
end
  

module Make
    (G : GRAMMAR) (A : Recover_attrib.S with module G = G)
    (S : Synthesis.S with module G = G) (R : Recovery.S with module G = G) : sig
  ...
  let (_code, get_instr, iter_entries) =
    Lr1.iter (fun st ->
      ...
      List.iter (fun (_case, items) -> C.record_item (list_last items)) cases
    );
    let (code, get_instr) = C.normalize () in
    let all_instrs =
      Lr1.tabulate (fun st ->
        let (_depth, cases) = R.recover st in
        List.map (fun (_case, items) -> get_instr (list_last items)) cases
      )
    in
    code, get_instr, (fun f -> Lr1.iter (fun st -> List.iter f (all_instrs st)))
  ...
  let emit_recoveries ppf =
    ...
    let rec alloc_entry =
      ...
      | C.Ref (r, instr) ->
        if !r = (-1) then
          (alloc_entry instr; r := !k; instrs := (!k, instr) :: !instrs; incr k)
    and alloc_entry_action =
      function
    ...
    and emit_instr ppf =
      function
      | C.Nil -> fprintf ppf "[]"
      | C.Cons (act, C.Nil) -> fprintf ppf "[%a]" emit_action act
      | C.Cons (act, instr) ->
  ...
  let emit ppf =
    ...
    emit_defs ppf; emit_depth ppf; emit_can_pop ppf; emit_recoveries ppf
end
