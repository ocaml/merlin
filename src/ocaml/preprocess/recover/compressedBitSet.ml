...
type t = N | C of int * int * t
type element = int

let word_size = Sys.word_size - 1
let empty = N
...
let rec fold f s accu =
  match s with N -> accu | C (base, ss, qs) -> loop f qs base ss accu
...
let cardinal s = fold (fun _ m -> m + 1) s 0
let elements s = fold (fun tl hd -> tl :: hd) s []
...
let rec subset s1 s2 =
  ...
  | C (addr1, ss1, qs1), C (addr2, ss2, qs2) ->
    if addr1 < addr2 then
      ...
      (if ss1 land ss2 <> ss1 then false else subset qs1 qs2)
    else
      subset s1 qs2
...
let rec union s1 s2 =
  ...
  | C (addr1, ss1, qs1), C (addr2, ss2, qs2) ->
    if addr1 < addr2 then
      ...
      if ss == ss2 && s == qs2 then s2 else C (addr1, ss, s)
...
let choose s =
  try iter (fun x -> raise (Found x)) s; raise Not_found with Found x -> x
...
let rec disjoint s1 s2 =
  ...
  | C (addr1, ss1, qs1), C (addr2, ss2, qs2) ->
    if addr1 = addr2 then
      (if ss1 land ss2 = 0 then disjoint qs1 qs2 else false)
    else if addr1 < addr2 then
      disjoint qs1 s2
    else
      disjoint s1 qs2
