...
module Make (G : GRAMMAR) (S : Synthesis.S with module G = G) :
  S with module G = G
= struct
  module G = G 
...
*)
  ...
  module Trace = struct
    ...
    let cat (Trace (c1, tr1)) (Trace (c2, tr2)) = Trace (c1 +. c2, tr1 @ tr2)
  (*
...
*)
  ...
  module State = struct
    ...
    let rec merge_level l1 l2 : level =
      ...
      | (nt1, c1) :: xs1, x2 :: xs2 ->
        let (nt2, c2) = x2 in
        begin match compare nt1 nt2 with
        | 0 ->
...
*)
  ...
  let synthesize =
    let rec add_nt tr nt =
      function
      | [] -> [ nt, tr ]
      | x :: xs ->
        let c = compare nt (fst x) in
        if c = 0 then
          (nt, Trace.min tr (snd x)) :: xs
        else if c < 0 then
          (nt, tr) :: xs
        else
          x :: add_nt tr nt xs
    ...
    let add_item cost item stack =
      ...
      if cost = infinity then
        ...
        and stack_tl = function [] -> [] | _ :: xs -> xs
        in
        let rec aux stack =
          function
          | 0 ->
            add_nt (Trace (cost, [ item ])) (Production.lhs prod)
              (stack_hd stack) ::
              stack_tl stack
  ...
  let step st ntss =
    ...
    let rec aux =
      ...
      | ((nt, tr) :: x) :: xs
        when
          not (CompressedBitSet.mem (Nonterminal.to_int nt) !seen) &&
  ...
  let recover st =
    ...
    let pos =
      ...
      List.fold_left (fun pos (_, pos') -> max pos pos') (snd (List.hd items))
        (List.tl items)
    in
    (* Walk this prefix *)
    let traces =
      let acc = ref [ init st ] in
      for _i = 1 to pos - 1 do acc := List.concat (List.map expand !acc) done;
      !acc
    in
    (* Last step *)
    let select_trace traces =
      ...
      | (_, trace) :: alternatives ->
        List.fold_left (fun tr1 (_, tr2) -> Trace.min tr1 tr2) trace
          alternatives
    in
    let process_trace trace =
      match expand trace with
      | [] -> (* Initial state *) assert (snd trace = []); []
      | states ->
    ...
    List.flatten @@
      List.map (fun trace ->
        List.map (fun (st, Trace (_, reductions)) -> st, reductions)
          (process_trace trace)
      ) traces
