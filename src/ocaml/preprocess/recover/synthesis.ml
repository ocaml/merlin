...
= struct
  ...
  let pred =
    ...
    let revert_transition s1 (sym, s2) =
      assert
        begin match Lr0.incoming (Lr1.lr0 s2) with
        | None -> false
        | Some sym' -> sym = sym'
        end;
  ...
  type 'a paction = Abort | Reduce of production | Shift of symbol | Var of 'a
  
  let paction_to_string variable_to_string =
    function
    | Abort -> "Abort"
...
*)
  ...
  let cost_of =
    ...
    | Head (st, n) ->
      let acc =
        List.fold_left (fun acc (_sym, st') ->
      ...
      in
      let cost =
        List.fold_left (fun acc (_, prods) ->
      ...
      if penalty = infinity then
        ...
        let head =
          ...
          if cost < infinity then
            ...
            match sym with T _ -> const infinity | N n -> var (Head (st, n))
        in
        let tail =
          let (sym, _, _) = (Production.rhs prod).(pos) in
  ...
  let select var1 var2 = arg_min_float cost_of_action var1 var2
      ...
      if penalty = infinity then
        ...
        let head =
          ...
          if cost < infinity then
            ...
            match sym with T _ -> Abort | N n -> Var (Head (st, n))
        in
        let tail =
          let (sym, _, _) = (Production.rhs prod).(pos) in
          match List.assoc sym (Lr1.transitions st) with
          | st' -> Var (Tail (st', prod, pos + 1))
          | exception Not_found -> Abort
        in
        [ head; tail ]
  ...
  let report ppf =
    ...
    let solutions =
      Lr1.fold (fun st acc ->
        match
          List.fold_left (fun (item, cost) (prod, pos) ->
        ...
        with
        | None, _ -> fprintf ppf "no synthesis from %d\n" (Lr1.to_int st); acc
    ...
    List.iter (fun (item, states) ->
      ...
      List.iter (fun (cost, states) ->
        fprintf ppf "at cost %f from states %s\n\n" cost
          (list_fmt (fun x -> string_of_int (Lr1.to_int x)) states)
      ) (group_assoc states)
