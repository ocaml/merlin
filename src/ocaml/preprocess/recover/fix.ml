  ...
  let lfp (eqs : equations) : valuation =
    ...
    (* Our workset is based on a Queue, but it could just as well be based on a
      ...
      (* Initialize the workset. *)
        ...
        let repeat f =
          while not (Queue.is_empty workset) do f (Queue.pop workset) done
      end
      ...
      let rec solve (node : node) : unit =
        ...
        let request (v : variable) : property =
          ...
          | Not_found ->
            ...
            (if not (P.is_maximal p) then subjects := subject :: !subjects); p
        in
        ...
        if not (P.equal data.property new_property) then
          (data.property <- new_property; signal node)
      ...
      let get (v : variable) : property =
        ...
        | Not_found ->
          ...
          Workset.repeat solve; freeze (); inactive := true; property node
    (* -------------------------------------------------------------------------- *)
