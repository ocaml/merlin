...
let array_exists arr f =
  try
    for i = 0 to Array.length arr - 1 do if f arr.(i) then raise (Found i) done;
    false
  with
  | Found _ -> true
...
let array_find arr f = arr.(array_findi arr f)
let array_assoc arr x = snd (array_find arr (fun (x', _) -> compare x x' = 0))
let list_fmt f l = "[" ^ String.concat "; " (List.map f l) ^ "]"
let fst3 (x, _, _) = x

let rec list_last =
  function [ x ] -> x | _ :: xs -> list_last xs | [] -> invalid_arg "list_last"
