...
module Make (G : Cmly_api.GRAMMAR) : S with module G = G = struct
  ...
  let cost_of_symbol =
    let measure ~has_default prj attrs =
      if
        List.exists (Attribute.has_label "recovery") (prj attrs) || has_default
      then
    ...
    let ft =
      Terminal.tabulate (fun t ->
        if Terminal.typ t = None then
          measure ~has_default:true Terminal.attributes t
        else
          measure ~has_default:false Terminal.attributes t
      )
    in
    let fn =
      Nonterminal.tabulate (measure ~has_default:false Nonterminal.attributes)
    in
    function T t -> ft t | N n -> fn n
  ...
  let default_prelude ppf =
    List.iter (fun a ->
      if
        Attribute.has_label "header" a ||
          Attribute.has_label "recovery.header" a
      then
