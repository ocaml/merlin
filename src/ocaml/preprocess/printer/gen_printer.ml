...
let symbol_printer default attribs =
  ...
  | exception Not_found -> sprintf "%S" default
...
let print_symbol () =
  ...
  Terminal.iter case_t; Nonterminal.iter case_n
  ...
  let case t =
    ...
    | `REGULAR | `EOF ->
      printf "  | %s%s -> print_value (%s.T %s.T_%s) %s\n" (Terminal.name t)
        begin match Terminal.typ t with
        | None -> ""
        | Some _typ -> " v"
        end menhir menhir (Terminal.name t)
        begin match Terminal.typ t with
        | None -> "()"
        | Some _typ -> "v"
        end
...
let print_token_of_terminal () =
  let case t =
    match Terminal.kind t with
    | `REGULAR | `EOF ->
      printf "  | %s.T_%s -> %s%s\n" menhir (Terminal.name t) (Terminal.name t)
        (if Terminal.typ t <> None then " v" else "")
    | `ERROR -> printf "  | %s.T_%s -> assert false\n" menhir (Terminal.name t)
    | `PSEUDO -> ()
  in
  printf
...
let () =
  ...
  print_newline ();
  print_value ();
  print_newline (); print_token (); print_newline (); print_token_of_terminal ()
