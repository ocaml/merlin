...
class_expr:
  ...
  | LET OPEN override_flag attributes mkrhs(mod_longident) IN class_expr
      { wrap_class_attrs ((mkclass ~loc:(make_loc $symbolstartpos $endpos))(Pcl_open($3, $5, $7))) $4 }
...
class_signature:
  ...
  | LET OPEN override_flag attributes mkrhs(mod_longident) IN class_signature
      { wrap_class_type_attrs ((mkcty ~loc:(make_loc $symbolstartpos $endpos))(Pcty_open($3, $5, $7))) $4 }
;
class_sig_body:
...
%public expr [@recovery default_expr ()]:
  ...
  | simple_expr DOTOP LBRACKET expr RBRACKET LESSMINUS expr
      { let id = (mkexp ~loc:(make_loc $symbolstartpos $endpos)) @@ Pexp_ident( (ghloc ~loc:(make_loc $symbolstartpos $endpos)) @@ Lident ("." ^ $2 ^ "[]<-")) in
        (mkexp ~loc:(make_loc $symbolstartpos $endpos)) @@ Pexp_apply(id , [Nolabel, $1; Nolabel, $4; Nolabel, $7]) }
  ...
  | simple_expr DOT mod_longident DOTOP LBRACKET expr RBRACKET LESSMINUS expr
      { let id = (mkexp ~loc:(make_loc $symbolstartpos $endpos)) @@ Pexp_ident( (ghloc ~loc:(make_loc $symbolstartpos $endpos)) @@ Ldot($3,"." ^ $4 ^ "[]<-")) in
        (mkexp ~loc:(make_loc $symbolstartpos $endpos)) @@ Pexp_apply(id , [Nolabel, $1; Nolabel, $6; Nolabel, $9]) }
...
%public simple_expr:
  ...
  | simple_expr DOTOP LBRACKET expr RBRACKET
      { let id = (mkexp ~loc:(make_loc $symbolstartpos $endpos)) @@ Pexp_ident( (ghloc ~loc:(make_loc $symbolstartpos $endpos)) @@ Lident ("." ^ $2 ^ "[]")) in
        (mkexp ~loc:(make_loc $symbolstartpos $endpos)) @@ Pexp_apply(id, [Nolabel, $1; Nolabel, $4]) }
  ...
  | simple_expr DOT mod_longident DOTOP LBRACKET expr RBRACKET
      { let id = (mkexp ~loc:(make_loc $symbolstartpos $endpos)) @@ Pexp_ident( (ghloc ~loc:(make_loc $symbolstartpos $endpos)) @@ Ldot($3, "." ^ $4 ^ "[]")) in
        (mkexp ~loc:(make_loc $symbolstartpos $endpos)) @@ Pexp_apply(id, [Nolabel, $1; Nolabel, $6]) }
...
let_binding_body:
  ...
  | val_ident type_constraint EQUAL seq_expr
      { let v = (mkpatvar $1 (make_loc $startpos($1) $endpos($1))) in (* PR#7344 *)
        let t =
          match $2 with
            Some t, None -> t
          | _, Some t -> t
          | _ -> assert false
        in
        ((ghpat ~loc:(make_loc $symbolstartpos $endpos))(Ppat_constraint(v, (ghtyp ~loc:(make_loc $symbolstartpos $endpos))(Ptyp_poly([],t)))),
         (mkexp_constraint ~loc:(make_loc $symbolstartpos $endpos)) $4 $2) }
...
field:
  mkrhs(label) COLON poly_type_no_attr attributes
    { Otag ($1, add_info_attrs ((symbol_info $endpos)) $4, $3) }
;
...
%public structure_item:
| lwt_bindings
    { (val_of_lwt_bindings ~loc:(make_loc $symbolstartpos $endpos)) $1 }
...
| lwt_bindings IN seq_expr
    { (expr_of_lwt_bindings ~loc:(make_loc $symbolstartpos $endpos)) $1 (merloc $endpos($2) $3) }
| MATCH_LWT ext_attributes seq_expr WITH opt_bar match_cases
    { let expr = (mkexp_attrs ~loc:(make_loc $symbolstartpos $endpos))
...
| TRY_LWT ext_attributes seq_expr %prec below_WITH
    { (reloc_exp ~loc:(make_loc $symbolstartpos $endpos)) (Fake.app Fake.Lwt.in_lwt $3) }
| TRY_LWT ext_attributes seq_expr WITH opt_bar match_cases
    { (mkexp_attrs ~loc:(make_loc $symbolstartpos $endpos))
...
| WHILE_LWT ext_attributes seq_expr DO seq_expr DONE
  { let expr = Pexp_while ($3, Fake.(app Lwt.un_lwt $5)) in
    Fake.(app Lwt.to_lwt ((mkexp_attrs ~loc:(make_loc $symbolstartpos $endpos)) expr $2)) }
