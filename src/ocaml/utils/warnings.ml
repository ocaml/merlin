  ...
  lazy
    (let prev = backup () in
     ...
     try
       ...
       restore prev; r
     with
     | exn -> restore prev; raise exn)

let set_alert ~error ~enable s =
  let upd =
    match s with
    | "all" -> Std.String.Set.empty, not enable
    | s ->
      let (set, pos) = if error then !current.alert_errors else !current.alerts
      in
      let f = if enable = pos then Std.String.Set.add else Std.String.Set.remove
      in
      f s set, pos
  in
  if error then
...
let parse_opt error active errflag s =
  ...
  let rec loop i =
    if i >= String.length s then
      ...
      match s.[i] with
      | 'A' .. 'Z' ->
        List.iter set (letter (Char.lowercase_ascii s.[i])); loop (i + 1)
      | 'a' .. 'z' -> List.iter clear (letter s.[i]); loop (i + 1)
  ...
  and loop_letter_num myset i =
    if i >= String.length s then
      ...
      | '0' .. '9' ->
        ...
        for n = n1 to min n2 last_warning_number do myset n done; loop i
      | 'A' .. 'Z' ->
        List.iter myset (letter (Char.lowercase_ascii s.[i])); loop (i + 1)
      | 'a' .. 'z' -> List.iter myset (letter s.[i]); loop (i + 1)
      | _ -> error ()
  in
      ...
      if arr.(i) then
        let x =
          try
            if verbose then
              let (_, desc, _) =
                List.find ~f:(fun (n, _, _) -> n = i) descriptions
              in
