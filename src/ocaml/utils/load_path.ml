...
let reset () =
  ...
  files := SMap.empty; files_uncap := SMap.empty; dirs := []
...
let add dir =
  ...
  let (new_files, new_files_uncap) =
    add_to_maps (Filename.concat dir.Dir.path) dir.Dir.files !files !files_uncap
  in
  files := new_files; files_uncap := new_files_uncap

let init l = reset (); dirs := List.rev_map Dir.create l; List.iter add !dirs
...
let add dir =
  assert (not Config.merlin || Local_store.is_bound ());
  let (new_files, new_files_uncap) =
    add_to_maps (Filename.concat dir.Dir.path) dir.Dir.files SMap.empty
      SMap.empty
  in
