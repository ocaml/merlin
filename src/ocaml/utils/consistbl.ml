...
= struct
  ...
  let check_noadd tbl name crc source =
    try check_ tbl name crc source with Not_found -> raise (Not_available name)
  ...
  let extract_map mod_names tbl =
    Module_name.Set.fold (fun name result ->
      try
        let (crc, _) = Module_name.Tbl.find tbl name in
  ...
  let filter p tbl =
    ...
    Module_name.Tbl.iter (fun name _ ->
      if not (p name) then to_remove := name :: !to_remove
    ) tbl;
    List.iter (fun name ->
      while Module_name.Tbl.mem tbl name do Module_name.Tbl.remove tbl name done
    ) !to_remove
end
