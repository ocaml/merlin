...
let write_ast magic ast =
  ...
  output_value oc !Location.input_name; output_value oc ast; close_out oc; fn
...
let ppx_commandline cmd fn_in fn_out =
  Printf.sprintf "%s %s %s 1>&2" cmd (Filename.quote fn_in)
...
let apply_rewriter magic ppx (fn_in, failures) =
  ...
  let failure =
    ...
    (* check magic before passing to the next ppx *)
      ...
      in
      (close_in ic; if buffer <> magic then Some (WrongMagic comm) else None)
  in
  match failure with
  | Some err ->
    ...
    let fallback =
      ...
      | () -> fallback
      | exception exn ->
        log ~title "exception while renaming ast: %a" Logger.exn exn; fn_in
    in
    report_error err; fallback, failures + 1
  | None -> Misc.remove_file fn_in; fn_out, failures
...
let read_ast magic fn =
  ...
  with
  | exn -> close_in ic; Misc.remove_file fn; raise exn

let rewrite magic ast ppxs =
  let (fn_out, _) =
    List.fold_right ~f:(apply_rewriter magic) ~init:(write_ast magic ast, 0)
      ppxs
  in
...
let apply_rewriters ~ppx ?restore ~tool_name =
  function
  | `Interface ast ->
