...
let reset_type_variables () =
  reset_global_level ();
  Ctype.reset_reified_var_counter (); type_variables := TyVarMap.empty
  ...
  | Ptyp_var name ->
    let ty =
      try
        ...
        ignore (TyVarMap.find name !type_variables); raise Already_bound
      with
      | Not_found ->
        let v = new_global_var ~name () in
        type_variables := TyVarMap.add name v !type_variables; v
    in
    {
...
and transl_type_aux env policy styp =
  ...
  | Ptyp_constr (lid, stl) ->
    ...
    (try Ctype.enforce_constraints env constr
     with
     | Unify trace -> raise (Error (styp.ptyp_loc, env, Type_mismatch trace)));
  ...
  | Ptyp_class (lid, stl) ->
    let (path, decl, _is_variant) =
      try
        ...
        ignore (Env.lookup_type ~loc:lid.loc lid.txt env); path, decl, true
      with
      | Not_found ->
        (try
           let lid2 =
    ...
    let ty =
      ...
      | Tobject (fi, _) ->
        ...
        (if policy = Univars then pre_univars := tv :: !pre_univars); ty
      | _ -> assert false
    in
    ...
    let add_field field =
      ...
      let rf_desc =
        ...
        | Rtag (l, c, stl) ->
          ...
          in
          add_typed_field styp.ptyp_loc l.txt f; Ttag (l, c, tl)
        | Rinherit sty ->
  ...
  | Ptyp_poly (vars, st) ->
    ...
    unify_var env (newvar ()) ty'; ctyp (Ttyp_poly (vars, cty)) ty'
  | Ptyp_package (p, l) ->
  ...
  let add_field { pof_desc; pof_loc; pof_attributes } =
    ...
    let of_desc =
      ...
      | Otag (s, ty1) ->
        ...
        add_typed_field ty1.ctyp_loc s.txt ty1.ctyp_type; field
      | Oinherit sty ->
        let cty = transl_type env policy sty in
        let nm =
          match repr cty.ctyp_type with
        ...
        | { desc = Tobject ({ desc = (Tfield _ | Tnil) as tf }, _) }, _ ->
          ...
          let rec iter_add =
            function
            | Tfield (s, _k, ty1, ty2) ->
              add_typed_field sty.ptyp_loc s ty1; iter_add ty2.desc
            | Tnil -> ()
            | _ -> assert false
          in
...
let globalize_used_variables env fixed =
  let r = ref [] in
  TyVarMap.iter (fun name (ty, loc) ->
    let v = new_global_var () in
    let snap = Btype.snapshot () in
    if try unify env v ty; true with _ -> Btype.backtrack snap; false then
...
let transl_simple_type env fixed styp =
  ...
  globalize_used_variables env fixed (); make_fixed_univars typ.ctyp_type; typ
...
let transl_simple_type_delayed env styp =
  ...
  generalize typ.ctyp_type; typ, instance typ.ctyp_type, force
...
let report_error env ppf =
  ...
  | Type_arity_mismatch (lid, expected, provided) ->
    fprintf ppf
      "@[The type constructor %a@ expects %i argument(s),@ \
        but is here applied to %i argument(s)@]" longident lid expected provided
  ...
  | Type_mismatch trace ->
    Printtyp.report_unification_error ppf Env.empty trace (function
  ...
  | Present_has_no_type l ->
    fprintf ppf
      "@[<v>@[The constructor %s is missing from the upper bound@ \
       ...
       or remove it@ from the lower bound.@]@]" l l
  ...
  | Opened_object nm ->
    fprintf ppf "Illegal open object type%a" (fun ppf ->
      function Some p -> fprintf ppf "@ %a" path p | None -> fprintf ppf ""
    ) nm
  | Not_an_object ty ->
