...
let rec typexp copy_scope s ty =
  ...
  | (Tvar _ | Tunivar _) as desc ->
    if s.for_saving || ty.id < 0 then
      ...
      in
      (For_copy.save_desc copy_scope ty desc; ty.desc <- Tsubst ty'; ty')
    else
      ty
  | Tsubst ty -> ty
  | Tfield (m, k, _t1, _t2)
      ...
      (if has_fixed_row then
         ...
         | Tvariant row ->
           ...
           | Tsubst { desc = Ttuple [ _; ty2 ] } ->
             ...
             ty.desc <- Tsubst ty2; (* avoid Tlink in the new type *) Tlink ty2
           | _ ->
             let dup =
               s.for_saving ||
             ...
             (* Return a new copy *)
             let row = copy_row (typexp copy_scope s) true row (not dup) more'
             in
...
let class_signature copy_scope s sign =
  {
    ...
    csig_inher =
      List.map (fun (p, tl) -> type_path s p, List.map (typexp copy_scope s) tl)
        sign.csig_inher
  }
