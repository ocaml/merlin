...
let structure sub { str_items; str_final_env; _ } =
  List.iter (sub.structure_item sub) str_items; sub.env sub str_final_env

let class_infos sub f x =
  List.iter (fun (ct, _) -> sub.typ sub ct) x.ci_params; f x.ci_expr
...
let constructor_decl sub { cd_args; cd_res; _ } =
  constructor_args sub cd_args; Option.iter (sub.typ sub) cd_res
...
let type_declaration sub { typ_cstrs; typ_kind; typ_manifest; typ_params; _ } =
  List.iter (fun (c1, c2, _) -> sub.typ sub c1; sub.typ sub c2) typ_cstrs;
...
let extension_constructor sub { ext_kind; _ } =
  match ext_kind with
  | Text_decl (ctl, cto) ->
    constructor_args sub ctl; Option.iter (sub.typ sub) cto
  | Text_rebind _ -> ()
...
let pat : type k. iterator -> k general_pattern -> unit =
  fun sub { pat_extra = extra; pat_desc; pat_env; _ } ->
    ...
    | Tpat_or (p1, p2, _) -> sub.pat sub p1; sub.pat sub p2
...
let expr sub { exp_extra; exp_desc; exp_env; _ } =
  let extra =
    ...
    | Texp_coerce (cty1, cty2) ->
      Option.iter (sub.typ sub) cty1; sub.typ sub cty2
    | Texp_newtype _ -> ()
  ...
  | Texp_let (rec_flag, list, exp) ->
    sub.value_bindings sub (rec_flag, list); sub.expr sub exp
  | Texp_function { cases; _ } -> List.iter (sub.case sub) cases
  | Texp_apply (exp, list) ->
    sub.expr sub exp;
    List.iter (fun (_, o) -> Option.iter (sub.expr sub) o) list
  | Texp_match (exp, cases, _) ->
    sub.expr sub exp; List.iter (sub.case sub) cases
  | Texp_try (exp, cases) -> sub.expr sub exp; List.iter (sub.case sub) cases
  ...
  | Texp_setfield (exp1, _, _, exp2) -> sub.expr sub exp1; sub.expr sub exp2
  | Texp_array list -> List.iter (sub.expr sub) list
  | Texp_ifthenelse (exp1, exp2, expo) ->
    sub.expr sub exp1; sub.expr sub exp2; Option.iter (sub.expr sub) expo
  | Texp_sequence (exp1, exp2) -> sub.expr sub exp1; sub.expr sub exp2
  | Texp_while (exp1, exp2) -> sub.expr sub exp1; sub.expr sub exp2
  | Texp_for (_, _, exp1, exp2, _, exp3) ->
    sub.expr sub exp1; sub.expr sub exp2; sub.expr sub exp3
  | Texp_send (exp, _, expo) ->
    sub.expr sub exp; Option.iter (sub.expr sub) expo
  | Texp_new _ -> ()
  | Texp_instvar _ -> ()
  ...
  | Texp_letmodule (_, _, _, mexpr, exp) ->
    sub.module_expr sub mexpr; sub.expr sub exp
  ...
  | Texp_letop { let_ = l; ands; body; _ } ->
    sub.binding_op sub l; List.iter (sub.binding_op sub) ands; sub.case sub body
  | Texp_unreachable -> ()
  | Texp_extension_constructor _ -> ()
  | Texp_open (od, e) -> sub.open_declaration sub od; sub.expr sub e
  | Texp_hole -> ()
...
let signature sub { sig_items; sig_final_env; _ } =
  sub.env sub sig_final_env; List.iter (sub.signature_item sub) sig_items
...
let functor_parameter sub =
  function Unit -> () | Named (_, _, mtype) -> sub.module_type sub mtype
...
let open_declaration sub { open_expr; open_env; _ } =
  sub.module_expr sub open_expr; sub.env sub open_env
...
let module_coercion sub =
  ...
  | Tcoerce_alias (env, _, c1) -> sub.env sub env; sub.module_coercion sub c1
...
let module_expr sub { mod_desc; mod_env; _ } =
  ...
  | Tmod_apply (mexp1, mexp2, c) ->
    ...
    sub.module_expr sub mexp2; sub.module_coercion sub c
  | Tmod_constraint (mexpr, _, Tmodtype_implicit, c) ->
    sub.module_expr sub mexpr; sub.module_coercion sub c
  | Tmod_constraint (mexpr, _, Tmodtype_explicit mtype, c) ->
    sub.module_expr sub mexpr;
    sub.module_type sub mtype; sub.module_coercion sub c
...
let class_expr sub { cl_desc; cl_env; _ } =
  ...
  | Tcl_constraint (cl, clty, _, _, _) ->
    sub.class_expr sub cl; Option.iter (sub.class_type sub) clty
  | Tcl_structure clstr -> sub.class_structure sub clstr
  | Tcl_fun (_, pat, priv, cl, _) ->
    sub.pat sub pat;
    List.iter (fun (_, e) -> sub.expr sub e) priv; sub.class_expr sub cl
  | Tcl_apply (cl, args) ->
  ...
  | Tcl_let (rec_flag, value_bindings, ivars, cl) ->
    ...
    List.iter (fun (_, e) -> sub.expr sub e) ivars; sub.class_expr sub cl
  | Tcl_ident (_, _, tyl) -> List.iter (sub.typ sub) tyl
  | Tcl_open (od, e) -> sub.open_description sub od; sub.class_expr sub e
...
let class_type sub { cltyp_desc; cltyp_env; _ } =
  ...
  | Tcty_arrow (_, ct, cl) -> sub.typ sub ct; sub.class_type sub cl
  | Tcty_open (od, e) -> sub.open_description sub od; sub.class_type sub e

let class_signature sub { csig_self; csig_fields; _ } =
  sub.typ sub csig_self; List.iter (sub.class_type_field sub) csig_fields
...
let class_type_field sub { ctf_desc; _ } =
  ...
  | Tctf_constraint (ct1, ct2) -> sub.typ sub ct1; sub.typ sub ct2
  | Tctf_attribute _ -> ()
...
let typ sub { ctyp_desc; ctyp_env; _ } =
  ...
  | Ttyp_any -> ()
  | Ttyp_var _ -> ()
  | Ttyp_arrow (_, ct1, ct2) -> sub.typ sub ct1; sub.typ sub ct2
...
let class_structure sub { cstr_self; cstr_fields; _ } =
  sub.pat sub cstr_self; List.iter (sub.class_field sub) cstr_fields
...
let class_field sub { cf_desc; _ } =
  ...
  | Tcf_constraint (cty1, cty2) -> sub.typ sub cty1; sub.typ sub cty2
...
let case sub { c_lhs; c_guard; c_rhs } =
  sub.pat sub c_lhs; Option.iter (sub.expr sub) c_guard; sub.expr sub c_rhs

let value_binding sub { vb_pat; vb_expr; _ } =
  sub.pat sub vb_pat; sub.expr sub vb_expr
