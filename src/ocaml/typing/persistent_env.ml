...
let clear penv =
  let
    { persistent_structures; imported_units; imported_opaque_units; crc_units;
      can_load_cmis; short_paths_basis } =
    penv
  in
...
let import_crcs penv ~source crcs =
  ...
  let import_crc (name, crco) =
    ...
    | None -> ()
    | Some crc ->
      add_import penv name; Consistbl.check crc_units name crc source
  in
...
let short_paths_basis penv = !(penv.short_paths_basis)

let without_cmis penv f x =
  let log = Lazy_backtrack.log () in
  let res =
    Misc.(
      protect_refs [ R (penv.can_load_cmis, Cannot_load_cmis log) ] (fun () ->
        f x
      )
    )
  in
  Lazy_backtrack.backtrack log; res

let fold { persistent_structures; _ } f x =
  Hashtbl.fold (fun modname pso x ->
    match pso with Missing -> x | Found (_, pm) -> f modname pm x
  ) persistent_structures x

let register_pers_for_short_paths penv ps components =
  let (deps, alias_deps) =
    List.fold_left (fun (deps, alias_deps) (name, digest) ->
  ...
  in
  let desc = Short_paths.Desc.Module.(Fresh (Signature components)) in
  let is_deprecated =
  ...
  in
  Short_paths.Basis.load (short_paths_basis penv) ps.ps_name deps alias_deps
    desc deprecated
...
let save_pers_struct penv crc ps pm =
  ...
  Consistbl.set crc_units modname crc ps.ps_filename; add_import penv modname
...
let acknowledge_pers_struct penv short_path_comps check modname pers_sig pm =
  ...
  List.iter (function
    ...
    | Unsafe_string ->
      if Config.safe_string then error (Depend_on_unsafe_string_unit ps.ps_name)
    | Alerts _ -> ()
    | Opaque -> register_import_as_opaque penv modname
  ) ps.ps_flags;
  ...
  Hashtbl.add persistent_structures modname (Found (ps, pm));
  register_pers_for_short_paths penv ps (short_path_comps ps.ps_name pm); ps
  ...
  | exception Not_found ->
    ...
    | Can_load_cmis ->
      let psig =
        ...
        | Some psig -> psig
        | None ->
          Hashtbl.add persistent_structures name Missing; raise Not_found
      in
      add_import penv name;
      let pm = val_of_pers_sig psig in
      let ps = acknowledge_pers_struct penv short_path_comps check name psig pm
      in
      ps, pm
    end
      ...
      | Illegal_renaming (name, ps_name, filename) ->
        Format.asprintf
          " %a@ contains the compiled interface for @ \
           %s when %s was expected" Location.print_filename filename ps_name
          name
...
let find penv f1 f2 name = snd (find_pers_struct penv f1 f2 true name)
    ...
    (add_import penv name;
     if Warnings.is_active (Warnings.No_cmi_file ("", None)) then
       !add_delayed_check_forward (fun () ->
         check_pers_struct penv f1 f2 ~loc name
       ))
...
let save_cmi penv psig pm =
  ...
  Misc.try_finally (fun () ->
    let
      { cmi_name = modname; cmi_sign = _; cmi_crcs = imports; cmi_flags = flags
      } =
      cmi
    in
    let crc =
  ...
  | Illegal_renaming (modname, ps_name, filename) ->
    fprintf ppf
      "Wrong file naming: %a@ contains the compiled interface for@ \
       %s when %s was expected" Location.print_filename filename ps_name modname
...
let forall ~found ~missing t =
  Std.Hashtbl.forall t.persistent_structures (fun name ->
    function
