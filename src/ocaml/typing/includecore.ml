...
and compare_variants
    ~loc env params1 params2 n (cstrs1 : Types.constructor_declaration list)
...
=
  ...
  | cd1 :: rem1, cd2 :: rem2 ->
    if Ident.name cd1.cd_id <> Ident.name cd2.cd_id then
      ...
      (Builtin_attributes.check_alerts_inclusion ~def:cd1.cd_loc ~use:cd2.cd_loc
         loc cd1.cd_attributes cd2.cd_attributes (Ident.name cd1.cd_id);
       match
...
=
  if ld1.ld_mutable <> ld2.ld_mutable then
    ...
    Ctype.equal env true (ld1.ld_type :: params1) (ld2.ld_type :: params2)
  then
    None
  else
...
and compare_records
    ~loc env params1 params2 n (labels1 : Types.label_declaration list)
...
let compare_records_with_representation
    ~loc env params1 params2 n labels1 labels2 rep1 rep2
=
...
let type_declarations ?(equality=false) ~loc env ~mark name decl1 path decl2 =
  ...
  if decl1.type_arity <> decl2.type_arity then
    Some Arity
  else if not (private_flags decl1 decl2) then
    Some Privacy
  else
    let err =
      ...
      | None, Some ty2 ->
        ...
        in
        if Ctype.equal env true decl1.type_params decl2.type_params then
          (if Ctype.equal env false [ ty1 ] [ ty2 ] then None else Some Manifest)
        else
          Some Constraint
    in
    if err <> None then
            ...
            (if mark then
               ...
               let usage =
                 if decl2.type_private = Public then
                   ...
                   Env.Privatize
               in
               (mark usage cstrs1; if equality then mark Env.Positive cstrs2));
            Option.map (fun var_err -> Variant_mismatch var_err)
...
let extension_constructors ~loc env ~mark id ext1 ext2 =
  ...
  if
    not
      (Ctype.equal env true (ty1 :: ext1.ext_type_params)
