...
let rec generalize_class_type gen =
  ...
  | Cty_constr (_, params, cty) ->
    List.iter gen params; generalize_class_type gen cty
  ...
  | Cty_arrow (_, ty, cty) -> gen ty; generalize_class_type gen cty
...
let virtual_methods sign =
  ...
  in
  List.fold_left (fun virt (lab, _, _) ->
    if lab = dummy_method then
      virt
    else if Concr.mem lab sign.csig_concr then
      virt
    else
      lab :: virt
  ) [] fields
...
let extract_constraints cty =
  ...
  (let (fields, _) = Ctype.flatten_fields (Ctype.object_fields sign.csig_self)
   in
   List.fold_left (fun meths (lab, _, _) ->
...
let rec limited_generalize rv =
  ...
  | Cty_constr (_path, params, cty) ->
    List.iter (Ctype.limited_generalize rv) params; limited_generalize rv cty
  | Cty_signature sign ->
...
let enter_val cl_num vars inh lab mut virt ty class_env loc =
  ...
  in
  vars := Vars.add lab (id, mut, virt, ty) !vars; result

let concr_vals vars =
  Vars.fold (fun id (_, vf, _) s -> if vf = Virtual then s else Concr.add id s)
    vars Concr.empty
...
let inheritance self_type env ovf concr_meths warn_vals loc parent =
  ...
  | Cty_signature cl_sig ->
    ...
    | Some Fresh ->
      let cname =
        match parent with Cty_constr (p, _, _) -> Path.name p | _ -> "inherited"
      in
    ...
    and warn_vals = Concr.union concr_vals warn_vals
    in
...
and class_type_field_aux
    env self_type meths (fields, val_sig, concr_meths, inher) ctf
=
...
and class_field_aux
    self_loc cl_num self_type meths vars
...
=
  ...
  | Pcf_inherit (ovf, sparent, super) ->
    ...
    let (class_env, inh_vars) =
      Vars.fold (fun lab info (class_env, inh_vars) ->
  ...
  | Pcf_val (lab, mut, Cfk_virtual styp) ->
    ...
    (if !Clflags.principal then
       (Ctype.end_def (); Ctype.generalize_structure ty));
    let (id, class_env') =
  ...
  | Pcf_method (lab, priv, Cfk_concrete (ovf, expr)) ->
    ...
    let (_, ty) = Ctype.filter_self_method val_env lab.txt priv meths self_type
    in
    (try
       match expr.pexp_desc with
       | Pexp_poly (sbody, sty) ->
    ...
    let field =
      lazy
        (Ctype.raise_nongen_level ();
         ...
         Ctype.end_def (); mkcf (Tcf_initializer texp))
    in
    class_env,
    field :: fields,
    concr_meths,
...
=
  ...
  (if final then
   (* Unify private_self and a copy of self_type. self_type will not
     ...
     ((if not (Ctype.close_object self_type) then
      ...
      let vals =
        Vars.fold (fun name (_mut, vr, _ty) l ->
          if vr = Virtual then name :: l else l
        ) sign.csig_vars []
      in
      (if mets <> [] || vals <> [] then
         raise (Error (loc, val_env, Virtual_class (true, final, mets, vals))));
      let self_methods =
        List.fold_right (fun (lab, kind, ty) rem ->
  ...
  let l1 = names priv_meths
  and l2 = names pub_meths'
  in
  ...
  let sign =
    if final then
      ...
      { sign with  Types.csig_self = Ctype.expand_head val_env public_self }
  in
  {
    cstr_self = pat;
    cstr_fields = fields;
...
and class_expr_aux cl_num val_env met_env scl =
  ...
  | Pcl_apply (scl', sargs) ->
    ...
    let cl = class_expr cl_num val_env met_env scl' in
    (if !Clflags.principal then
       (Ctype.end_def (); generalize_class_type false cl.cl_type));
  ...
  | Pcl_let (rec_flag, sdefs, scl') ->
    ...
    in
    let (vals, met_env) =
      List.fold_right (fun (id, _id_loc, _typ) (vals, met_env) ->
        let path = Pident id in
...
=
  ...
  let (expr, typ) =
    try
      ...
      Typecore.self_coercion := List.tl !Typecore.self_coercion; res
    with
    | exn -> Typecore.self_coercion := []; raise exn
  in
  Ctype.end_def ();
  ...
  | Some reason ->
    let printer =
      if define_class then
        (function ppf -> Printtyp.class_declaration id ppf clty)
      else
  ...
  | loc :: _ ->
    let (cl_ty, obj_ty) =
      ...
      | Some cl_ab, Some obj_ab ->
        ...
        in
        List.iter2 (Ctype.unify env) cl_params obj_params; cl_ty, obj_ty
      | _ -> assert false
    in
...
let class_declarations env cls =
  ...
  in
  check_recursive_class_bindings env ids exprs; info, env
...
let type_object env loc s =
  ...
  unify_parents_struct env sign.csig_self desc; desc, sign, meths
...
let report_error env ppf =
  ...
  | Unconsistent_constraint trace ->
    ...
    Printtyp.report_unification_error ppf env trace (fun ppf ->
      fprintf ppf "Type"
  ...
  | Abbrev_type_clash (abbrev, actual, expected) ->
    ...
    fprintf ppf
      "@[The abbreviation@ %a@ expands to type@ %a@ \
       but is used with type@ %a@]" !Oprint.out_type
      (Printtyp.tree_of_typexp false abbrev) !Oprint.out_type
      (Printtyp.tree_of_typexp false actual) !Oprint.out_type
  ...
  | Virtual_class (cl, imm, mets, vals) ->
    ...
    let print_msg ppf =
      if imm then
        fprintf ppf "This object has virtual %s" missings
      else if cl then
        fprintf ppf "This class should be virtual"
      else
        fprintf ppf "This class type should be virtual"
    in
  ...
  | Parameter_arity_mismatch (lid, expected, provided) ->
    fprintf ppf
      "@[The class constructor %a@ expects %i type argument(s),@ \
         but is here applied to %i type argument(s)@]" Printtyp.longident lid
      expected provided
  ...
  | Bad_parameters (id, params, cstrs) ->
    ...
    fprintf ppf
      "@[The abbreviation %a@ is used with parameters@ %a@ \
         which are incompatible with constraints@ %a@]" Printtyp.ident id
      !Oprint.out_type (Printtyp.tree_of_typexp false params) !Oprint.out_type
  ...
  | Unbound_type_var (printer, reason) ->
    let print_common ppf kind ty0 real lab ty =
      ...
      fprintf ppf "The %s %s@ has type@;<1 2>%a@ where@ %a@ is unbound" kind lab
        !Oprint.out_type (Printtyp.tree_of_typexp false ty) !Oprint.out_type
        (Printtyp.tree_of_typexp false ty0)
    in
    let print_reason ppf =
      function
  ...
  | Cannot_coerce_self ty ->
    fprintf ppf
      "@[The type of self cannot be coerced to@ \
         ...
         Some occurrences are contravariant@]" Printtyp.type_scheme ty
  | Non_collapsable_conjunction (id, clty, trace) ->
    fprintf ppf
      "@[The type of this class,@ %a,@ \
         contains non-collapsible conjunctive types in constraints.@ %t@]"
      (Printtyp.class_declaration id) clty (fun ppf ->
      Printtyp.report_unification_error ppf env trace (fun ppf ->
        fprintf ppf "Type"
      ) (fun ppf -> fprintf ppf "is not compatible with type")
    )
