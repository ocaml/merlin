...
module Conflicts = struct
  ...
  let list_explanations () =
    ...
    reset (); c |> M.bindings |> List.map snd |> List.sort Stdlib.compare
      ...
      | [ namespace, a ] ->
        Format.fprintf ppf
          "@ \
           ...
           @ Did you try to redefine them?@]" Namespace.pp namespace a
      ...
      | (namespace, _) :: _ :: _ ->
        Format.fprintf ppf
          "@ \
           ...
           @ Did you try to redefine them?@]" pp_namespace_plural namespace
...
let non_shadowed_pervasive =
  function Pdot (Pident id, _) -> Ident.same id ident_stdlib | _ -> false
...
let print_name ppf =
  function None -> fprintf ppf "None" | Some name -> fprintf ppf "\"%s\"" name

let string_of_label =
  function Nolabel -> "" | Labelled s -> s | Optional s -> "?" ^ s
...
let raw_type_expr ppf t =
  visited := [];
  kind_vars := [];
  kind_count := 0; raw_type ppf t; visited := []; kind_vars := []
...
type type_result =
  Short_paths.type_result
  =
  | Nth of int
  | Path of int list option * Path.t

type type_resolution =
  Short_paths.type_resolution
  =
  | Nth of int
  | Subst of int list
  | Id
...
let apply_subst_opt nso args =
  match nso with None -> args | Some ns -> apply_subst ns args

let apply_nth n args = List.nth args n

let best_type_path p =
  if !Clflags.real_paths || !printing_env == Env.empty then
    Path (None, p)
  else
...
let rec new_name () =
  ...
  in
  incr name_counter; if name_is_already_used name then new_name () else name
...
let rec new_weak_name ty () =
  ...
  if name_is_already_used name then
    ...
    (named_weak_vars := String.Set.add name !named_weak_vars;
     weak_var_map := TypeMap.add ty name !weak_var_map; name)
...
let add_alias ty =
  let px = proxy ty in
  if not (is_aliased px) then (aliased := px :: !aliased; add_named_var px)

let aliasable ty =
  match ty.desc with
  | Tvar _ | Tunivar _ | Tpoly _ -> false
  | Tconstr (p, _, _) ->
    begin match best_type_path_resolution p with
    | Nth _ -> false
    | Subst _ | Id -> true
    end
  | _ -> true
...
let rec mark_loops_rec visited ty =
  ...
  if List.memq px visited && aliasable ty then
    ...
    | Tconstr (p, tyl, _) ->
      begin match best_type_path_resolution p with
      | Nth n -> mark_loops_rec visited (apply_nth n tyl)
      | Subst ns -> List.iter (mark_loops_rec visited) (apply_subst ns tyl)
      | Id -> List.iter (mark_loops_rec visited) tyl
      end
    ...
    | Tpoly (ty, tyl) ->
      List.iter (fun t -> add_alias t) tyl; mark_loops_rec visited ty
    | Tunivar _ -> add_named_var ty

let mark_loops ty = normalize_type ty; mark_loops_rec [] ty
  ...
  if List.mem_assq px !names && not (List.memq px !delayed) then
    ...
    let pr_typ () =
      ...
      | Tvariant row ->
        ...
        | _ ->
          let non_gen = not (row.row_closed && all_present) && is_non_gen sch px
          in
...
let type_expr ppf ty =
  ...
  reset_and_mark_loops ty; marked_type_expr ppf ty
...
let filter_params tyl =
  let params =
    List.fold_left (fun tyl ty ->
      let ty = repr ty in
      if List.memq ty tyl then Btype.newgenty (Tsubst ty) :: tyl else ty :: tyl
    ) [] tyl
  in
  List.rev params
...
let rec tree_of_type_decl id decl =
  ...
  let type_defined decl =
    ...
    in
    Ident.name id,
    List.map2 (fun ty cocn -> type_param (tree_of_typexp false ty), cocn) params
      vari
  in
...
and tree_of_constructor cd =
  ...
  | Some res ->
    ...
    let args = arg () in
    names := nm; name, args, Some ret

and tree_of_label l =
  Ident.name l.ld_id, l.ld_mutable = Mutable, tree_of_typexp false l.ld_type

let constructor ppf c =
  reset_except_context (); !Oprint.out_constr ppf (tree_of_constructor c)

let label ppf l =
  reset_except_context (); !Oprint.out_label ppf (tree_of_label l)
...
let tree_of_value_description id decl =
  ...
  in
  let vd = match decl.val_kind with Val_prim p -> Primitive.print p vd | _ -> vd
  in
  Osig_value vd
...
let tree_of_metho sch concrete csil (lab, kind, ty) =
  if lab <> dummy_method then
    ...
    (remove_names tyl; Ocsg_method (lab, priv, virt, tty) :: csil)
  else
    csil
...
let rec prepare_class_type params =
  ...
  | Cty_constr (_p, tyl, cty) ->
    ...
    if
      List.memq (proxy sty) !visited_objects ||
        not (List.for_all is_Tvar params) || List.exists (deep_occur sty) tyl
    then
  ...
  | Cty_signature sign ->
    ...
    let (fields, _) = Ctype.flatten_fields (Ctype.object_fields sign.csig_self)
    in
    List.iter (fun met -> mark_loops (fst (method_type met))) fields;
    Vars.iter (fun _ (_, _, ty) -> mark_loops ty) sign.csig_vars
  | Cty_arrow (_, ty, cty) -> mark_loops ty; prepare_class_type params cty
...
let rec tree_of_class_type sch params =
  ...
  | Cty_signature sign ->
    ...
    let csil =
      List.fold_left (fun csil (l, m, v, t) ->
...
let tree_of_class_param param variance =
  begin match tree_of_typexp true param with
  | Otyp_var (_, s) -> s
  | _ -> "?"
  end,
  (if is_Tvar (repr param) then
     Asttypes.(NoVariance, NoInjectivity)
   else
     variance)
...
let hide ids env =
  List.fold_right (fun id ->
    Env.add_type ~check:false (Ident.rename_no_exn id) dummy
  ) ids env
...
let protect_rec_items items =
  let rec get_ids recs =
    function
    | [] -> []
    | item :: rem ->
      begin match recursive_sigitem item with
...
let stop_type_group env =
  Naming_context.reset_protected (); set_printing_env env
...
let still_in_type_group env' in_type_group item =
  ...
  | _, Some (_, (Trec_not | Trec_first), _) ->
    let env' = Env.update_short_paths env' in
    set_printing_env env'; stop_type_group env'; true
  | _ ->
    let env' = Env.update_short_paths env' in
    set_printing_env env'; stop_type_group env'; false
...
let refresh_weak () =
  ...
  in
  named_weak_vars := s; weak_var_map := m

let print_items showval env x =
  refresh_weak ();
  reset_naming_context ();
  Conflicts.reset ();
  let rec print showval in_type_group env =
    function
    | [] -> stop_type_group env; []
    | item :: rem as items ->
      let in_type_group = still_in_type_group env in_type_group item in
...
let trees_of_type_expansion (t, t') =
  if same_path t t' then
    (add_delayed (proxy t); Same (tree_of_typexp false t))
  else
...
let prepare_expansion (t, t') =
  ...
  mark_loops t; (if not (same_path t t') then mark_loops t'); t, t'
...
let explain_fixed_row pos expl =
  ...
  | Types.Reified p ->
    ...
    dprintf "The %a variant type is bound to %a" print_pos pos !Oprint.out_ident
      p
  | Types.Rigid -> ignore
  ...
  let pre =
    ...
    | None ->
      ...
      | Trace.Univ _, Some (Trace.Incompatible_fields { name; diff }) ->
        dprintf
          "@,@[The method %s has type@ %a,@ \
           but the expected method type was@ %a@]" name type_expr diff.Trace.got
          type_expr diff.Trace.expected
      | _ -> ignore
      end
  in
  match e with
...
let explanation intro prev env =
  function
...
let head_error_printer txt_got txt_but =
  function
  | None -> ignore
  | Some d ->
...
let warn_on_missing_defs env ppf =
  ...
  | Some { Trace.got = te1, _; expected = te2, _ } ->
    warn_on_missing_def env ppf te1; warn_on_missing_def env ppf te2
  ...
  | elt :: tr ->
    (try
       ...
       fprintf ppf
         "@[<v>\
           @[%t%t@]%a%t\
          @]" head_error ty_expect_explanation
         (trace false "is not compatible with type") tr (explain mis);
       (if env <> Env.empty then warn_on_missing_defs env ppf head);
       Conflicts.print_explanations ppf; print_labels := true
     with
     | exn -> print_labels := true; raise exn)

let report_unification_error
    ppf env tr ?(type_expected_explanation=(fun _ -> ())) txt1 txt2
=
  ...
  wrap_printing_env ~error:true env (fun () ->
    ...
    | [ tp ] ->
      fprintf ppf
        "@[%t@;<1 2>%a@ \
           %t@;<1 2>%a\
         @]" txt1 type_path_expansion (trees_of_type_path_expansion tp) txt3
