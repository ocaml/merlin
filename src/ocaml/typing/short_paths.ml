...
module Rev_deps : sig
  ...
  module Stamp = Natural.Make(struct end) 
  ...
  type t = { mutable stamp : Stamp.t; mutable items : item Dependency.Array.t }
  
  let create () = { stamp = Stamp.one; items = Dependency.Array.empty }
  
  let extend_up_to t next =
    match Dependency.pred next with
    | None -> ()
    | Some curr ->
      if not (Dependency.Array.contains t.items curr) then
        let items =
          Dependency.Array.extend t.items curr (fun _ ->
            {
              set = Dependency.Set.empty;
              edges = [];
              alias_edges = [];
              last = Stamp.zero
            }
  ...
  let add t ~source ~target =
    ...
    item.edges <- target :: item.edges; t.stamp <- Stamp.succ t.stamp
  ...
  let update t dep item =
    if Stamp.less_than item.last t.stamp then
      let rec add_edges t item acc =
        let rec loop t acc added =
          function
          | [] ->
          ...
          | dep :: rest ->
            if Dependency.Set.mem dep acc then
      ...
      (item.set <- set; item.last <- t.stamp)
  
  let get t dep =
    let item = Dependency.Array.get t.items dep in
    update t dep item; item.set
  ...
  let before t origin1 origin2 =
    ...
    | Dependency dep1, Dependencies deps2 ->
      let rev_dep = get t dep1 in
      List.exists (fun dep2 -> Dependency.Set.mem dep2 rev_dep) deps2
    | Dependencies _, Environment _ -> true
    | Dependencies deps1, Dependency dep2 ->
      List.for_all (fun dep1 -> Dependency.Set.mem dep2 (get t dep1)) deps1
    | Dependencies deps1, Dependencies deps2 ->
      let rev_dep =
        match deps1 with
        | [] -> failwith "Rev_deps.before: invalid origin"
        | dep1 :: deps1 ->
          List.fold_left (fun acc dep1 -> Dependency.Set.inter acc (get t dep1))
            (get t dep1) deps1
      in
      List.exists (fun dep2 -> Dependency.Set.mem dep2 rev_dep) deps2
end
...
module Origin_range_tbl = struct
  ...
  let add rev_deps origin data t =
    ...
    | Origin.Dependencies deps ->
      ...
      in
      begin match
        List.find (fun dep -> Dependency.Set.mem dep rev_dep) deps
      with
      | dep -> add_dependency dep data t
      | exception Not_found ->
        begin match Dependency.Set.choose rev_dep with
        | dep -> add_dependency dep data t
  ...
  let pop_dependency rev_dep t =
    ...
    let items =
      Dependency.Set.fold (fun dep acc ->
        let data = Dependency.Tbl.find t.deps dep in
        Dependency.Tbl.remove t.deps dep; List.rev_append data acc
      ) matching []
    in
    let items =
      Age.Map.fold (fun _ data acc -> List.rev_append data acc) t.envs items
    in
    t.envs <- Age.Map.empty; items
  
  let pop_age age t =
    let (envs, first, matching) = Age.Map.split age t.envs in
    let items = match first with None -> [] | Some first -> first in
    let items =
      Age.Map.fold (fun _ data acc -> List.rev_append data acc) matching items
    in
    t.envs <- envs; items
  ...
  let pop rev_deps origin t =
    ...
    | Origin.Dependencies deps ->
      ...
      in
      pop_dependency rev_dep t
    | Origin.Environment age -> pop_age age t
  
  let is_origin_empty rev_deps origin t =
    match origin with
    | Origin.Dependency dep ->
      if not (Age.Map.is_empty t.envs) then
        false
      else
        let rev_dep = Rev_deps.get rev_deps dep in
        let matching = Dependency.Set.inter rev_dep t.dep_keys in
    ...
    | Origin.Dependencies deps ->
      if not (Age.Map.is_empty t.envs) then
        ...
        let rev_dep =
          match rev_dep_opt with
          | None -> failwith "Origin_range_tbl.is_origin_empty: invalid origin"
          | Some rev_dep -> rev_dep
        in
        let matching = Dependency.Set.inter rev_dep t.dep_keys in
        Dependency.Set.is_empty matching
    | Origin.Environment age ->
      begin match Age.Map.max_binding t.envs with
      | exception Not_found -> true
      | max, _ -> Age.less_than max age
      end
  
  let is_completely_empty t =
    Age.Map.is_empty t.envs && Dependency.Set.is_empty t.dep_keys
end
  

module Height = Natural.Make_no_zero(struct end) 

module Todo = struct
  module Item = struct
    type t =
      | Base of Diff.Item.t
      | Children of { md : Module.t; path : Path.t; seen : Path_set.t }
      | Update of { id : Ident.t; origin : Origin.t }
      | Forward of { id : Ident.t; decl : Origin.t; origin : Origin.t }
  end
    
  
  type t = { mutable table : Item.t Origin_range_tbl.t Height.Array.t }
  ...
  let create graph rev_deps diff =
    ...
    List.iter (fun item ->
      ...
      | None -> Origin_range_tbl.add rev_deps origin (Item.Base item) tbl
      | Some decl ->
  ...
  let get_table t height =
    (if not (Height.Array.contains t.table height) then
       t.table <-
         Height.Array.extend t.table height (fun _ ->
           Origin_range_tbl.create ()
         ));
  ...
  let retract_empty t =
    let rec loop height =
      ...
      | None -> t.table <- Height.Array.empty
      | Some prev ->
        let tbl = Height.Array.get t.table prev in
        if Origin_range_tbl.is_completely_empty tbl then
          loop prev
        else
    ...
    | Some last ->
      ...
      if Origin_range_tbl.is_completely_empty tbl then loop last else ()
  ...
  let pop rev_deps t height origin =
    match get_table_opt t height with
    | None -> retract_empty t; None
    | Some tbl ->
      begin match Origin_range_tbl.pop rev_deps origin tbl with
      | [] ->
        let empty_from = is_empty_from rev_deps t (Height.succ height) origin in
        if not empty_from then Some [] else (retract_empty t; None)
...
module Forward_path_map : sig
  ...
  let find t path =
    ...
    | new_paths ->
      begin match Path_map.find path t.old_paths with
  ...
  let rebase t base =
    let old_paths =
      Path_map.union (fun _ paths1 paths2 -> Some (paths1 @ paths2))
        base.new_paths base.old_paths
    in
    let forwards =
      Ident_map.union (fun _ pset1 pset2 -> Some (Path_set.union pset1 pset2))
...
module History : sig
  ...
  type t = { mutable head : Revision.t }
  ...
  let commit t rev_deps diff =
    ...
    head.Revision.next <- Some rev; t.head <- rev
end
...
module Shortest = struct
  module Section = struct
    ...
    let rebase t parent =
      ...
      t.module_types <-
        Forward_path_map.rebase t.module_types parent.module_types;
  ...
  module Sections = struct
    type range = Until of Height.t | All
    ...
    let create age origin =
      ...
      let (initialised, versioning) =
        if Age.equal age Age.zero then
          All, Completion History.Stamp.zero
        else
          match origin with
          | Origin.Environment age' ->
            let initialised =
              if Age.less_than_or_equal age age' then All else Until Height.one
            in
    ...
    let update t stamp =
      ...
      | Initialisation initialised ->
        if History.Stamp.less_than initialised stamp then
          (t.initialised <- Until Height.one;
           t.versioning <- Initialisation stamp)
      | Completion completed ->
        if History.Stamp.less_than completed stamp then
          (t.completed <- Until Height.one; t.versioning <- Completion stamp)
    
    let expand t height =
      let sections = t.sections in
      if not (Height.Array.contains sections height) then
        let sections =
          Height.Array.extend sections height (fun _ -> Section.create ())
        in
        (t.sections <- sections; sections)
      else
        sections
    ...
    let set_initialised t height =
      match t.initialised with
      | All -> failwith "Section.set_initialised: already initialised"
      | Until until ->
        (if not (Height.equal until height) then
    ...
    let set_completed t height =
      match t.completed with
      | All -> failwith "Section.set_completed: already completed"
      | Until until ->
        (if not (Height.equal until height) then
           (if Height.less_than until height then
    ...
    let is_finished t =
      match t.initialised, t.completed with All, All -> true | _, _ -> false
    ...
    let check_versions t parent =
      ...
      | (Completion stamp
         | Initialisation stamp),
        (Completion parent_stamp
         | Initialisation parent_stamp)
        ->
    ...
    let initialise t height parent =
      ...
      | Some parent ->
        ...
        Section.rebase section parent; set_initialised t height
      | None ->
    ...
    let iter_forwards ~type_ ~class_type ~module_type ~module_ t height id =
      ...
      match get t height with
      | Some section ->
        Section.iter_forwards ~type_ ~class_type ~module_type ~module_ section
          id;
        true
      | None -> not all_initialised
    
    type result = Not_found_here | Not_found_here_or_later | Found of Path.t
    
    let rec get_visible_type graph =
      function
      | [] -> None
      | path :: rest ->
        let visible = Graph.is_type_path_visible graph path in
        if visible then Some path else get_visible_type graph rest
    
    let rec get_visible_class_type graph =
      function
      | [] -> None
      | path :: rest ->
        let visible = Graph.is_class_type_path_visible graph path in
        if visible then Some path else get_visible_class_type graph rest
    
    let rec get_visible_module_type graph =
      function
      | [] -> None
      | path :: rest ->
        let visible = Graph.is_module_type_path_visible graph path in
        if visible then Some path else get_visible_module_type graph rest
    
    let rec get_visible_module graph =
      function
      | [] -> None
      | (path, _) :: rest ->
        let visible = Graph.is_module_path_visible graph path in
        if visible then Some path else get_visible_module graph rest
    ...
    let find_type graph t height typ =
      ...
      match get t height with
      | Some section ->
        begin match Section.find_type graph section typ with
        | exception Not_found -> Not_found_here
        | paths ->
          begin match get_visible_type graph paths with
          | None -> Not_found_here
          | Some path -> Found path
          end
        end
      | None ->
        if is_finished t then Not_found_here_or_later else Not_found_here
    ...
    let find_class_type graph t height mty =
      ...
      match get t height with
      | Some section ->
        begin match Section.find_class_type graph section mty with
        | exception Not_found -> Not_found_here
        | paths ->
          begin match get_visible_class_type graph paths with
          | None -> Not_found_here
          | Some path -> Found path
          end
        end
    ...
    let find_module_type graph t height mty =
      ...
      match get t height with
      | Some section ->
        begin match Section.find_module_type graph section mty with
        | exception Not_found -> Not_found_here
        | paths ->
          begin match get_visible_module_type graph paths with
          | None -> Not_found_here
          | Some path -> Found path
          end
        end
    ...
    let find_module graph t height md =
      ...
      match get t height with
      | Some section ->
        begin match Section.find_module graph section md with
        | exception Not_found -> Not_found_here
        | paths ->
          begin match get_visible_module graph paths with
          | None -> Not_found_here
          | Some path -> Found path
          end
        end
  ...
  type _ kind =
    | Basis : { history : History.t } -> basis kind
    | Env
        : { mutable revision : History.Revision.t; parent : 'a t; age : Age.t }
        -> env kind
  ...
  let age (type k) (t : k t) =
    match t.kind with Basis _ -> Age.zero | Env { age; _ } -> age
  ...
  let stamp t = History.Revision.stamp (revision t)
  let rev_deps t = History.Revision.rev_deps (revision t)
    ...
    | Env ({ revision } as e) ->
      let rec loop graph revision =
        ...
        | Some revision ->
          ...
          Todo.merge graph rev_deps t.todos diff; loop graph revision
      in
      let (revision, graph) = loop t.graph revision in
      t.graph <- graph; e.revision <- revision
  ...
  let local_or_open conc =
    match conc with Desc.Local -> Component.Local | Desc.Open -> Component.Open
  ...
  let env parent desc =
    ...
    let components =
      List.map (fun desc ->
        ...
        | Desc.Declare_type id -> Component.Declare_type (origin, id)
        ...
        | Desc.Declare_module id -> Component.Declare_module (origin, id)
      ) desc
    in
  ...
  let process_module t height path seen md =
    ...
    if not (Path_set.mem canonical_path seen) then
  ...
  let process_children t height path seen md =
    ...
    let modules =
      ...
      | None -> String_map.empty
    in
    String_map.iter (fun name typ ->
      if not (Type.hidden typ) then
        let path = Path.Pdot (path, name) in
        process_type t height path typ
    ) types;
    String_map.iter (fun name clty ->
      if not (Class_type.hidden clty) then
        let path = Path.Pdot (path, name) in
        process_class_type t height path clty
    ) class_types;
    String_map.iter (fun name mty ->
      if not (Module_type.hidden mty) then
        let path = Path.Pdot (path, name) in
        process_module_type t height path mty
    ) module_types;
    String_map.iter (fun name md ->
      if not (Module.hidden md) then
        let path = Path.Pdot (path, name) in
    ...
    fun t origin height ->
      ...
      | Some items ->
        List.iter (function
          ...
          | Todo.Item.Update { id; origin } -> process_update t origin height id
  ...
  and process_update : 'k. 'k t -> _ =
    fun t origin height id ->
      let sections = sections t origin in
      let more =
        Sections.iter_updates sections height id ~type_:(fun canon path ->
          ...
          | None -> ()
          | Some seen -> process_module t height path seen md
        )
      in
      if more then Todo.add_next_update (rev_deps t) t.todos height origin id
  
  and process_forward : 'k. 'k t -> _ =
    fun t origin height id decl ->
      let sections = init t decl height in
      let more =
        Sections.iter_forwards sections height id ~type_:(fun canon path ->
          let typ = Graph.find_type t.graph canon in
          process_type t height path typ
        ) ~class_type:(fun canon path ->
          let clty = Graph.find_class_type t.graph canon in
  ...
  and initialise : type k. k t -> _ =
    fun t sections origin height ->
      if not (Sections.is_initialised sections height) then
        (begin match Height.pred height with
         ...
         let parent =
           ...
           | Env { parent; _ } -> update parent; force parent origin height
         in
  ...
  and init : 'k. 'k t -> _ =
    fun t origin height ->
      ...
      initialise t sections origin height; sections
  
  and complete : 'k. 'k t -> _ =
    fun t sections origin height ->
      if not (Sections.is_completed sections height) then
        (begin match Height.pred height with
         | None -> ()
         ...
         if finished then
           Sections.set_completed_from sections height
         else
           Sections.set_completed sections height)
  ...
  and force : 'k. 'k t -> _ =
    fun t origin height ->
      ...
      complete t sections origin height; sections
  ...
  module Search = struct
    ...
    type name = { name : string; height : Height.t }
    ...
    let min_application fst snd = Height.plus (min_height fst) (min_height snd)
    let max_application fst snd = Height.plus (max_height fst) (max_height snd)
    
    let min_dot parent name =
      let base = min_height parent in
      Height.plus base name.height
    
    let path_application fst snd = Path.Papply (best fst, best snd)
    let path_dot parent name = Path.Pdot (best parent, name.name)
      ...
      let rec loop : type k. k kind -> Path.t -> k t =
        fun kind path ->
          ...
          | Path.Pdot (parent, name) ->
            ...
            let finished = false in
            let name_height = if not hidden then Height.one else Height.maximum
            in
          ...
          | Path.Papply (func, arg) ->
            ...
            let func_first =
              Rev_deps.before (rev_deps shortest) (search_origin arg)
                (search_origin func)
            in
            let finished = false in
    ...
    let find (type k) shortest origin height (kind : k kind) (node : k) =
      ...
      match kind with
      | Type -> Sections.find_type shortest.graph sections height node
      | Class_type ->
      ...
      | Module -> Sections.find_module shortest.graph sections height node
    
    let rec step : type k. _ shortest -> k t -> k t =
      fun shortest search ->
        if finished search then
          search
        else
          match search with
          | Ident r ->
            begin match find shortest r.origin r.min r.kind r.node with
          ...
          | Dot r ->
            ...
            let parent =
              let should_try_dot = Height.equal (min_dot parent r.name) r.min in
              if not should_try_dot then parent else step shortest parent
            in
            let found =
              finished parent && Height.equal (min_dot parent r.name) r.min
            in
            if found then
              let best = path_dot parent r.name in
              let max = r.min in
              let finished = true in
              Dot { r with  best; parent; max; finished }
            else
              let (best, max, searched, finished) =
                if r.searched then
                  r.best, r.max, r.searched, r.finished
                else
                  ...
                  | Sections.Found path -> path, r.min, true, true
              in
              let finished =
                finished ||
                  searched &&
                    Height.less_than_or_equal r.max (min_dot parent r.name)
              in
          ...
          | Application r ->
            let try_app searched =
              let (fst, snd) =
                if r.func_first then r.func, r.arg else r.arg, r.func
              in
              let (fst, snd) =
                let should_try_app =
                  Height.equal (min_application fst snd) r.min
                in
                if not should_try_app then
                  fst, snd
                else
                  ...
                  if not should_try_app then
                    ...
                    fst, step shortest snd
              in
              let (func, arg) = if r.func_first then fst, snd else snd, fst in
              let found =
                finished func &&
                  finished arg && Height.equal (min_application fst snd) r.min
              in
              if found then
                let best = path_application func arg in
                let max = r.min in
                let finished = true in
                Application { r with  best; func; arg; max; searched; finished }
              else
                let finished =
                  searched &&
                    Height.less_than_or_equal r.max (min_application fst snd)
                in
                let min = if finished then r.max else Height.succ r.min in
                Application { r with  func; arg; min; searched; finished }
            in
            if r.searched then
              try_app true
            else
              begin match find shortest r.origin r.min r.kind r.node with
              | Sections.Not_found_here -> try_app (Height.equal r.min r.max)
              | Sections.Not_found_here_or_later -> try_app true
              | Sections.Found path ->
                let best = path in
                let max = r.min in
                let searched = true in
    ...
    let rec perform shortest search =
      if finished search then
        ...
        perform shortest (step shortest search)
  end
...
module Basis = struct
  ...
  let update_assignments t additions =
    String_set.iter (fun name ->
      if not (String_map.mem name t.assignment) then
  ...
  let update_rev_deps t loads =
    ...
    List.iter (fun { name; depends; alias_depends; _ } ->
  ...
  let update_shortest t additions loads =
    let components =
      List.map (fun { name; desc; deprecated; _ } ->
        let index = String_map.find name t.assignment in
    ...
    | None -> t.shortest <- Some (Shortest.basis t.rev_deps components)
    | Some shortest -> Shortest.mutate shortest t.rev_deps components
  ...
  let update t =
    ...
    | _, _ ->
      ...
      update_rev_deps t loads; update_shortest t additions loads
  ...
  let shortest t =
    ...
    | None ->
      ...
      t.shortest <- Some shortest; shortest
...
type state =
  | Initial of Basis.t
  | Unforced of { parent : t; desc : Desc.t list Lazy.t }
  | Forced of { basis : Basis.t; shortest : Shortest.env Shortest.t }
...
let add parent desc = ref (Unforced { parent; desc })
...
let shortest t =
  match force t with
  | Unforced _ -> assert false
  | Initial basis -> Basis.update basis; Shortest (Basis.shortest basis)
  | Forced { basis; shortest } -> Basis.update basis; Shortest shortest
