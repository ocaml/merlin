    ...
    if Variance.subset vari vari' then
      ...
      (visited := TypeMap.add ty vari !visited;
       ...
       | Tarrow (_, ty1, ty2, _) ->
         ...
         in
         compute_variance_rec v1 ty1; compute_same ty2
  ...
  (if check then
   (* Check variance of parameters *)
     ...
     (List.iter2 (fun ty (c, n, i) ->
        ...
        and ij = mem Inj var
        in
        if Btype.is_Tvar ty && (co && not c || cn && not n) || not ij && i then
          raise
            (Error
               (loc,
                Bad_variance
...
let for_constr =
  ...
  | Types.Cstr_record l ->
    List.map (fun { Types.ld_mutable; ld_type } ->
...
=
  ...
  | Some ret_type ->
    ...
    | { desc = Tconstr (_, tyl, _) } ->
      ...
      let _ =
        List.fold_left2 (fun (fv1, fv2) ty (c, n, _) ->
          match fv2 with
          | [] -> assert false
          | fv :: fv2 ->
...
let compute_variance_decl env ~check decl (required, _ as rloc) =
  ...
  then
    ...
    let mn = match decl.type_manifest with None -> [] | Some ty -> [ false, ty ]
    in
...
let update_class_decls env cldecls =
  ...
  in
  List.map2 (fun (_, decl) (_, _, cl_abbr, clty, cltydef, _) ->
