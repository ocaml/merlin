...
let type_open_ ?used_slot ?toplevel ovf env loc lid =
  ...
  | Error _ ->
    ...
    ignore (extract_sig_open env lid.loc md.md_type); assert false
...
=
  ...
  let open_module env m =
    ...
    | (Typetexp.Error _
      ...
      ->
      Msupport.raise_error exn; env
    | exn ->
      Printf.ksprintf failwith
        "Uncaught exception %s in initial_env.open_module: %s"
        Obj.Extension_constructor.(name (of_val exn)) (Printexc.to_string exn)
  in
  let add_units env units =
    String.Set.fold (fun name env ->
  ...
  { super with
    ...
    Btype.it_signature =
      (fun self sg ->
         ...
         super.Btype.it_signature self sg; env := env_before);
    Btype.it_module_type =
      (fun self ->
         ...
         | Mty_functor (param, mty_body) ->
           ...
           self.Btype.it_module_type self mty_body; env := env_before
    ...
    { super with
      ...
      it_signature =
        (fun self sg ->
           ...
           check_signature env sg; super.it_signature self sg)
    }
  in
...
let params_are_constrained =
  let rec loop =
    ...
    | [] -> false
    | hd :: tl ->
      begin match (Btype.repr hd).desc with
...
and approx_sig env ssg =
  ...
  | item :: srem ->
    ...
    | Psig_type (rec_flag, sdecls) ->
      ...
      map_rec_type ~rec_flag (fun rs (id, info) ->
        Sig_type (id, info, rs, Exported)
      ) decls rem
...
let mkmty desc typ env loc attrs =
  ...
  in
  Cmt_format.add_saved_type (Cmt_format.Partial_module_type mty); mty

let mksig desc env loc =
  let sg = { sig_desc = desc; sig_loc = loc; sig_env = env } in
  Cmt_format.add_saved_type (Cmt_format.Partial_signature_item sg); sg
...
and transl_modtype_aux env smty =
  ...
  | Pmty_with (sbody, constraints) ->
    ...
    let (rev_tcstrs, final_sg) =
      List.fold_left (fun (rev_tcstrs, sg) sdecl ->
        let (tcstr, sg) =
...
and transl_signature ?(keep_warnings=false) env sg =
    ...
    | item :: srem ->
      ...
      | Psig_value sdesc ->
        begin match
          ...
          in
          Signature_names.check_value names tdesc.val_loc tdesc.val_id; res
        with
        | tdesc, newenv ->
      ...
      | Psig_type (rec_flag, sdecls) ->
        begin match
          let (decls, _ as res) = Typedecl.transl_type_decl env rec_flag sdecls
          in
          List.iter (fun td ->
          ...
          List.iter (fun td ->
            if
              td.ptype_kind <> Ptype_abstract ||
                td.ptype_manifest = None || td.ptype_private = Private
            then
      ...
      | Psig_class cl ->
        ...
        | classes, newenv ->
          ...
          let typedtree =
            mksig
              (Tsig_class
                 (List.map (fun decr -> decr.Typeclass.cls_info) classes)) env
              loc ::
              trem
          in
  ...
  Msupport.with_saved_types
    ...
    ~save_part:(fun sg ->
    Cmt_format.Partial_signature sg
  ) (fun () ->
...
and transl_recmodule_modtypes env sdecls =
  let make_env curr =
    List.fold_left (fun env (id, _, md, _) ->
      Option.fold ~none:env ~some:(fun id ->
        Env.add_module_declaration ~check:true ~arg:true id Mp_present md env
      ) id
    ) env curr
  in
  let transition env_c curr =
    List.map2 (fun pmd (id, id_loc, md, _) ->
      let tmty =
        Builtin_attributes.warning_scope pmd.pmd_attributes (fun () ->
  ...
  in
  let approx_env =
    List.fold_left (fun env ->
      Option.fold ~none:env ~some:(fun id -> (* cf #5965 *)
        Env.enter_unbound_module (Ident.name id) Mod_unbound_illegal_recursion
          env
      )
    ) env ids
  in
  let init =
    List.map2 (fun id pmd ->
      let md =
        {
...
let check_recmodule_inclusion env bindings =
  ...
  let rec check_incl first_time n env s =
    ...
    if n > 0 then
      ...
      let bindings1 =
        List.map
          ...
          let ids =
            Option.map (fun id ->
              id, Ident.create_scoped ~scope (Ident.name id)
            ) id
          in
          ids, mty_actual
        ) bindings
      ...
      let env' =
        List.fold_left (fun env (ids, mty_actual) ->
          match ids with
          | None -> env
          | Some (id, id') ->
            let mty_actual' =
      ...
      let s' =
        List.fold_left (fun s (ids, _mty_actual) ->
          match ids with
          | None -> s
    ...
    else(* Base case: check inclusion of s(mty_actual) in s(mty_decl)
...
let rec type_module ?(alias=false) sttn funct_body anchor env smod =
  ...
  | exn ->
    ...
    {
      mod_desc =
        Tmod_structure { str_items = []; str_type = []; str_final_env = env };
...
and type_module_aux ~alias sttn funct_body anchor env smod =
  ...
  | Pmod_constraint (sarg, smty) ->
    ...
    (try
     ...
     | exn ->
       ...
       if sarg.pmod_desc = Pmod_hole then
         (Msupport.raise_error exn;
          {
            mod_desc = Tmod_hole;
  ...
  | Pmod_unpack sexp ->
    ...
    (if !Clflags.principal then
       (Ctype.end_def (); Ctype.generalize_structure exp.exp_type));
    let mty =
...
and type_structure
    ?(toplevel=false) ?(keep_warnings=false) funct_body anchor env sstr
=
  let names = Signature_names.create () in
  let type_str_item env { pstr_loc = loc; pstr_desc = desc } =
    ...
    | Pstr_type (rec_flag, sdecls) ->
      ...
      map_rec_type_with_row_types ~rec_flag (fun rs info ->
...
*),
    ...
    | Pstr_extension (ext, _attrs) ->
      raise (Error_forward (Builtin_attributes.error_of_extension ext))
    | Pstr_attribute x ->
      Builtin_attributes.warning_attribute x; Tstr_attribute x, [], env
  in
  let rec type_struct env sstr =
    ...
    | pstr :: srem ->
      ...
      begin match type_str_item env pstr with
      | desc, sg, new_env ->
      ...
      | exception exn -> Msupport.raise_error exn; type_struct env srem
      end
  in
  Msupport.with_saved_types
    ?warning_attribute:(if toplevel || keep_warnings then None else Some [])
    ~save_part:(fun (str, _, _, _) ->
...
let type_package env m p nl =
  ...
  let mty =
    if nl = [] then
      ...
      modtype_of_package env modl.mod_loc p nl' tl'
  in
  List.iter2 (fun n ty ->
    try Ctype.unify env ty (Ctype.newvar ())
    with
    | Ctype.Unify _ -> raise (Error (modl.mod_loc, env, Scoping_pack (n, ty)))
  ) nl' tl';
  let modl = wrap_constraint env true modl mty Tmodtype_implicit in
  (* Dropped exports should have produced an error above *)
  assert (List.length nl = List.length tl'); modl, tl'
    ...
    if !Clflags.print_types then
      (Typecore.force_delayed_checks ();
       Printtyp.wrap_printing_env ~error:false initial_env (fun () ->
         fprintf std_formatter "%a@." (Printtyp.printed_signature sourcefile)
           simple_sg
       );
...
let package_signatures units =
  ...
  in
  let subst =
    List.fold_left (fun acc (oldid, newid, _) ->
      Subst.add_module oldid (Pident newid) acc
    ) Subst.identity units_with_ids
  in
  List.map (fun (_, newid, sg) ->(* This signature won't be used for anything, it'll just be saved in a cmi
                                    and cmt. *)
...
let package_units initial_env objfiles cmifile modulename =
  ...
  if Sys.file_exists mlifile then
    ...
    ((if not !Clflags.dont_write_files then
        let cmi =
          Env.save_signature_with_imports ~alerts:Misc.String.Map.empty sg
...
let report_error ppf =
  ...
  | Implementation_is_required intf_name ->
    fprintf ppf
      "@[The interface %a@ declares values, not just types.@ \
         An implementation must be provided.@]" Location.print_filename
      intf_name
  ...
  | Cannot_scrape_package_type p ->
    fprintf ppf "The type of this packed module refers to %a, which is missing"
      path p
  ...
  | Cannot_hide_id
    ...
    fprintf ppf
      ...
      (String.capitalize_ascii shadowed_item_kind) Ident.print shadowed_item_id
      Location.print_loc user_loc (Sig_component_kind.to_string user_kind)
  ...
  | Cannot_hide_id
    ...
    fprintf ppf
      "@[<v>The %s %a introduced by this open appears in the signature@ \
       %a:@;<1 2>The %s %s has no valid type if %a is hidden@]" opened_item_kind
