...
let rec nondep_mty_with_presence env va ids pres mty =
  ...
  | Mty_functor (Named (param, arg), res) ->
    let var_inv = match va with Co -> Contra | Contra -> Co | Strict -> Strict
    in
    let res_env =
      match param with
      | None -> env
  ...
  let res =
    ...
    | Mty_alias _ ->
      ...
      if mty' = mty then
        ...
        (args'.modified <- true; remove_aliases_mty env args' Mp_present mty')
    | mty -> Mp_present, mty
  in
  if args'.modified then (args.modified <- true; res) else pres, mty
...
let lower_nongen nglev mty =
  ...
  it.it_module_type it mty; it.it_module_type unmark_iterators mty
