...
let close_variant env row =
  ...
  let nm =
    List.fold_left (fun nm (_tag, f) ->
      match Btype.row_field_repr f with
      | Reither (_, _, false, e) ->
        (* m=false means that this tag is not explicitly matched *)
        Btype.set_row_field e Rabsent; None
...
let rec orify_many =
  function [] -> assert false | [ x ] -> x | x :: xs -> orify x (orify_many xs)
...
let complete_constrs constr all_tags =
  ...
  let others =
    List.filter (fun cnstr -> ConstructorTagHashtbl.mem not_tags cnstr.cstr_tag)
      constrs
  in
  let (const, nonconst) =
...
let build_other ext env =
  ...
  | (d, _) :: _ ->
    ...
    | Variant { cstr_row; type_row } ->
      ...
      let row = type_row () in
      begin match
        List.fold_left (fun others (tag, f) ->
    ...
    | Constant (Const_char _) ->
      let all_chars =
        List.map (fun (p, _) ->
          match p.pat_desc with Constant (Const_char c) -> c | _ -> assert false
        ) env
      in
...
and has_instances =
  function [] -> true | q :: rem -> has_instance q && has_instances rem
  ...
  | q :: qs ->
    ...
    | `Any ->
      let rec remove_first_column =
        function (_ :: ps) :: rem -> ps :: remove_first_column rem | _ -> []
      in
...
and specialize_and_exhaust ext pss n =
  ...
  if not (all_coherent (first_column pss)) then
    ...
    Seq.empty
  else(* Assuming the first column is ill-typed but considered coherent, we
...
let is_var_column rs =
  List.for_all (fun r ->
    match r.active with p :: _ -> is_var p | [] -> assert false
  ) rs
...
let rec every_satisfiables pss qs =
  ...
  | q :: rem ->
    ...
    | `Variant (l, _, r) when is_absent l r -> (* Ah Jacques... *) Unused
    | #Patterns.Simple.view as view ->
      let q = { q with  pat_desc = view } in
...
let rec lub p q =
  ...
  | _, Tpat_or (q1, q2, _) -> orlub q1 q2 p
...
let pressure_variants_in_computation_pattern tdefs patl =
  ...
  in
  pressure_variants tdefs val_pss; pressure_variants tdefs exn_pss
(*****************************)
...
module Conv = struct
  ...
  let fresh name =
    ...
    name_counter := !name_counter + 1; "#$" ^ name ^ Int.to_string current
  ...
  let conv typed =
    ...
    let rec loop pat =
      ...
      | Tpat_record (subpatterns, _closed_flag) ->
        let fields =
          List.map (fun (_, lbl, p) ->
            let id = fresh lbl.lbl_name in
            Hashtbl.add labels id lbl; mknoloc (Longident.Lident id), loop p
          ) subpatterns
        in
...
let reduce f =
  function [] -> invalid_arg "reduce" | x :: xs -> List.fold_left f x xs
  ...
  | (Positive { row = []; _ } | Negative []) :: _ as empty_rows ->
    ...
    let get_varsets =
      ...
      | Negative n ->
        ...
        assert (n = []); raise Negative_empty_row
      | Positive p -> assert (p.row = []); p.varsets
    in
  ...
  | m ->
    ...
    if List.for_all is_negative m then
      ...
      if not (all_coherent (first_column m)) then
        All
      else(* If the column is ill-typed but deemed coherent, we might
...
let all_rhs_idents exp =
  ...
  iterator.expr iterator exp; !ids
...
let do_complete_partial ?pred pss =
  ...
  | ps :: _ ->
    let typecheck p =
      ...
      | None -> Some (p, None)
    in
    exhaust None pss (List.length ps) |> Seq.filter_map typecheck |> List.of_seq
