...
let global = function Local _ | Scoped _ -> false | Global _ | Predef _ -> true
...
let print ~with_scope ppf =
  ...
  | Predef { name; stamp = n } -> fprintf ppf "%s/%i!" name n
  | Local { name; stamp = n } -> fprintf ppf "%s/%i" name n
...
let rec get_all =
  function None -> [] | Some k -> (k.ident, k.data) :: get_all k.previous
...
let make_key_generator () =
  ...
  | Local _ | Scoped _ ->
    let stamp = !c in
    decr c; Local { name = key_name; stamp = stamp }
  | global_id ->
