...
let print_lident ppf =
  function "::" -> pp_print_string ppf "(::)" | s -> pp_print_string ppf s
...
let valid_float_lexeme s =
  ...
  let rec loop i =
    if i >= l then
      ...
      match s.[i] with '0' .. '9' | '-' -> loop (i + 1) | _ -> s
  in
  loop 0
...
let parenthesize_if_neg ppf fmt v isneg =
  ...
  fprintf ppf fmt v; if isneg then pp_print_char ppf ')'
  ...
  if !n = String.length s then
    ...
    (n := 0;
     for i = 0 to String.length s - 1 do
       ...
       | '\r' -> Bytes.unsafe_set s' !n '\\'; incr n; Bytes.unsafe_set s' !n 'r'
       | '\b' -> Bytes.unsafe_set s' !n '\\'; incr n; Bytes.unsafe_set s' !n 'b'
       | ('\000' .. '\031' | '\127') as c ->
         ...
         incr n; Bytes.unsafe_set s' !n (Char.chr (48 + a mod 10))
...
let print_out_value ppf tree =
  ...
  and print_constr_param ppf =
    ...
    | Oval_string (_, _, Ostr_bytes) as tree ->
      pp_print_char ppf '('; print_simple_tree ppf tree; pp_print_char ppf ')'
    | tree -> print_simple_tree ppf tree
  ...
  and print_tree_list print_item sep ppf tree_list =
    let rec print_list first ppf =
      ...
      | tree :: tree_list ->
        ...
        print_item ppf tree; print_list false ppf tree_list
    in
...
and print_out_type_1 ppf =
  ...
  | Otyp_arrow (lab, ty1, ty2) ->
    ...
    pp_print_string ppf " ->";
    pp_print_space ppf (); print_out_type_1 ppf ty2; pp_close_box ppf ()
...
and print_simple_out_type ppf =
  ...
  | Otyp_constr (id, tyl) ->
    pp_open_box ppf 0;
    print_typargs ppf tyl; print_ident ppf id; pp_close_box ppf ()
  ...
  | (Otyp_alias _ | Otyp_poly _ | Otyp_arrow _ | Otyp_tuple _) as ty ->
    pp_open_box ppf 1;
    pp_print_char ppf '(';
    print_out_type ppf ty; pp_print_char ppf ')'; pp_close_box ppf ()
...
and print_typlist print_elem sep ppf =
  ...
  | ty :: tyl ->
    ...
    pp_print_space ppf (); print_typlist print_elem sep ppf tyl
...
and print_typargs ppf =
  ...
  | tyl ->
    ...
    pp_print_char ppf ')'; pp_close_box ppf (); pp_print_space ppf ()
...
let rec print_out_class_type ppf =
  ...
  | Octy_signature (self_ty, csil) ->
    let pr_param ppf =
      function Some ty -> fprintf ppf "@ @[(%a)@]" !out_type ty | None -> ()
    in
...
let collect_functor_arguments mty =
  ...
  let rec uncollect_anonymous_suffix acc rest =
    ...
    | Some (None, mty_arg) :: acc ->
      uncollect_anonymous_suffix acc (Omty_functor (Some (None, mty_arg), rest))
    | _ :: _ | [] -> acc, rest
  in
...
and print_out_functor ppf =
  function
  | Omty_functor _ as t ->
    let rec print_functor ppf =
      function
      ...
      | Omty_functor _ as non_anonymous_functor ->
        ...
        fprintf ppf "@[<2>functor@ %a@]@ ->@ %a"
          (pp_print_list ~pp_sep:pp_print_space print_arg) args print_functor
          rest
...
and print_out_sig_item ppf =
  ...
  | Osig_value vd ->
    ...
    fprintf ppf "@[<2>%s %a :@ %a%a%a@]" kwd value_ident vd.oval_name !out_type
      vd.oval_type pr_prims vd.oval_prims (fun ppf ->
...
and print_out_type_decl kwd ppf td =
  ...
  let print_private ppf =
    function Asttypes.Private -> fprintf ppf " private" | Asttypes.Public -> ()
  in
