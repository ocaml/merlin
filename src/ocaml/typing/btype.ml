...
let rec repr_link compress t d =
  ...
  | t' ->
    (if compress then (log_change (Ccompress (t, t.desc, d)); t.desc <- d)); t'
...
let rec commu_repr =
  function Clink r when !r <> Cunknown -> commu_repr !r | c -> c
...
let rec row_field tag row =
  let rec find =
    ...
    | [] ->
      begin match repr row.row_more with
...
let iter_type_expr_kind f =
  ...
  | Type_variant cstrs ->
    List.iter (fun cd ->
      iter_type_expr_cstr_args f cd.cd_args; Option.iter f cd.cd_res
    ) cstrs
...
let type_iterators =
  ...
  and it_class_declaration it cd =
    ...
    Option.iter (it.it_type_expr it) cd.cty_new; it.it_path cd.cty_path
  and it_class_type_declaration it ctd =
    List.iter (it.it_type_expr it) ctd.clty_params;
    it.it_class_type it ctd.clty_type; it.it_path ctd.clty_path
  and it_functor_param it =
    function Unit -> () | Named (_, mt) -> it.it_module_type it mt
  and it_module_type it =
    function
    | Mty_ident p | Mty_alias p -> it.it_path p
    | Mty_for_hole -> ()
    | Mty_signature sg -> it.it_signature it sg
    | Mty_functor (p, mt) -> it.it_functor_param it p; it.it_module_type it mt
  and it_class_type it =
    function
    | Cty_constr (p, tyl, cty) ->
      it.it_path p; List.iter (it.it_type_expr it) tyl; it.it_class_type it cty
    | Cty_signature cs ->
      it.it_type_expr it cs.csig_self;
      Vars.iter (fun _ (_, _, ty) -> it.it_type_expr it ty) cs.csig_vars;
      List.iter (fun (p, tl) -> it.it_path p; List.iter (it.it_type_expr it) tl)
        cs.csig_inher
    | Cty_arrow (_, ty, cty) -> it.it_type_expr it ty; it.it_class_type it cty
...
let rec mark_type ty =
  ...
  if ty.level >= lowest_level then
    (ty.level <- pivot_level - ty.level; iter_type_expr mark_type ty)
...
let rec unmark_type ty =
  let ty = repr ty in
  if ty.level < lowest_level then
    (ty.level <- pivot_level - ty.level; iter_type_expr unmark_type ty)
...
let rec forget_abbrev_rec mem path =
  ...
  | Mlink mem' -> mem' := forget_abbrev_rec !mem' path; raise Exit
...
let set_typeset rs s = log_change (Ctypeset (rs, !rs)); rs := s
...
let rec rev_log accu =
  ...
  | Change (ch, next) ->
    let d = !next in
    next := Invalid; rev_log (ch :: accu) d
...
let is_valid (changes, _old) = match !changes with Invalid -> false | _ -> true
let on_backtrack f = log_change (Cfun f)
