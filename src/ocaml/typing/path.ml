...
let exists_free ids p = match find_free_opt ids p with None -> false | _ -> true
...
let rec head =
  function Pident id -> id | Pdot (p, _s) -> head p | Papply _ -> assert false
...
let is_uident s =
  assert (s <> ""); match s.[0] with 'A' .. 'Z' -> true | _ -> false
