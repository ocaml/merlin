...
module IntOps = struct
  ...
  let less_than_or_equal (x : t) (y : t) = x <= y
...
module Array_zero_indexed = struct
  ...
  let extend t idx init =
    ...
    if old_len = 0 then
      ...
      (Array.blit t 0 extended 0 old_len;
       for i = old_len to idx do Array.unsafe_set extended i (init i) done;
       extended)
  ...
  let last t =
    let len = Array.length t in
    if len = 0 then None else Some (len - 1)
  
  let set t idx data = t.(idx) <- data
  let get t idx = t.(idx)
end
...
module Array_one_indexed = struct
  ...
  let extend t idx init =
    ...
    if old_len = 0 then
      ...
      (for i = 1 to idx - 1 do Array.unsafe_set res i (init (i + 1)) done; res)
    else
  ...
  let set t idx data = t.(idx - 1) <- data
  let get t idx = t.(idx - 1)
end
...
module Nat = struct
  ...
  let one = 1
  let maximum = max_int
  let succ t = if t = maximum then t else t + 1
  let pred t = if t = 0 then None else Some (t - 1)
  
  let plus t1 t2 =
    let res = t1 + t2 in
    if res < 0 then maximum else res
...
module Nat_no_zero = struct
  ...
  let maximum = max_int
  let succ t = if t = maximum then t else t + 1
  let pred t = if t = 1 then None else Some (t - 1)
