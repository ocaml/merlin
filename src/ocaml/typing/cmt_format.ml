...
let need_to_clear_env =
  try ignore (Sys.getenv "OCAML_BINANNOT_WITHENV"); false with Not_found -> true
...
let output_cmt oc cmt =
  output_string oc Config.cmt_magic_number; output_value oc (cmt : cmt_infos)
...
let read_cmi filename =
  match read filename with
  | None, _ -> raise Magic_numbers.Cmi.(Error (Not_an_interface filename))
  | Some cmi, _ -> cmi
