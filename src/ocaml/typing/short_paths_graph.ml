...
module Ident = struct
  ...
  let name = Ident.name
  let global name = Ident.create_persistent name
end
...
module Path = struct
  ...
  let rec equal t1 t2 =
    ...
    | Pdot (parent1, name1), Pdot (parent2, name2) ->
      equal parent1 parent2 && String.equal name1 name2
...
module Desc = struct
  ...
  module Type = struct
    type t = Fresh | Nth of int | Subst of Path.t * int list | Alias of Path.t
  end
    
  
  module Module_type = struct
    type t = Fresh | Alias of Path.t
  end
    
  
  module Class_type = struct
    type t = Fresh | Subst of Path.t * int list | Alias of Path.t
  end
  ...
  module Module = struct
    ...
    and kind = Signature of components Lazy.t | Functor of (Path.t -> t)
    
    and t = Fresh of kind | Alias of Path.t
  end
    
  
  type source = Local | Open
...
module Sort = struct
  type t = Defined | Declared of Ident_set.t
...
module Age = Natural.Make(struct end) 
module Dependency = Natural.Make(struct end) 
...
module Origin = struct
  ...
  let rec deps_equal deps1 deps2 =
    ...
    | _ :: _, [] -> false
    | dep1 :: rest1, dep2 :: rest2 ->
      Dependency.equal dep1 dep2 && deps_equal rest1 rest2
  ...
  let application t1 t2 =
    ...
    | Dependency dep1, Dependencies deps2 -> Dependencies (deps_add dep1 deps2)
    | Dependency _, Environment _ -> t2
    | Dependencies deps1, Dependency dep2 -> Dependencies (deps_add dep2 deps1)
    ...
    | Environment age1, Environment age2 -> Environment (Age.max age1 age2)
  ...
  if name <> "" && name.[0] = '_' then
    ...
    try
      for i = 1 to String.length name - 2 do
        if name.[i] = '_' && name.[i + 1] = '_' then raise Exit
      done;
      false
    with
    | Exit -> true
...
let hidden_ident id =
  if !Clflags.unsafe_string && Ident.equal id Predef.ident_bytes then
    true
  else
...
module rec Type : sig
  ...
  type resolved = Nth of int | Path of int list option * t
  ...
  type t =
    | Declaration of { origin : Origin.t; id : Ident.t; hidden : bool }
    | Definition of
        {
          origin : Origin.t;
          path : Path.t;
  ...
  let declaration t =
    match t with Definition _ -> None | Declaration { origin; _ } -> Some origin
  ...
  let raw_origin t =
    match t with Declaration { origin; _ } | Definition { origin; _ } -> origin
  ...
  let rec normalize_loop root t =
    ...
    | Definition ({ definition = Alias alias } as r) ->
      begin match Graph.find_type root alias with
  ...
  let normalize root t =
    ...
    | Definition { sort = Sort.Declared _ } | Declaration _ ->
      begin match Graph.find_type root (raw_path t) with
      | exception Not_found -> normalize_loop root t
      | t -> normalize_loop root t
      end
  
  let origin root t = raw_origin (normalize root t)
  let path root t = raw_path (normalize root t)
  let sort root t = raw_sort (normalize root t)
  ...
  let rec resolve root t =
    ...
    | Definition { definition = Subst (p, ns) } ->
      begin match Graph.find_type root p with
...
and Class_type : sig
  ...
  let rec normalize_loop root t =
    ...
    | Definition ({ definition = Alias alias } as r) ->
      begin match Graph.find_class_type root alias with
      | exception Not_found -> Definition { r with  definition = Unknown }
      | aliased -> normalize_loop root aliased
      end
  ...
  let normalize root t =
    ...
    | Definition { sort = Sort.Defined } -> normalize_loop root t
    | Definition { sort = Sort.Declared _ } | Declaration _ ->
      begin match Graph.find_class_type root (raw_path t) with
      | exception Not_found -> normalize_loop root t
      | t -> normalize_loop root t
      end
  ...
  let rec resolve root t =
    ...
    | Definition { definition = Subst (p, ns) } ->
      begin match Graph.find_class_type root p with
...
and Module_type : sig
  ...
  type definition = Alias of Path.t | Defined | Unknown
  ...
  let rec normalize_loop root t =
    ...
    | Definition ({ definition = Alias alias } as r) ->
      begin match Graph.find_module_type root alias with
      | exception Not_found -> Definition { r with  definition = Unknown }
      | aliased -> normalize_loop root aliased
      end
  ...
  let normalize root t =
    ...
    | Definition { sort = Sort.Defined } -> normalize_loop root t
    | Definition { sort = Sort.Declared _ } | Declaration _ ->
      begin match Graph.find_module_type root (raw_path t) with
      | exception Not_found -> normalize_loop root t
      | t -> normalize_loop root t
      end
...
and Module : sig
  ...
  and definition =
    | Alias of Path.t
    | Signature of { mutable components : components }
    | Functor of
        {
  ...
  let rec normalize_loop root t =
    ...
    | Definition ({ definition = Alias alias } as r) ->
      begin match Graph.find_module root alias with
      | exception Not_found -> Definition { r with  definition = Unknown }
      | aliased -> normalize_loop root aliased
      end
  ...
  let normalize root t =
    ...
    | Definition { sort = Sort.Defined } -> normalize_loop root t
    | Definition { sort = Sort.Declared _ } | Declaration _ ->
      begin match Graph.find_module root (raw_path t) with
      | exception Not_found -> normalize_loop root t
      | t -> normalize_loop root t
      end
  ...
  let force root t =
    ...
    | Unknown | Functor _ | Signature { components = Forced _ } -> t
    | Signature ({ components = Unforced components; _ } as r) ->
      let rec loop types class_types module_types modules =
        function
      ...
      r.components <- components; t
  
  let types root t =
    let t = force root t in
    match definition t with
    | Alias _ | Signature { components = Unforced _ } -> assert false
    | Unknown | Functor _ -> None
    | Signature { components = Forced { types; _ }; _ } -> Some types
  ...
  let class_types root t =
    ...
    | Alias _ | Signature { components = Unforced _ } -> assert false
    | Unknown | Functor _ -> None
    | Signature { components = Forced { class_types; _ } } -> Some class_types
  ...
  let module_types root t =
    ...
    | Alias _ | Signature { components = Unforced _ } -> assert false
    | Unknown | Functor _ -> None
    | Signature { components = Forced { module_types; _ } } -> Some module_types
  ...
  let modules root t =
    ...
    | Alias _ | Signature { components = Unforced _ } -> assert false
    | Unknown | Functor _ -> None
    | Signature { components = Forced { modules; _ } } -> Some modules
  
  let find_type root t name =
    let t = force root t in
    match definition t with
    | Alias _ | Signature { components = Unforced _ } -> assert false
    | Unknown -> Type.child t name None Not_deprecated
    | Functor _ -> raise Not_found
  ...
  let find_class_type root t name =
    ...
    match definition t with
    | Alias _ | Signature { components = Unforced _ } -> assert false
    | Unknown -> Class_type.child t name None Not_deprecated
    | Functor _ -> raise Not_found
  ...
  let find_module_type root t name =
    ...
    match definition t with
    | Alias _ | Signature { components = Unforced _ } -> assert false
    | Unknown -> Module_type.child t name None Not_deprecated
    | Functor _ -> raise Not_found
  ...
  let find_module root t name =
    ...
    match definition t with
    | Alias _ | Signature { components = Unforced _ } -> assert false
    | Unknown -> Module.child t name None Not_deprecated
    | Functor _ -> raise Not_found
  ...
  let find_application root t path =
    ...
    | Functor ({ apply; applications } as r) ->
      ...
      begin match Path_map.find arg_path applications with
      | md -> md
      | exception Not_found ->
        let md = Module.application t arg (Some (apply arg_path)) in
        r.applications <- Path_map.add arg_path md applications; md
      end
end
...
and Component : sig
  type source = Global | Local | Open
  
  type t =
    | Type of Origin.t * Ident.t * Desc.Type.t * source * Desc.deprecated
    | Class_type of
        Origin.t
        * Ident.t
        * Desc.Class_type.t
    ...
    | Module_type of
        ...
        * source
        * Desc.deprecated
    | Module of Origin.t * Ident.t * Desc.Module.t * source * Desc.deprecated
...
and Graph : sig
  ...
  let previous_type t id =
    ...
    | prev ->
      begin match Type.declaration prev with
  ...
  let previous_class_type t id =
    ...
    | exception Not_found -> None
    | prev ->
      begin match Class_type.declaration prev with
  ...
  let previous_module_type t id =
    ...
    | exception Not_found -> None
    | prev ->
      begin match Module_type.declaration prev with
  ...
  let previous_module t id =
    ...
    | exception Not_found -> None
    | prev ->
      begin match Module.declaration prev with
  ...
  let add_name source id names =
    ...
    let defs =
      ...
      | Component.Open ->
        begin match String_map.find name names with
  ...
  let merge_name id names =
    ...
    | exception Not_found -> String_map.add name (Global id) names
    | _ -> names
  
  let add t descs =
    let rec loop acc diff declarations =
      function
      ...
      | (Component.Declare_module (_, id) as decl) :: rest ->
        ...
        loop acc diff declarations rest
    and loop_declarations acc diff =
      function
      | [] -> acc, diff
      | Component.Declare_type (origin, id) :: rest ->
        if Ident_map.mem id acc.types then
      ...
      | Component.Declare_class_type (origin, id) :: rest ->
        if Ident_map.mem id acc.class_types then
          loop_declarations acc diff rest
        else
      ...
      | Component.Declare_module_type (origin, id) :: rest ->
        if Ident_map.mem id acc.module_types then
          loop_declarations acc diff rest
        else
      ...
      | Component.Declare_module (origin, id) :: rest ->
        if Ident_map.mem id acc.modules then
          loop_declarations acc diff rest
        else
  ...
  let rec find_module t path =
    match path with
    | Path.Pident id -> Ident_map.find id t.modules
    | Path.Pdot (p, name) ->
  ...
  let find_type t path =
    match path with
    | Path.Pident id -> Ident_map.find id t.types
    | Path.Pdot (p, name) ->
      let md = find_module t p in
  ...
  let find_class_type t path =
    match path with
    | Path.Pident id -> Ident_map.find id t.class_types
    | Path.Pdot (p, name) ->
      let md = find_module t p in
  ...
  let find_module_type t path =
    match path with
    | Path.Pident id -> Ident_map.find id t.module_types
    | Path.Pdot (p, name) ->
      let md = find_module t p in
  ...
  let is_class_type_path_visible t =
    function
  ...
  let is_module_type_path_visible t =
    function
