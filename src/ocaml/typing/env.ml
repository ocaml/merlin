...
and module_components =
  {
    ...
    uid : Uid.t;
    comps :
      (components_maker,(module_components_repr,module_components_failure)
      result)
      Lazy_backtrack.t
  }
...
let sign_of_cmi ~freshen { Persistent_env.Persistent_signature.cmi; _ } =
  ...
  let mda_declaration = Lazy_backtrack.create (id_subst, Subst.Make_local, md)
  in
  let mda_components =
    let freshening_subst = if freshen then Some id_subst else None in
    components_of_module ~alerts ~uid:md.md_uid empty freshening_subst id_subst
      path mda_address (Mty_signature sign)
  in
  let result = { mda_declaration; mda_components; mda_address } in
  result
...
let find_pers_mod name =
  Persistent_env.find !persistent_env read_sign_of_cmi short_paths_components
    name

let check_pers_mod ~loc name =
  Persistent_env.check !persistent_env read_sign_of_cmi short_paths_components
    ~loc name

let crc_of_unit name =
  Persistent_env.crc_of_unit !persistent_env read_sign_of_cmi
    short_paths_components name
...
let reset_cache () =
  ...
  Persistent_env.clear !persistent_env; reset_declaration_caches (); ()

let reset_cache_toplevel () =
  Persistent_env.clear_missing !persistent_env; reset_declaration_caches (); ()
(* get_components *)
  ...
  | mty ->
    (try Hashtbl.find fcomp.fcomp_subst_cache p2
     ...
     | Not_found ->
       ...
       in
       Hashtbl.add fcomp.fcomp_subst_cache p2 mty; mty)
...
let find_type_full path env =
  ...
  | Cstr (ty_path, s) ->
    let tda = try find_type_full ty_path env with Not_found -> assert false in
  ...
  | Ext (mod_path, s) ->
    let comps =
      try find_structure_components mod_path env with Not_found -> assert false
    in
    let cstrs =
...
let rec is_functor_arg path env =
  match path with
  | Pident id ->
    (try Ident.find_same id env.functor_args; true with Not_found -> false)
...
let iter_env wrap proj1 proj2 f env () =
  ...
  in
  IdTbl.iter wrap_module (fun id (path, entry) ->
    match entry with
    | Mod_unbound _ -> ()
...
let find_shadowed_types path env =
  List.map fst
    (find_shadowed wrap_identity (fun env -> env.types) (fun comps ->
       comps.comp_types
     ) path env)
...
let prefix_idents root freshening_sub prefixing_sub sg =
  ...
  let rec prefix_idents root items_and_paths freshening_sub prefixing_sub =
    ...
    | Sig_type (id, td, rs, vis) :: rem ->
      ...
      prefix_idents root ((Sig_type (id', td, rs, vis), p) :: items_and_paths)
    ...
    | Sig_typext (id, ec, es, vis) :: rem ->
      ...
      prefix_idents root ((Sig_typext (id', ec, es, vis), p) :: items_and_paths)
        freshening_sub (Subst.add_type id' p prefixing_sub) rem
    ...
    | Sig_modtype (id, mtd, vis) :: rem ->
      ...
      let (id', freshening_sub) =
        refresh id (fun i p s -> Subst.add_modtype i (Mty_ident p) s)
          freshening_sub
      in
      prefix_idents root ((Sig_modtype (id', mtd, vis), p) :: items_and_paths)
    ...
    | Sig_class (id, cd, rs, vis) :: rem ->
      ...
      let p = Pdot (root, Ident.name id) in
      let (id', freshening_sub) = refresh id Subst.add_type freshening_sub in
      prefix_idents root ((Sig_class (id', cd, rs, vis), p) :: items_and_paths)
        freshening_sub (Subst.add_type id' p prefixing_sub) rem
...
let short_paths_type predef id decl old =
  if not predef && !Clflags.real_paths then old else Type (id, decl) :: old

let short_paths_type_open path decls old =
  if !Clflags.real_paths then old else Type_open (path, decls) :: old

let unbound_class = Path.Pident (Ident.create_local "*undef*")
let is_dummy_class decl = Path.same decl.clty_path unbound_class

let short_paths_class_type id decl old =
  if !Clflags.real_paths || is_dummy_class decl then
    old
  else
    Class_type (id, decl) :: old

let short_paths_class_type_open path decls old =
  if !Clflags.real_paths then old else Class_type_open (path, decls) :: old

let short_paths_module_type id decl old =
  if !Clflags.real_paths then old else Module_type (id, decl) :: old

let short_paths_module_type_open path decls old =
  if !Clflags.real_paths then old else Module_type_open (path, decls) :: old

let short_paths_module id decl comps old =
  if !Clflags.real_paths then old else Module (id, decl, comps) :: old

let short_paths_module_open path comps old =
  if !Clflags.real_paths then old else Module_open (path, comps) :: old
...
=
  ...
  | Mty_signature sg ->
    ...
    let next_address () =
      ...
      in
      incr pos; Lazy_backtrack.create addr
    in
    ...
    List.iter (fun (item, path) ->
      ...
      | Sig_type (id, decl, _, _) ->
        let fresh_decl = may_subst Subst.type_declaration freshening_sub decl in
        ...
        let constructors =
          List.map snd
            (Datarepr.constructors_of_type ~current_unit:(get_unit_name ()) path
               final_decl)
        in
        let labels = List.map snd (Datarepr.labels_of_type path final_decl) in
        let tda =
          {
        ...
        List.iter (fun descr ->
          ...
          c.comp_constrs <- add_to_tbl descr.cstr_name cda c.comp_constrs
        ) constructors;
        List.iter (fun descr ->
      ...
      | Sig_module (id, pres, md, _, _) ->
        let md' =
          (* The prefixed items get the same scope as [cm_path], which is
             the prefix. *)
          Lazy_backtrack.create (sub, Subst.Rescope (Path.scope cm_path), md)
        in
        let addr =
          match pres with
          | Mp_absent ->
        ...
        in
        let mda =
          { mda_declaration = md'; mda_components = comps; mda_address = addr }
        in
      ...
      | Sig_modtype (id, decl, _) ->
        let fresh_decl =
          ...
          may_subst (Subst.modtype_declaration Make_local) freshening_sub decl
        in
        let final_decl =
          (* The prefixed items get the same scope as [cm_path], which is
             the prefix. *)
          Subst.modtype_declaration (Rescope (Path.scope cm_path)) prefixing_sub
            fresh_decl
        in
        c.comp_modtypes <-
...
and store_type ~check ~predef id info env =
  ...
  (if check then
     check_usage loc id info.type_uid (fun s ->
  ...
  { env with
    ...
    constrs =
      List.fold_right (fun (id, descr) constrs ->
        let cda = { cda_description = descr; cda_address = None } in
        TycompTbl.add id cda constrs
      ) constructors env.constrs;
    labels =
      List.fold_right (fun (id, descr) labels -> TycompTbl.add id descr labels)
        labels env.labels;
...
and store_module ~check ~freshening_sub id addr presence md env =
  ...
  Option.iter (fun f -> check_usage loc id md.md_uid f !module_declarations)
    check;
  let alerts = Builtin_attributes.alerts_of_attrs md.md_attributes in
...
let components_of_functor_appl ~loc f env p1 p2 =
  ...
  | Not_found ->
    ...
    in
    Hashtbl.add f.fcomp_cache p2 comps; comps
...
let rec add_signature sg env =
  match sg with [] -> env | comp :: rem -> add_signature rem (add_item comp env)
...
let add_components slot root env0 comps =
  ...
  let (types, additions) =
    add_types (fun x -> `Type x) comps.comp_types env0.types
      env0.short_paths_additions
  in
  let (modtypes, additions) =
    add_modtypes (fun x -> `Module_type x) comps.comp_modtypes env0.modtypes
      additions
  in
  let classes = add (fun x -> `Class x) comps.comp_classes env0.classes in
  let (cltypes, additions) =
    add_cltypes (fun x -> `Class_type x) comps.comp_cltypes env0.cltypes
      additions
  in
  let (modules, additions) =
    add_modules (fun x -> `Module x) comps.comp_modules env0.modules additions
  in
  { env0 with
...
=
  ...
  in
  let warn_unused = Warnings.is_active (unused root)
  and warn_shadow_id =
...
let unit_name_of_filename fn =
  ...
  | ".cmi" ->
    ...
    if Std.String.for_all is_identchar_latin1 unit then Some unit else None
  | _ -> None
...
let (initial_safe_string, initial_unsafe_string) =
  Predef.build_initial_env (add_type ~check:false ~predef:true)
    (add_extension ~check:false ~rebind:false) empty

let add_type ~check id info env = add_type ~check ~predef:false id info env
(* Tracking usage *)
...
let use_module ~use ~loc path mda =
  if use then
    ...
    (mark_module_used comps.uid;
     Misc.String.Map.iter (fun kind message ->
...
let lookup_ident_module (type a) (load : a load) ~errors ~use ~loc s env =
  ...
  | Mod_persistent ->
    begin match load with
    | Don't_load -> check_pers_mod ~loc s; path, (() : a)
    | Load ->
      begin match find_pers_mod s with
      | mda -> use_module ~use ~loc path mda; path, (mda : a)
      | exception Not_found ->
        may_lookup_error errors loc env (Unbound_module (Lident s))
      end
    end
...
let lookup_ident_value ~errors ~use ~loc name env =
  ...
  | path, Val_bound vda ->
    use_value ~use ~loc path vda; path, vda.vda_description
...
let lookup_ident_type ~errors ~use ~loc s env =
  ...
  | path, data as res -> use_type ~use ~loc path data; res
  | exception Not_found ->
...
let lookup_ident_modtype ~errors ~use ~loc s env =
  ...
  | path, data as res -> use_modtype ~use ~loc path data; res
  | exception Not_found ->
...
let lookup_ident_class ~errors ~use ~loc s env =
  ...
  | path, clda -> use_class ~use ~loc path clda; path, clda.clda_declaration
  | exception Not_found ->
...
let lookup_ident_cltype ~errors ~use ~loc s env =
  ...
  | path, data as res -> use_cltype ~use ~loc path data; res
  | exception Not_found ->
...
let lookup_all_ident_labels ~errors ~use ~loc s env =
  ...
  | lbls ->
    List.map (fun (lbl, use_fn) ->
      let use_fn () = use_label ~use ~loc env lbl; use_fn () in
      lbl, use_fn
    ) lbls
...
let lookup_all_ident_constructors ~errors ~use ~loc usage s env =
  ...
  | cstrs ->
    List.map (fun (cda, use_fn) ->
      let use_fn () = use_constructor ~use ~loc usage env cda; use_fn () in
...
and lookup_dot_module ~errors ~use ~loc l s env =
  ...
  | mda ->
    ...
    use_module ~use ~loc path mda; path, mda
  | exception Not_found ->
...
let lookup_dot_type ~errors ~use ~loc l s env =
  ...
  | tda ->
    let path = Pdot (p, s) in
    use_type ~use ~loc path tda; path, tda
  | exception Not_found ->
...
let lookup_dot_modtype ~errors ~use ~loc l s env =
  ...
  | desc ->
    let path = Pdot (p, s) in
    use_modtype ~use ~loc path desc; path, desc
  | exception Not_found ->
...
let lookup_dot_class ~errors ~use ~loc l s env =
  ...
  | clda ->
    let path = Pdot (p, s) in
    use_class ~use ~loc path clda; path, clda.clda_declaration
  | exception Not_found ->
...
let lookup_dot_cltype ~errors ~use ~loc l s env =
  ...
  | desc ->
    let path = Pdot (p, s) in
    use_cltype ~use ~loc path desc; path, desc
  | exception Not_found ->
...
let lookup_module_path ~errors ~use ~loc ~load lid env : Path.t =
  ...
  | Lapply (l1, l2) ->
    let (p1, f, arg) = lookup_functor_components ~errors ~use ~loc l1 env in
    let (p2, md2) = lookup_module ~errors ~use ~loc l2 env in
    check_functor_appl ~errors ~loc env p1 f arg p2 md2; Papply (p1, p2)
...
let lookup_instance_variable ?(use=true) ~loc name env =
  ...
  | path, Val_bound vda ->
    ...
    | Val_ivar (mut, cl_num) ->
      use_value ~use ~loc path vda; path, mut, cl_num, desc.val_type
...
let fold_modules f lid env acc =
  match lid with
  | None ->
    IdTbl.fold_name wrap_module (fun name (p, entry) acc ->
      match entry with
      | Mod_unbound _ -> acc
      | Mod_local mda ->
        let md = Lazy_backtrack.force subst_modtype_maker mda.mda_declaration in
      ...
      | Mod_persistent ->
        ...
        | None -> acc
        | Some mda ->
          let md = Lazy_backtrack.force subst_modtype_maker mda.mda_declaration
          in
          f name p md acc
        end
    ) env.modules acc
...
and fold_classes f =
  find_all wrap_identity (fun env -> env.classes) (fun sc -> sc.comp_classes)
    (fun k p clda acc ->
    f k p clda.clda_declaration acc
  )
...
let filter_non_loaded_persistent f env =
  let to_remove =
    IdTbl.fold_name wrap_module (fun name (_, entry) acc ->
      match entry with
      | Mod_local _ -> acc
      | Mod_unbound _ -> acc
      | Mod_persistent ->
  ...
  let remove_ids tbl ids =
    String.Set.fold (fun name tbl ->
      IdTbl.remove (Ident.create_persistent name) tbl
    ) ids tbl
  in
...
let keep_only_summary env =
  if !last_env == env then
    ...
    in
    (last_env := env; last_reduced_env := new_env; new_env)
...
let extract_instance_variables env =
  fold_values (fun name _ descr acc ->
...
let with_cmis f = Persistent_env.with_cmis !persistent_env f ()
...
let short_paths_type_desc decl =
  ...
  | Some ty ->
    ...
    if ty.level <> generic_level then
      ...
      begin match decl.type_private, decl.type_kind with
      ...
      | _, _ ->
        ...
        | { desc = Tconstr (path, args, _) } ->
          ...
          then
            Alias path
          else if List.length params <= List.length args || not (uniq args) then
            Fresh
          else
            begin match List.map (index params) args with
            | exception Not_found -> Fresh
            | ns -> Subst (path, ns)
            end
        | ty ->
          begin match index params ty with
          | exception Not_found -> Fresh
          | n -> Nth n
          end
        end
...
let short_paths_class_type_desc clty =
  ...
  | Cty_constr (path, args, _) ->
    ...
    let args = List.map repr args in
    if
      List.length params = List.length args && List.for_all2 (==) params args
    then
      Alias path
    else if List.length params <= List.length args || not (uniq args) then
      Fresh
...
let deprecated_of_alerts alerts =
  if
    String.Map.exists (fun key _ ->
      match key with "deprecated" | "ocaml.deprecated" -> true | _ -> false
    ) alerts
  then
...
let rec short_paths_module_desc env mpath mty comp =
  ...
  | Mty_ident path ->
    begin match find_modtype_expansion path env with
    | exception Not_found -> Fresh (Signature (lazy []))
    | mty -> short_paths_module_desc env mpath mty comp
    end
  | Mty_signature _ ->
    let components = lazy (short_paths_module_components_desc env mpath comp) in
    Fresh (Signature components)
  | Mty_functor _ ->
    let apply path = short_paths_functor_components_desc env mpath comp path in
    Fresh (Functor apply)
  ...
  | Structure_comps c ->
    ...
    let comps =
      String.Map.fold (fun name { mda_declaration; mda_components; _ } acc ->
        ...
        let desc = short_paths_module_desc env mpath mty.md_type mda_components
        in
...
and short_paths_functor_components_desc env mpath comp path =
  ...
  | Functor_comps f ->
    let mty =
      try Hashtbl.find f.fcomp_subst_cache path
      with
      | Not_found ->
        let mty =
          let subst =
          ...
          in
          Subst.modtype (Rescope (Path.scope (Papply (mpath, path)))) subst
            f.fcomp_res
        in
        Hashtbl.add f.fcomp_subst_cache path mty; mty
    in
    let loc = Location.(in_file !input_name) in
...
let short_paths_additions_desc env additions =
  List.fold_left (fun acc add ->
    ...
    | Module_open (root, decls) ->
      String.Map.fold (fun name { mda_components = comps; _ } acc ->
        let id = Ident.create_local name in
        let path = Pdot (root, name) in
...
let update_short_paths env =
  ...
  | _ :: _ as additions ->
    ...
    in
    { env with  short_paths = Some short_paths; short_paths_additions = [] }
