...
let merlin_recovery_attributes attrs =
  ...
  match attrs with [] -> attrs' | attrs -> attrs' @ attrs
...
let trace_copy ?(copy=deep_copy ()) tr = Unification_trace.map_types copy tr

let error (loc, env, err) =
  let err =
    match err with
    | Label_mismatch (li, trace) -> Label_mismatch (li, trace_copy trace)
    ...
    | Apply_non_function t -> Apply_non_function (deep_copy () t)
    | Apply_wrong_label (l, t, b) -> Apply_wrong_label (l, deep_copy () t, b)
    | Wrong_name (s1, t, wn) ->
      Wrong_name (s1, { t with  ty = deep_copy () t.ty }, wn)
    | Undefined_method (t, s, l) -> Undefined_method (deep_copy () t, s, l)
    | Private_type t -> Private_type (deep_copy () t)
    | Private_label (li, t) -> Private_label (li, deep_copy () t)
    | Not_subtype (t1, t2) ->
    ...
    | Scoping_let_module (s, t) -> Scoping_let_module (s, deep_copy () t)
    | Less_general (s, tr) -> Less_general (s, trace_copy tr)
    | Not_a_packed_module t -> Not_a_packed_module (deep_copy () t)
    | err -> err
  in
  Error (loc, env, err)
...
*)
let re node =
  Cmt_format.add_saved_type (Cmt_format.Partial_expression node); node

let rp node =
  Cmt_format.add_saved_type (Cmt_format.Partial_pattern (Value, node)); node
...
let constant_or_raise env loc cst =
  match constant cst with Ok c -> c | Error err -> raise (error (loc, env, err))
  ...
  try
    ...
    | Some allow_recursive ->
      unify_gadt ~equations_level:(get_gadt_equations_level ()) ~allow_recursive
        env ty ty'
    | None -> unify !env ty ty'; nothing_equated
  with
...
let unify_pat ?refine env pat expected_ty =
  ...
  with
  | Error (loc, env, Pattern_type_clash (trace, None)) ->
...
let reset_pattern allow =
  pattern_variables := [];
  pattern_force := []; allow_modules := allow; module_variables := []
...
let sort_pattern_variables vs =
  List.sort (fun { pv_id = x; _ } { pv_id = y; _ } ->
  ...
  let rec unify_vars p1_vs p2_vs =
    ...
    | { pv_id = x1; pv_type = t1; _ } :: rem1,
      ...
      if x1 == x2 then
        ...
        ((try unify_var env (newvar ()) t1; unify env t1 t2
          with
          | Unify trace ->
...
let rec build_as_type env p =
  ...
  List.fold_left (fun as_ty (extra, _loc, _attrs) ->
    ...
    | Tpat_constraint cty ->
      ...
      unify !env (instance as_ty) (instance ty); ty
  ) as_ty p.pat_extra
...
and build_as_type_aux env p =
  ...
  | Tpat_record (lpl, _) ->
    ...
    if lbl.lbl_private = Private then
      ...
      in
      (Array.iter do_label lbl.lbl_all; ty)
  | Tpat_or (p1, p2, row) ->
    ...
    | None ->
      ...
      in
      unify_pat env { p2 with  pat_type = ty2 } ty1; ty1
    | Some row ->
...
let build_or_pat env loc lid =
  ...
  in
  let (pats, fields) =
    List.fold_left (fun (pats, fields) (l, f) ->
...
let split_cases env cases =
  let add_case lst case =
    function None -> lst | Some c_lhs -> { case with  c_lhs } :: lst
  in
  List.fold_right (fun ({ c_lhs; c_guard } as case) (vals, exns) ->
    ...
    | Some _, Some _ when c_guard <> None ->
      raise
        (error
           (c_lhs.pat_loc, env, Mixed_value_and_exception_patterns_under_guard))
...
= struct
  ...
  let warn_if_ambiguous warn lid env lbl rest =
    ...
    let expansion = Format.asprintf "%t" Printtyp.Conflicts.print_explanations
    in
    if paths <> [] then
      warn lid.loc
  ...
  =
    let lbl =
      ...
      | None ->
        ...
        | Ok ((lbl, use) :: rest) ->
          use (); warn_if_ambiguous warn lid env lbl rest; lbl
        end
        ...
        *)
        ...
        | exception Not_found ->
          ...
          | lbl ->
            ...
            (if not principal then warn_non_principal warn lid); lbl
          | exception Not_found ->
            begin match filter (force_error candidates_in_scope) with
            | Ok lbls | Error lbls ->
...
let map_fold_cont f xs k =
  List.fold_right (fun x k ys -> f x (fun y -> k (y :: ys))) xs (fun ys ->
    k (List.rev ys)
  ) []
...
=
  ...
  let lbl_a_list =
    List.sort (fun (_, lbl1, _) (_, lbl2, _) ->
...
let unify_head_only ~refine loc env ty constr =
  ...
  | Tconstr (p, args, m) ->
    ...
    enforce_constraints !env ty_res; unify_pat_types ~refine loc env ty_res ty
  | _ -> assert false
(* Typing of patterns *)
...
let set_state s env =
  Btype.backtrack s.snapshot; Ctype.set_levels s.levels; env := s.env
...
=
  fun category ~no_existentials ~mode ~env sp expected_ty k ->
    Builtin_attributes.warning_scope sp.ppat_attributes (fun () ->
      ...
      try type_pat_aux category ~no_existentials ~mode ~env sp expected_ty k
      with
      | Error _ as exn when mode = Normal ->
...
=
  fun category ~no_existentials ~mode ~env sp expected_ty k ->
    ...
    in
    let loc = sp.ppat_loc in
    let refine = match mode with Normal -> None | Counter_example _ -> Some true
    in
    let unif (x : pattern) : pattern =
      unify_pat ~refine env x (instance expected_ty); x
    in
    let rp x =
    ...
    | Ppat_construct (lid, sarg) ->
      ...
      let refine =
        if
          refine = None && constr.cstr_generalized && no_existentials = None
        then
          Some false
        else
          refine
      ...
      map_fold_cont (fun (p, t) -> type_pat Value p t)
        (List.combine sargs ty_args) (fun args ->
        rvp k
          {
    ...
    | Ppat_record (lid_sp_list, closed) ->
      ...
      let type_label_pat (label_lid, label, sarg) k =
        ...
        (try unify_pat_types ~refine loc env ty_res (instance record_ty)
         with
         | Error (_loc, _env, Pattern_type_clash (trace, _)) ->
           raise
    ...
    | Ppat_constraint (sp, sty) ->
      ...
      type_pat category sp expected_ty' (fun p ->(*Format.printf "%a@.%a@."
...
=
  ...
  try
    ...
    Some typed_p
  with
  | Error _ | Empty_branch -> set_state state env; None
...
let check_unused ?(lev=get_current_level ()) env expected_ty cases =
  Parmatch.check_unused (fun refute constrs labels spat ->
    match
      partial_pred ~lev ~splitting_mode:Backtrack_or ~explode:5 env expected_ty
        constrs labels spat
    with
...
let type_class_arg_pattern cl_num val_env met_env l spat =
  ...
  (if has_variants pat then
     (Parmatch.pressure_variants val_env [ pat ]; finalize_variants pat));
...
let force_delayed_checks () =
  ...
  List.iter (fun (f, w) ->
    Warnings.restore w; try f () with exn -> Msupport.raise_error exn
  ) (List.rev !delayed_checks);
  Warnings.restore w_old; reset_delayed_checks (); Btype.backtrack snap
...
let rec is_nonexpansive exp =
  ...
  | Texp_match (e, cases, _) ->
    ...
    in
    is_nonexpansive e &&
      List.for_all (fun { c_lhs; c_guard; c_rhs } ->
  ...
  | Texp_record { fields; extended_expression } ->
    Array.for_all (fun (lbl, definition) ->
  ...
  | Texp_object ({ cstr_fields = fields; cstr_type = { csig_vars = vars } }, _)
    ...
    List.for_all (fun field ->
      ...
      Vars.fold (fun _ (mut, _, _) b -> decr count; b && mut = Immutable) vars
        true &&
        !count = 0
...
and is_nonexpansive_mod mexp =
  ...
  | Tmod_ident _ | Tmod_functor _ | Tmod_hole -> true
...
let rec type_approx env sexp =
  ...
  | Pexp_coerce (e, sty1, sty2) ->
    let approx_ty_opt =
      function None -> newvar () | Some sty -> approx_type env sty
    in
...
let check_univars env kind exp ty_expected vars =
  ...
  let (exp_ty, vars) =
    ...
    | Tpoly (body, tl) ->
      ...
      unify_exp_types exp.exp_loc env exp_ty ty'; exp_ty, vars
    | _ -> assert false
  in
  end_def ();
  generalize exp_ty;
...
let generalize_and_check_univars env kind exp ty_expected vars =
  ...
  List.iter generalize vars; check_univars env kind exp ty_expected vars
...
let create_package_type loc env (p, l) =
  ...
  let fields =
    List.map (fun (name, ct) -> name, Typetexp.transl_simple_type env false ct)
      l
  in
  let ty =
    newty
...
let exists_ppat f p =
  ...
  let rec loop p = (if f p then raise Found else ()); shallow_iter_ppat loop p
  in
  match loop p with exception Found -> true | () -> false

let contains_polymorphic_variant p =
  exists_ppat (function
...
let unify_exp env exp expected_ty =
  ...
  with
  | Error (loc, env, Expr_type_clash (trace, tfc, None)) ->
...
and type_expect ?in_function ?recarg env sexp ty_expected_explained =
  Msupport.with_saved_types ~warning_attribute:sexp.pexp_attributes
    ?save_part:None (fun () ->
    let saved = save_levels () in
    try type_expect_ ?in_function ?recarg env sexp ty_expected_explained
    with
    | exn ->
        ...
        exp_desc =
          Texp_ident
            (Path.Pident (Ident.create_local "*type-error*"),
             ...
             {
               Types.val_type = ty_expected_explained.ty;
               val_kind = Val_reg;
               val_loc = loc;
               val_attributes = [];
...
=
  ...
  | Pexp_apply (sfunct, sargs) ->
    ...
    (if !Clflags.principal then
       (end_def (); generalize_structure funct.exp_type));
    ...
    (try
       ...
       let opt_exp =
         ...
         | Some sexp ->
           ...
           (if !Clflags.principal then
              (end_def (); generalize_structure exp.exp_type));
           Some exp
       in
       let (ty_record, expected_type) =
         ...
         | None | Some (_, _, false) ->
           ...
           | None -> ty, opath
           | Some exp ->
             begin match get_path exp.exp_type with
             | None -> ty, opath
             | Some (_, p', _) as opath ->
               let decl = Env.find_type p' env in
               begin_def ();
               let ty = newconstr p' (instance_list decl.type_params) in
               end_def (); generalize_structure ty; ty, opath
             end
           end
         | _ -> ty_expected, opath
       in
       ...
       let lbl_exp_list =
         wrap_disambiguate "This record expression is expected to have"
           ...
           (type_label_a_list loc closed env (fun e k ->
              k (type_label_exp true env loc ty_record e)
       ...
       let (opt_exp, label_definitions) =
         ...
         let matching_label lbl =
           List.find (fun (_, lbl', _) -> lbl'.lbl_pos = lbl.lbl_pos)
             lbl_exp_list
         in
         match opt_exp with
         | None ->
    ...
    let (arg, ty', cty, cty') =
      ...
      | None ->
        ...
        | _
          ...
          ->
          if
            not gen && (* first try a single coercion *)
              let snap = snapshot () in
              let (ty, _b) = enlarge_type env ty' in
              try force (); Ctype.unify env arg.exp_type ty; true
              with
              | Unify _ -> backtrack snap; false
          then
            ()
          else
            (try
    ...
    (try
       let (meth, exp, typ) =
         ...
         | Texp_ident (_path, lid, { val_kind = Val_anc (methods, cl_num) }) ->
           ...
           in
           begin match
             Env.find_value_by_name (Longident.Lident ("selfpat-" ^ cl_num)) env,
             Env.find_value_by_name (Longident.Lident ("self-" ^ cl_num)) env
           with
           | (_, ({ val_kind = Val_self (meths, _, _, privty) } as desc)),
             ...
             ->
             obj_meths := Some meths;
             let (_, typ) = filter_self_method env met Private meths privty in
     ...
     | Unify _ ->
       let valid_methods =
         ...
         | None ->
           begin match (expand_head env obj.exp_type).desc with
           | Tobject (fields, _) ->
  ...
  | Pexp_letop { let_ = slet; ands = sands; body = sbody } ->
    ...
    (if !Clflags.principal then
       (end_def ();
        ...
        generalize_structure ty_func_result; generalize_structure ty_result));
...
and type_function ?in_function loc attrs env ty_expected_explained l caselist =
  ...
  let (ty_arg, ty_res) =
    ...
    with
    | Unify _ ->
      begin match expand_head env ty_expected with
      | { desc = Tarrow _ } as ty ->
  ...
  in
  (if separate then
     (end_def (); generalize_structure ty_arg; generalize_structure ty_res));
  let (cases, partial) =
...
and type_label_access env srecord lid =
  ...
  (if !Clflags.principal then (end_def (); generalize_structure record.exp_type));
  let ty_exp = record.exp_type in
  let expected_type =
    try
      let (p0, p, _) = extract_concrete_record env ty_exp in
      Some
        (p0, p, (repr ty_exp).level = generic_level || not !Clflags.principal)
    with
    | Not_found -> None
  in
  try
    ...
    let label =
      wrap_disambiguate "This expression has" (mk_expected ty_exp)
        (Label.disambiguate () lid env expected_type) labels
    in
  ...
  try
    CamlinternalFormatBasics.(
      CamlinternalFormat.(
        ...
        and mk_lid_loc lid = { txt = lid; loc = loc }
        in
...
and type_label_exp create env loc ty_expected (lid, label, sarg) =
  ...
  (if separate then
     (end_def ();
      ...
      generalize_structure ty_arg; generalize_structure ty_res));
...
and type_argument ?explanation ?recarg env sarg ty_expected' ty_expected =
  ...
  | { desc = Tarrow (Nolabel, ty_arg, ty_res, _); level = lv }
    ...
    (if !Clflags.principal then (end_def (); generalize_structure texp.exp_type));
    ...
    if not (simple_res || no_labels ty_res) then
      (unify_exp env texp ty_expected; texp)
    else
...
and type_application env funct sargs =
  ...
  let type_unknown_arg (ty_fun, typed_args) (lbl, sarg) =
    let (ty_arg, ty_res) =
      try
        ...
        | Tarrow (l, t1, t2, _)
          when
            l = lbl || !Clflags.classic && lbl = Nolabel && not (is_optional l)
          ->
          t1, t2
        | td ->
      ...
      with
      | Msupport.Resume -> newvar (), ty_fun
    in
    let arg () =
      let arg = type_expect env sarg (mk_expected ty_arg) in
      (if is_optional lbl then unify_exp env arg (type_option (newvar ()))); arg
    in
    ...
    | ({ desc = Tarrow (l, ty, ty_fun, com); level = lv } as ty_fun'),
      ...
      let may_warn loc w =
        if not !warned && !Clflags.principal && lv <> generic_level then
          (warned := true; Location.prerr_warning loc w)
      in
      ...
      let (remaining_sargs, arg) =
        if ignore_labels then
        ...
        else(* Arguments can be commuted, try to fetch the argument
          ...
          | None ->
            ...
            (if optional && List.mem_assoc Nolabel sargs then
             ...
             else(* No argument was given for this parameter, we abstract over
                    it. *)
             
               (may_warn funct.exp_loc
                  (Warnings.Non_principal_labels "commuted an argument");
                omitted_parameters := (l, ty, lv) :: !omitted_parameters; None))
      in
  ...
  | [ Nolabel, sarg ] when is_ignore funct ->
    ...
    check_partial_application false exp; [ Nolabel, Some exp ], ty_res
  | _ ->
...
and type_construct env loc lid sarg ty_expected_explained attrs =
  ...
  (if separate then
     (end_def ();
      ...
      end_def ();
      List.iter generalize_structure ty_args; generalize_structure ty_res));
  let (ty_args0, ty_res) =
...
and type_unpacks ?in_function env unpacks sbody expected_ty =
  ...
  let (extended_env, tunpacks) =
    List.fold_left (fun (env, unpacks) (name, loc, uid) ->
      ...
      in
      let (id, env) = Env.enter_module_declaration ~scope name.txt pres md env
      in
      Typetexp.widen context; env, (id, name, pres, modl) :: unpacks
    ) (env, []) unpacks
  in
  (* ideally, we should catch Expr_type_clash errors
     in type_expect triggered by escaping identifiers from the local module
  ...
  *)
  ...
  in
  List.fold_left (fun body (id, name, pres, modl) ->(* go back to parent level *)
...
=
  ...
  ->
    ...
    let lev = (if may_contain_gadts then begin_def ()); get_current_level () in
    ...
    let half_typed_cases =
      List.map (fun ({ pc_lhs; pc_guard = _; pc_rhs = _ } as case) ->
    ...
    let cases =
      List.map
        ...
        ->
        let ext_env = if contains_gadt then do_copy_types ext_env else ext_env
        in
        let ext_env =
          add_pattern_variables ext_env pvs ~check:(fun s ->
            Warnings.Unused_var_strict s
...
=
  ...
  let spatl =
    List.map (fun { pvb_pat = spat; pvb_expr = sexp; pvb_attributes = attrs } ->
      attrs,
      ...
      | _, Pexp_constraint (_, sty)
        ...
        Pat.constraint_ ~loc:{ spat.ppat_loc with  Location.loc_ghost = true }
          spat sty
      | _ -> spat
      end
    ) spat_sexp_list
  ...
  List.iter (fun pat ->
    if has_variants pat then
      (Parmatch.pressure_variants env [ pat ]; finalize_variants pat)
  ) pat_list;
  ...
  let sexp_is_fun { pvb_expr = sexp; _ } =
    match sexp.pexp_desc with Pexp_fun _ | Pexp_function _ -> true | _ -> false
  in
  let exp_env =
    if is_recursive then
      new_env
    ...
    | { pbop_op = sop; pbop_exp = sexp; pbop_loc = loc; _ } :: rest ->
      ...
      (if !Clflags.principal then
         (end_def ();
          ...
          generalize_structure ty_arg; generalize_structure ty_result));
      ...
      (try unify env (instance ty_result) (instance expected_ty)
       with
       | Unify trace -> raise (error (loc, env, Bindings_type_clash trace)));
      let andop =
        {
          bop_op_name = sop;
          bop_op_path = op_path;
...
let report_unification_error
    ~loc ?sub env trace ?type_expected_explanation txt1 txt2
=
...
let report_error ~loc env =
  ...
  | Constructor_arity_mismatch (lid, expected, provided) ->
    Location.errorf ~loc
      "@[The constructor %a@ expects %i argument(s),@ \
       but is applied here to %i argument(s)@]" longident lid expected provided
  ...
  | Pattern_type_clash (trace, pat) ->
    ...
    Location.error_of_printer ~loc ~sub (fun ppf () ->
      Printtyp.report_unification_error ppf env trace (function
        | ppf -> fprintf ppf "This pattern matches values of type"
      ) (function
        | ppf ->
          fprintf ppf
            "but a pattern was expected which matches values of \
             type"
      )
    ) ()
  ...
  | Expr_type_clash (trace, explanation, exp) ->
    ...
    Location.error_of_printer ~loc ~sub (fun ppf () ->
      Printtyp.report_unification_error ppf env trace
        ...
        (report_type_expected_explanation_opt explanation) (function
    ...
    | Tarrow _ ->
      Location.errorf ~loc
        "@[<v>@[<2>This function has type@ %a@]\
         @ @[It is applied to too many arguments;@ %s@]@]" Printtyp.type_expr
        typ "maybe you forgot a `;'."
    | _ ->
  ...
  | Apply_wrong_label (l, ty, extra_info) ->
    ...
    Location.errorf ~loc ~sub:extra_info
      "@[<v>@[<2>The function applied to this argument has type@ %a@]@.\
       This argument cannot be applied %a@]" Printtyp.type_expr ty print_label l
        ...
        (if Path.is_constructor_typath type_path then
           fprintf ppf
             "@[The field %s is not part of the record \
              argument for the %a constructor@]" name.txt Printtyp.type_path
             type_path
         else
           fprintf ppf
             "@[@[<2>%s type@ %a%t@]@ \
              The %s %s does not belong to type %a@]" eorp Printtyp.type_expr ty
  ...
  | Name_type_mismatch (kind, lid, tp, tpl) ->
    ...
    let name = Datatype_kind.label_name kind in
    Location.error_of_printer ~loc (fun ppf () ->
      Printtyp.report_ambiguous_type_error ppf env tp tpl (function
        | ppf ->
        ...
        | ppf ->
          ...
          fprintf ppf
            "This expression cannot be coerced to type@;<1 2>%a;@ \
             it has type" (Printtyp.type_expansion ty) ty'
    ...
    if in_function then
      Location.errorf ~loc
        "This function expects too many arguments,@ \
         it should have type@ %a%t" Printtyp.type_expr ty
        (report_type_expected_explanation_opt explanation)
    else
      Location.errorf ~loc
        "This expression should not be a function,@ \
         the expected type is@ %a%t" Printtyp.type_expr ty
        (report_type_expected_explanation_opt explanation)
  ...
  | Less_general (kind, trace) ->
    report_unification_error ~loc env trace (fun ppf ->
      fprintf ppf "This %s has type" kind
  ...
  | Letop_type_clash (name, trace) ->
    report_unification_error ~loc env trace (function
      | ppf -> fprintf ppf "The operator %s has type" name
  ...
  | Bindings_type_clash trace ->
    report_unification_error ~loc env trace (function
      | ppf -> fprintf ppf "These bindings have type"
