...
let set_levels l =
  ...
  global_level := l.global_level; saved_level := l.saved_level
...
let end_def () =
  ...
  saved_level := List.tl !saved_level; current_level := cl; nongen_level := nl

let create_scope () = init_def (!current_level + 1); !current_level
let reset_global_level () = global_level := !current_level + 1

let increase_global_level () =
  let gl = !global_level in
  global_level := !current_level; gl
...
let wrap_trace_gadt_instances env f x =
  ...
  let y = f x in
  reset_trace_gadt_instances b; y
...
(**** unification mode ****)
type unification_mode = Expression (* unification in expression *) | Pattern
...
let in_current_module =
  function Path.Pident _ -> true | Path.Pdot _ | Path.Papply _ -> false
...
let is_self_type = function Tobject (ty, _) -> has_dummy_method ty | _ -> false
...
let close_object ty =
  let rec close ty =
    let ty = repr ty in
    match ty.desc with
    | Tvar _ -> link_type ty (newty2 ty.level Tnil); true
...
let free_vars ?env ty =
  ...
  free_variables := []; really_closed := None; res
...
let closed_parameterized_type params ty =
  ...
  List.iter unmark_type params; unmark_type ty; ok
...
let closed_type_decl decl =
  try
    ...
    | Type_abstract -> ()
    | Type_variant v ->
      List.iter (fun { cd_args; cd_res; _ } ->
        match cd_res with
        | Some _ -> ()
        | None ->
  ...
  with
  | Non_closed (ty, _) -> unmark_type_decl decl; Some ty
...
let closed_extension_constructor ext =
  try
    ...
    unmark_extension_constructor ext; None
  with
  | Non_closed (ty, _) -> unmark_extension_constructor ext; Some ty
...
let closed_class params sign =
  ...
  try
    ...
    List.iter unmark_type params; unmark_class_signature sign; None
  with
  | CCFailure reason ->
    mark_type_params (repr sign.csig_self);
    List.iter unmark_type params; unmark_class_signature sign; Some reason
(**********************)
(*  Type duplication  *)
...
let generalize ty = simple_abbrevs := Mnil; generalize ty
...
let generalize_structure ty =
  simple_abbrevs := Mnil; generalize_structure !current_level ty
...
let rec generalize_spine ty =
  ...
  if ty.level < !current_level || ty.level = generic_level then
    ...
    match ty.desc with
    | Tarrow (_, ty1, ty2, _) ->
      set_level ty generic_level; generalize_spine ty1; generalize_spine ty2
    | Tpoly (ty', _) -> set_level ty generic_level; generalize_spine ty'
    | Ttuple tyl | Tpackage (_, _, tyl) ->
      set_level ty generic_level; List.iter generalize_spine tyl
    | Tconstr (p, tyl, memo) when not (is_object_type p) ->
      set_level ty generic_level; memo := Mnil; List.iter generalize_spine tyl
    | _ -> ()
  ...
  if ty.level >= lowest_level then
    ((if level < ty.scope then raise (Trace.scope_escape ty));
     ...
     | Tconstr (p, _, _) when level < Path.scope p ->
       ...
       | ty' -> mark ty; check_scope_escape env level ty'
       | exception Cannot_expand ->
     ...
     | _ -> mark ty; iter_type_expr (check_scope_escape env level) ty)
...
let check_scope_escape env level ty =
  ...
  | Unify [ Trace.Escape x ] ->
    backtrack snap; raise Trace.(Unify [ Escape { x with  context = Some ty } ])

let update_scope scope ty =
  let ty = repr ty in
  let scope = max scope ty.scope in
  (if ty.level < scope then raise (Trace.scope_escape ty)); set_scope ty scope
    ...
    ((if level < ty.scope then raise (Trace.scope_escape ty));
     ...
     | Tconstr (p, _tl, _abbrev) when level < Path.scope p ->
       ...
       (try
        ...
        with
        | Cannot_expand -> raise Trace.(Unify [ escape (Constructor p) ]))
     | Tconstr (p, (_ :: _ as tl), _) ->
       ...
       let needs_expand =
         expand ||
           List.exists2 (fun var ty ->
             var = Variance.null && (repr ty).level > level
           ) variance tl
       in
       (try
        ...
        with
        | Cannot_expand ->
          set_level ty level; iter_type_expr (update_level env level expand) ty)
     | Tpackage (p, nl, tl) when level < Path.scope p ->
       let p' = normalize_package_path env p in
       (if Path.same p p' then raise Trace.(Unify [ escape (Module_type p) ]));
     ...
     | Tobject (_, ({ contents = Some (p, _tl) } as nm))
       ...
       set_name nm None; update_level env level expand ty
     | Tvariant row ->
       let row = row_repr row in
...
let update_level env level ty =
  ...
  if ty.level > level then
    ...
    with
    | Unify _ -> backtrack snap; update_level env level true ty
...
let rec lower_contravariant env var_level visited contra ty =
  ...
  if must_visit then
    (Hashtbl.add visited ty.id contra;
     ...
     | Tarrow (_, t1, t2, _) -> lower_rec true t1; lower_rec contra t2
  ...
  let rec inverse pty ty =
    ...
    if ty.level > !current_level || ty.level = generic_level then
      (decr idx;
       ...
       set_level ty !idx; iter_type_expr (inverse [ ty ]) ty)
...
let rec inv_type hash pty ty =
  ...
  | Not_found ->
    ...
    TypeHash.add hash ty inv; iter_type_expr (inv_type hash [ inv ]) ty
    ...
    acc &&
      ...
      (ty.level < lowest_level ||
         ty.level = generic_level &&
           (mark_type_node ty; fold_type_expr aux true ty))
  in
...
let rec copy ?partial ?keep_names scope ty =
  ...
  | Tsubst ty -> ty
  | _ ->
    if ty.level <> generic_level && partial = None then
      ty
    else(* We only forget types that are non generic and do not contain
           free univars *)
        ...
        (For_copy.save_desc scope ty desc;
          ...
          *)
           ...
           | Tvariant row0 ->
             ...
             | _ ->
               ...
               let more' =
                 ...
                 | Tsubst ty -> ty
                 | Tconstr _ | Tnil ->
                   For_copy.save_desc scope more more.desc; copy more
                 | Tvar _ | Tunivar _ ->
           ...
           | Tfield (_p, k, _ty1, ty2) ->
             ...
             | Fvar r -> For_copy.dup_kind scope r; copy_type_desc copy desc
             end
  ...
  For_copy.with_scope (fun scope ->
    ...
    | Some (env, expansion_scope) ->
      let process existential =
        ...
        assert (is_Tvar tv); link_type tv to_unify
      in
...
let rec copy_sep cleanup_scope fixed free bound visited ty =
  ...
  if TypeSet.is_empty univars then
    ...
    try
      ...
      (if dl <> [] && conflicts univars dl then raise Not_found); t
    with
    | Not_found ->
      let t = newvar () in
      (* Stub *)
      ...
      t.desc <-
        ...
        | Tpoly (t1, tl) ->
          ...
          let visited = List.map2 (fun ty t -> ty, (t, bound)) tl tl' @ visited
          in
...
let instance_poly' cleanup_scope ~keep_names fixed univars sch =
  ...
  List.iter Lazy.force !delayed_copy; delayed_copy := []; vars, ty
...
let subst env level priv abbrev ty params args body =
  ...
  try
    ...
    List.iter2 (!unify' env) params' args; current_level := old_level; body'
  with
  | Unify _ as exn -> current_level := old_level; raise exn
...
let check_abbrev_env env =
  if env != !previous_env then
    ...
    (cleanup_abbrev (); previous_env := env)
      ...
      | params, body, lv ->
        ...
        (if !trace_gadt_instances then
           ...
           ((if level < scope then raise (Trace.scope_escape ty));
            set_scope ty scope; set_scope ty' scope));
        ty'
      end
    end
  | _ -> assert false
...
let safe_abbrev env ty =
  ...
  with
  | Cannot_expand | Unify _ -> Btype.backtrack snap; false
...
let rec extract_concrete_typedecl env ty =
  ...
  | Tconstr (p, _, _) ->
    ...
    if decl.type_kind <> Type_abstract then
      ...
      let ty = try try_expand_safe env ty with Cannot_expand -> raise Not_found
      in
...
let expand_head_opt env ty =
  ...
  with
  | Cannot_expand | Unify _ -> (* expand_head shall never fail *)
...
let occur env ty0 ty =
  ...
  try
    while
      type_changed := false;
      occur_rec env allow_recursive TypeSet.empty ty0 ty; !type_changed
    do
      ()
    (* prerr_endline "changed" *) done;
...
let occur_univar env ty =
        ...
        (if TypeSet.is_empty bound then
           ...
           try
             ...
             if TypeSet.exists (fun x -> not (TypeSet.mem x bound)) bound' then
               ...
               false
           with
           | Not_found -> visited := TypeMap.add ty bound !visited; true)
    then
      match ty.desc with
      | Tunivar _ ->
      ...
      | Tconstr (p, tl, _) ->
        (try
           ...
           List.iter2 (fun t v ->(* The null variance only occurs in type abbreviations and
  ...
  in
  Misc.try_finally (fun () -> occur_rec TypeSet.empty ty) ~always:(fun () ->
    unmark_type ty
  )
...
let enter_poly env univar_pairs t1 tl1 t2 tl2 f =
  ...
  Misc.try_finally (fun () -> f t1 t2) ~always:(fun () ->
  ...
  let rec occur_rec ty =
    ...
    if ty.level >= t0.level then
      ((if ty == t0 then raise Occur);
       ty.level <- pivot_level - ty.level; iter_type_expr occur_rec ty)
  in
  try occur_rec ty; unmark_type ty; false with Occur -> unmark_type ty; true
          ...
          (try TypePairs.find type_pairs (t1', t2')
           ...
           | Not_found ->
             ...
             | Tarrow (l1, t1, u1, _), Tarrow (l2, t2, u2, _)
               ...
               mcomp type_pairs env t1 t2; mcomp type_pairs env u1 u2
...
and mcomp_fields type_pairs env ty1 ty2 =
  ...
  (if
   ...
   then
     raise (Unify []));
  List.iter (function
    | _n, k1, t1, k2, t2 -> mcomp_kind k1 k2; mcomp type_pairs env t1 t2
  ) pairs
  ...
  let rec find ty =
    ...
    if ty.level >= lowest_level then
      ((if ty.level < !lowest then lowest := ty.level);
       ty.level <- pivot_level - ty.level; iter_type_expr find ty)
  in
...
let add_gadt_equation env source destination =
  ...
  if local_non_recursive_abbrev !env source destination then
    ...
    let expansion_scope = max (Path.scope source) (get_gadt_equations_level ())
    in
    let decl = new_declaration expansion_scope (Some destination) in
    (env := Env.add_local_type source decl !env; cleanup_abbrev ())
...
let unify_package env unify_list lv1 p1 n1 tl1 lv2 p2 n2 tl2 =
  ...
  and ntl1 = complete_type_list env n2 lv1 (Mty_ident p1) n1 tl1
  in
...
let unify1_var env t1 t2 =
  ...
  let d1 = t1.desc in
  link_type t1 t2;
  try update_level env t1.level t2; update_scope t1.scope t2
  with
  | Unify _ as e -> t1.desc <- d1; raise e
    ...
    if unify_eq t1 t2 then
      ...
      try
        ...
        | Tunivar _, Tunivar _ ->
          ...
          update_scope t1.scope t2; link_type t1 t2
...
and unify2 env t1 t2 =
  ...
  if unify_eq t1' t2' then
    ...
    if unify_eq t1 t1' || not (unify_eq t2 t2') then
      ...
      with
      | Unify trace -> raise (Unify (Trace.swap trace))
...
and unify3 env t1 t1' t2 t2' =
  ...
  match d1, d2 with (* handle vars and univars specially *)
  | Tunivar _, Tunivar _ ->
    unify_univar t1' t2' !univar_pairs; link_type t1' t2'
  | Tvar _, _ -> occur !env t1' t2; occur_univar !env t2; link_type t1' t2
  | _, Tvar _ -> occur !env t2' t1; occur_univar !env t1; link_type t2' t1
  | Tfield _, Tfield _ -> (* special case for GADTs *) unify_fields env t1' t2'
  | _ ->
    ...
    (try
           ...
           List.iter2 (fun i (t1, t2) ->
             if i then
               ...
               set_mode_pattern ~generate:Forbidden ~injective:false
                 ...
                 try unify env t1 t2
                 with
                 | Unify _ -> backtrack snap; reify env t1; reify env t2
               )
       ...
       | Tconstr (path, [], _), Tconstr (path', [], _)
         ...
         in
         record_equation t1' t2'; add_gadt_equation env source destination
       | Tconstr (path, [], _), _
         when is_instantiable !env path && can_generate_equations () ->
         reify env t2'; record_equation t1' t2'; add_gadt_equation env path t2'
       | _, Tconstr (path, [], _)
         when is_instantiable !env path && can_generate_equations () ->
         reify env t1'; record_equation t1' t2'; add_gadt_equation env path t1'
       | Tconstr (_, _, _), _ | _, Tconstr (_, _, _) when !umode = Pattern ->
         reify env t1';
         reify env t2';
         if can_generate_equations () then
           (mcomp !env t1' t2'; record_equation t1' t2')
       ...
       | Tfield (f, kind, _, rem), Tnil | Tnil, Tfield (f, kind, _, rem) ->
         ...
         | Fvar r when f <> dummy_method ->
           ...
           if d2 = Tnil then
             ...
             unify env (newty2 rem.level Tnil) rem
         | _ ->
     ...
     with
     | Unify trace -> t1'.desc <- d1; raise (Unify trace))
...
and make_rowvar level use1 rest1 use2 rest2 =
  ...
  let name =
    ...
    | Tvar (Some _ as name), _ -> (if use2 then set_name rest2 name); name
    | _, Tvar (Some _ as name) -> (if use1 then set_name rest2 name); name
    | _ -> None
  in
...
and unify_fields env ty1 ty2 =
  ...
  in
  let (pairs, miss1, miss2) = associate_fields fields1 fields2 in
  let l1 = (repr ty1).level
  and l2 = (repr ty2).level
  in
  ...
  try
    ...
    List.iter (fun (n, k1, t1, k2, t2) ->
      unify_kind k1 k2;
      try
        (if !trace_gadt_instances then
           (update_level !env va.level t1; update_scope va.scope t1));
        unify env t1 t2
      with
      | Unify trace ->
  ...
  with
  | exn -> set_type_desc rest1 d1; set_type_desc rest2 d2; raise exn
  ...
  if unify_eq rm1 rm2 then
    ...
    ((if r1 <> [] && r2 <> [] then
     ...
     (if
      ...
      then
        raise Trace.(Unify [ Variant No_intersection ]));
     let name =
       if
         row1.row_name <> None &&
     ...
     let set_more row rest =
       ...
       if row_fixed row then
         ...
         (update_level !env rm.level ty;
          update_scope rm.scope ty; link_type rm ty)
     in
     let md1 = rm1.desc
     and md2 = rm2.desc
     in
     ...
     with
     | exn -> set_type_desc rm1 md1; set_type_desc rm2 md2; raise exn)
...
and unify_row_field env fixed1 fixed2 rm1 rm2 l f1 f2 =
  ...
  let if_not_fixed (pos, fixed) f =
    ...
    | None -> f ()
    | Some fix ->
      let tr = Trace.([ Variant (Fixed_row (pos, Cannot_add_tags [ l ], fix)) ])
      in
      raise (Unify tr)
  in
  let first = Trace.First, fixed1
  and second = Trace.Second, fixed2
  in
  let either_fixed = match fixed1, fixed2 with None, None -> false | _ -> true
  in
  if f1 == f2 then
    ()
  else
    ...
    | Reither (c1, tl1, m1, e1), Reither (c2, tl2, m2, e2) ->
      ...
      then
        ...
        (set_row_field e1 f; set_row_field e2 f; List.iter2 (unify env) tl1 tl2)
      else
        let redo =
          (m1 ||
             m2 ||
            ...
            | t1 :: tl ->
              ...
              List.iter (unify env t1) tl; !e1 <> None || !e2 <> None
        in
        if redo then
          ...
          and (tl2', tlu2) = split_univars tl2'
          in
          (begin match tlu1, tlu2 with
           ...
           List.iter (fun ty ->
             let rm = repr rm2 in
             update_level !env rm.level ty; update_scope rm.scope ty
           ) tl1';
           List.iter (fun ty ->
             let rm = repr rm1 in
             update_level !env rm.level ty; update_scope rm.scope ty
    ...
    | Rpresent (Some t1), Reither (false, tl, _, e2) ->
      if_not_fixed second (fun () ->
        ...
        try List.iter (unify env t1) tl with exn -> e2 := None; raise exn
      )
...
let unify env ty1 ty2 =
  ...
  with
  | Unify trace -> undo_compress snap; raise (Unify (expand_trace !env trace))
...
let unify_gadt ~equations_level:lev ~allow_recursive (env : Env.t ref) ty1 ty2 =
  try
    ...
    gadt_equations_level := None; TypePairs.clear unify_eq_set; equated_types
  with
  | e -> gadt_equations_level := None; TypePairs.clear unify_eq_set; raise e
  ...
  if t1 == t2 then
    ...
    | Tvar _, _ ->
      ...
      (try
         ...
         link_type t1 t2; reset_trace_gadt_instances reset_tracing
       with
       | Unify trace ->
         reset_trace_gadt_instances reset_tracing;
...
let expand_head_trace env t =
  ...
  reset_trace_gadt_instances reset_tracing; t
...
let rec filter_method_field env name priv ty =
  ...
  | Tfield (n, kind, ty1, ty2) ->
    ...
    if n = name && kind <> Fabsent then
      ((if priv = Public then unify_kind kind Fpresent); ty1)
    else
...
let filter_method env name priv ty =
  ...
  | Tvar _ ->
    ...
    link_type ty ty'; filter_method_field env name priv ty1
...
let filter_self_method env lab priv meths ty =
  ...
  | Not_found ->
    ...
    meths := Meths.add lab pair !meths; pair
(***********************************)
  ...
  let rec occur ty =
    ...
    if ty.level > level then
      ((if is_Tvar ty && ty.level >= generic_level - 1 then raise Occur);
       ty.level <- pivot_level - ty.level; iter_type_expr occur ty)
  in
  (try occur ty; unmark_type ty with Occur -> unmark_type ty; raise (Unify []));
    ...
    if t1 == t2 then
      ...
      try
        ...
        | Tvar _, _ when may_instantiate inst_nongen t1 ->
          ...
          update_scope t1.scope t2; occur env t1 t2; link_type t1 t2
        | Tconstr (p1, [], _), Tconstr (p2, [], _) when Path.same p1 p2 -> ()
        | _ ->
      ...
      with
      | Unify trace -> raise (Unify (Trace.diff t1 t2 :: trace))
...
and moregen_fields inst_nongen type_pairs env ty1 ty2 =
  ...
  List.iter (fun (n, k1, t1, k2, t2) ->
    ...
    | Unify trace ->
      let e =
        Trace.diff (newty (Tfield (n, k1, t1, rest2)))
...
and moregen_row inst_nongen type_pairs env row1 row2 =
  ...
  and row2 = row_repr row2
  in
  let rm1 = repr row1.row_more
  and rm2 = repr row2.row_more
  in
  if rm1 == rm2 then
    ()
  else
    ...
    ((if r1 <> [] || row1.row_closed && (not row2.row_closed || r2 <> []) then
     ...
     | _ when may_inst ->
       ...
       update_scope rm1.scope ext; link_type rm1 ext
     | Tconstr _, Tconstr _ -> moregen inst_nongen type_pairs env rm1 rm2
     | _ -> raise (Unify [])
     end;
     List.iter (fun (_l, f1, f2) ->
       let f1 = row_field_repr f1
       and f2 = row_field_repr f2
       in
       if f1 == f2 then
...
let moregen inst_nongen type_pairs env patt subj =
  univar_pairs := []; moregen inst_nongen type_pairs env patt subj
...
let moregeneral env inst_nongen pat_sch subj_sch =
  ...
  *)
  ...
  let res =
    try moregen inst_nongen (TypePairs.create 13) env patt subj; true
    with
    | Unify _ -> false
  in
  current_level := old_level; res
...
let all_distinct_vars env vars =
  ...
  List.for_all (fun ty ->
    ...
    if List.memq ty !tyl then false else (tyl := ty :: !tyl; is_Tvar ty)
  ) vars
...
let matches env ty ty' =
  ...
  cleanup_abbrev ();
  let ok =
    try unify env ty ty'; all_distinct_vars env vars with Unify _ -> false
  in
  backtrack snap; ok
(*********************************************)
...
let normalize_subst subst =
  ...
  then
    subst := List.map (fun (t1, t2) -> repr t1, repr t2) !subst
...
let eqtype_list rename type_pairs subst env tl1 tl2 =
  univar_pairs := [];
  let snap = Btype.snapshot () in
  Misc.try_finally ~always:(fun () -> backtrack snap) (fun () ->
    eqtype_list rename type_pairs subst env tl1 tl2
  )
...
let equal env rename tyl1 tyl2 =
  try eqtype_list rename (TypePairs.create 11) (ref []) env tyl1 tyl2; true
  with
  | Unify _ -> false
(*************************)
...
let rec moregen_clty trace type_pairs env cty1 cty2 =
  try
    ...
    | Cty_signature sign1, Cty_signature sign2 ->
      ...
      and (fields2, _rest2) = flatten_fields ty2
      in
      let (pairs, _miss1, _miss2) = associate_fields fields1 fields2 in
      List.iter (fun (lab, _k1, t1, _k2, t2) ->
...
let match_class_types ?(trace=true) env pat_sch subj_sch =
  ...
  *)
  ...
  let res =
    ...
    and (fields2, rest2) = flatten_fields (object_fields t2)
    in
    let (pairs, miss1, miss2) = associate_fields fields1 fields2 in
    let error =
      List.fold_right (fun (lab, k, _) err ->
        let err =
        ...
        in
        if lab = dummy_method || Concr.mem lab sign1.csig_concr then
          err
        else
          CM_Hide_virtual ("method", lab) :: err
      ) miss1 []
    in
    let missing_method = List.map (fun (m, _, _) -> m) miss2 in
    let error = List.map (fun m -> CM_Missing_method m) missing_method @ error
    in
    (* Always succeeds *)
    moregen true type_pairs env rest1 rest2;
    let error =
      List.fold_right (fun (lab, k1, _t1, k2, _t2) err ->
        try moregen_kind k1 k2; err with Unify _ -> CM_Public_method lab :: err
      ) pairs error
    in
    let error =
      Vars.fold (fun lab (mut, vr, _ty) err ->
        try
          ...
          if mut = Mutable && mut' <> Mutable then
            ...
            err
        with
        | Not_found -> CM_Missing_value lab :: err
      ) sign2.csig_vars error
    in
    let error =
      Vars.fold (fun lab (_, vr, _) err ->
    ...
    let error =
      ...
      ) (Concr.elements (Concr.diff sign2.csig_concr sign1.csig_concr)) error
    in
    match error with
    | [] ->
      (try moregen_clty trace type_pairs env patt subj; [] with Failure r -> r)
...
let match_class_declarations env patt_params patt_type subj_params subj_type =
  ...
  let error =
    List.fold_right (fun (lab, k, _) err ->
      ...
      in
      if Concr.mem lab sign1.csig_concr then
        err
      else
        CM_Hide_virtual ("method", lab) :: err
    ) miss1 []
...
let rec build_subtype env visited loops posi level t =
  ...
  | Tconstr (p, tl, abbrev)
    ...
    (try
       ...
       | Tobject _ when posi && not (opened_object t') ->
         ...
         (try unify_var env ty t with Unify _ -> assert false); t'', Changed
       | _ -> raise Not_found
     with
     | Not_found ->
  ...
  | Tvariant row ->
    let row = row_repr row in
    if memq_warn t visited || not (static_row row) then
      t, Unchanged
    else
      let level' = pred_enlarge level in
      let visited = t :: if level' < level then [] else filter_visited visited
      in
      let fields = filter_row_fields false row.row_fields in
      let fields =
        List.map (fun (l, f as orig) ->
          match row_field_repr f with
          | Rpresent None ->
            if posi then
  ...
  | Tobject (t1, _) ->
    if memq_warn t visited || opened_object t1 then
      t, Unchanged
    else
      let level' = pred_enlarge level in
      let visited = t :: if level' < level then [] else filter_visited visited
...
let rec subtype_rec env trace t1 t2 cstrs =
  ...
  if t1 == t2 then
    ...
    try TypePairs.find subtypes (t1, t2); cstrs
    with
    | Not_found ->
      ...
      | Tconstr (p1, tl1, _), Tconstr (p2, tl2, _) when Path.same p1 p2 ->
        (try
           let decl = Env.find_type p1 env in
           List.fold_left2 (fun cstrs v (t1, t2) ->
             let (co, cn) = Variance.get_upper v in
             if co then
               (if cn then
                  (trace,
      ...
      | Tpoly (u1, tl1), Tpoly (u2, tl2) ->
        (try
         ...
         with
         | Unify _ -> (trace, t1, t2, !univar_pairs) :: cstrs)
           ...
           if eq_package_path env p1 p2 then
             ...
             try
               ...
               if !package_subtype env p1 nl1 tl1 p2 nl2 tl2 then
                 ...
                 raise (Unify [])
             with
             | Unify _ -> Btype.backtrack snap; raise Not_found
         with
         | Not_found -> (trace, t1, t2, !univar_pairs) :: cstrs)
...
and subtype_list env trace tl1 tl2 cstrs =
  ...
  List.fold_left2 (fun cstrs t1 t2 ->
    subtype_rec env (Trace.diff t1 t2 :: trace) t1 t2 cstrs
  ) cstrs tl1 tl2
    ...
    if miss2 = [] then
      ...
      (trace,
       ...
       !univar_pairs) ::
        cstrs
  in
  List.fold_left (fun cstrs (_, _k1, t1, _k2, t2) ->(* These fields are always present *)
...
and subtype_row env trace row1 row2 cstrs =
  ...
  | (Tvar _ | Tconstr _ | Tnil), (Tvar _ | Tconstr _ | Tnil)
    ...
    List.fold_left (fun cstrs (_, f1, f2) ->
      match row_field_repr f1, row_field_repr f2 with
...
let closed_schema env ty =
  ...
  try closed_schema_rec env ty; visited := TypeSet.empty; true
  with
  | Non_closed0 -> visited := TypeSet.empty; false
       ...
       ty'.desc <-
           ...
           | Some id ->
             (try
              ...
              (*
                 ...
                 *)
              with
              | Cannot_expand | Unify _ -> raise (Nondep_cannot_erase id))
           | None ->
         ...
         | Tvariant row ->
           ...
           (try
              ...
              (* This variant type has been already copied *)
              TypeHash.add nondep_hash ty ty2; Tlink ty2
            with
            | Not_found ->
              (* Register new type first for recursion *)
...
let nondep_type env id ty =
  try
    ...
    clear_hash (); ty'
  with
  | Nondep_cannot_erase _ as exn -> clear_hash (); raise exn
...
let nondep_type_decl env mid is_covariant decl =
  try
    ...
    clear_hash ();
    let priv =
      match tm with Some ty when Btype.has_constr_row ty -> Private | _ -> priv
    in
    {
      type_params = params;
