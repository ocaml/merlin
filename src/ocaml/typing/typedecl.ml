...
let add_type ~check id decl env =
  Builtin_attributes.warning_scope ~ppwarning:false decl.type_attributes
    (fun () ->
    Env.add_type ~check id decl env
  )
...
let transl_labels env closed lbls =
  ...
  List.iter (fun { pld_name = { txt = name; loc } } ->
  ...
  | Some sret_type ->
    ...
    | _ ->
      raise
        (Error
           (sret_type.ptyp_loc,
            Constraint_failed (ret_type, Ctype.newconstr type_path type_params)))
    end;
    widen z; targs, Some tret_type, args, Some ret_type
...
let transl_declaration env sdecl (id, uid) =
  ...
  let cstrs =
    List.map (fun (sty, sty', loc) ->
      transl_simple_type env false sty, transl_simple_type env false sty', loc
    ) sdecl.ptype_cstrs
  in
  ...
  let (tkind, kind) =
    ...
    | Ptype_variant scstrs ->
      ...
      List.iter (fun { pcd_name = { txt = name } } ->
  ...
  List.iter (fun (cty, cty', loc) ->
    ...
    with
    | Ctype.Unify tr -> raise (Error (loc, Inconsistent_constraint (env, tr)))
  ) cstrs;
  Ctype.end_def ();
  (* Add abstract row *)
  ...
  if TypeSet.mem ty !visited then
    ...
    (visited := TypeSet.add ty !visited;
     ...
     | Tconstr (path, args, _) ->
       ...
       (try Ctype.enforce_constraints env ty'
        with
...
let check_constraints_labels env visited l pl =
  ...
  in
  List.iter (fun { Types.ld_id = name; ld_type = ty } ->
    check_constraints_rec env (get_loc (Ident.name name) pl) visited ty
  ) l

let check_constraints env sdecl (_, decl) =
  let visited = ref TypeSet.empty in
  List.iter2 (fun (sty, _) ty ->
  ...
  | Type_variant l ->
    ...
    in
    List.iter (fun { Types.cd_id = name; cd_args; cd_res } ->
  ...
  | { type_kind = (Type_variant _ | Type_record _ | Type_open);
    ...
    | Tconstr (path, args, _) ->
      (try
         ...
         in
         if err <> None then raise (Error (loc, Definition_mismatch (ty, err)))
       with
       | Not_found -> raise (Error (loc, Unavailable_type_constructor path)))
    ...
    if fini then
      ...
      let arg_exn =
        try
          ...
          in
          Btype.iter_type_expr (check ty0 parents) ty; None
        with
        | e -> visited := visited'; Some e
      in
      match ty.desc with
...
let check_recursion ~orig_env env loc path decl to_check =
  ...
  if decl.type_params = [] then
    ...
    Option.iter (fun body ->
      let (args, body) =
        Ctype.instance_parameterized_type ~keep_names:true decl.type_params body
      in
  ...
  List.iter (fun sdecl ->
    ...
    | Ptype_variant cl ->
      List.iter (fun pcd ->
        ...
        with
        | Not_found -> Hashtbl.add constrs pcd.pcd_name.txt sdecl.ptype_name.txt
      ) cl
    | Ptype_record fl ->
      List.iter (fun { pld_name = cname; pld_loc = loc } ->
        try
  ...
  let ids_slots (id, _uid as ids) =
    ...
    | Asttypes.Recursive when warn_unused ->
      ...
      Env.set_type_used_callback td (fun old_callback ->
        ...
        | None -> List.iter Env.mark_type_used (get_ref slot); old_callback ()
      );
      ids, Some slot
...
=
  Builtin_attributes.warning_scope sext.pext_attributes (fun () ->
    transl_extension_constructor ~scope env type_path type_params typext_params
      priv sext
  )
...
let transl_type_extension extend env loc styext =
  ...
  let err =
    if type_decl.type_arity <> List.length styext.ptyext_params then
      ...
      List.for_all2 (fun (c1, n1, _) (c2, n2, _) ->
        (not c2 || c1) && (not n2 || n1)
      ) type_variance
  ...
  List.iter (fun ext ->(* Note that [loc] here is distinct from [type_decl.type_loc], which
...
let transl_with_constraint id row_path ~sig_env ~sig_decl ~outer_env sdecl =
  ...
  List.iter (fun (cty, cty', loc) ->(* Note: contraints must also be enforced in [sig_env] because
...
let explain_unbound_single ppf tv ty =
  ...
  | Tvariant row ->
    let row = Btype.row_repr row in
    if row.row_more == tv then
      trivial ty
    else
      explain_unbound ppf tv row.row_fields (fun (_l, f) ->
...
let report_error ppf =
  ...
  | Definition_mismatch (ty, Some err) ->
    fprintf ppf "@[<v>@[<hov>%s@ %s@;<1 2>%a@]%a@]"
      "This variant or record definition" "does not match that of type"
      Printtyp.type_expr ty
      (Includecore.report_type_mismatch "the original" "this" "definition") err
  | Constraint_failed (ty, ty') ->
    ...
    fprintf ppf "@[%s@ @[<hv>Type@ %a@ should be an instance of@ %a@]@]"
      ...
      (Printtyp.tree_of_typexp false ty) !Oprint.out_type
    ...
    | [] ->
      fprintf ppf
        "@[<hv>This recursive type is not regular.@ \
         ...
         to be regular.@]" (Path.name definition) !Oprint.out_type
        (Printtyp.tree_of_typexp false defined_as) !Oprint.out_type
    ...
    | _ :: _ ->
      fprintf ppf
        "@[<hv>This recursive type is not regular.@ \
         ...
         to be regular.@]" (Path.name definition) !Oprint.out_type
        (Printtyp.tree_of_typexp false defined_as) !Oprint.out_type
        (Printtyp.tree_of_typexp false used_as) pp_expansions expansions
    end
  ...
  | Missing_native_external ->
    fprintf ppf
      "@[<hv>An external function with more than 5 arguments \
  ...
  | Unbound_type_var (ty, decl) ->
    ...
    | Type_record (tl, _), _ ->
      explain_unbound ppf ty tl (fun l -> l.Types.ld_type) "field" (fun l ->
  ...
  | Rebind_wrong_type (lid, env, trace) ->
    Printtyp.report_unification_error ppf env trace (function
      | ppf -> fprintf ppf "The constructor %a@ has type" Printtyp.longident lid
  ...
  | Cannot_unbox_or_untag_type Untagged ->
    fprintf ppf
      "@[Don't know how to untag this type.@ \
  ...
  | Separability (Typedecl_separability.Non_separable_evar evar) ->
    ...
    fprintf ppf
      "@[This type cannot be unboxed because@ \
       ...
       You should annotate it with [%@%@ocaml.boxed].@]" pp_evar evar
  | Boxed_and_unboxed ->
