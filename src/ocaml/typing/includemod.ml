...
and try_modtypes ~loc env ~mark cxt subst mty1 mty2 =
  ...
  | Mty_alias p1, _ ->
    ...
    strengthened_modtypes ~loc ~aliasable:true env ~mark cxt subst mty1 p1 mty2
  | Mty_ident p1, Mty_ident p2 ->
    ...
    if Path.same p1 p2 then
      ...
      try_modtypes ~loc env ~mark cxt subst (try_expand_modtype_path env p1)
        (try_expand_modtype_path env p2)
  | Mty_ident p1, _ ->
    let p1 = Env.normalize_modtype_path env p1 in
    try_modtypes ~loc env ~mark cxt subst (try_expand_modtype_path env p1) mty2
  | _, Mty_ident p2 ->
    let p2 = Env.normalize_modtype_path env (Subst.modtype_path subst p2) in
    try_modtypes ~loc env ~mark cxt subst mty1 (try_expand_modtype_path env p2)
  | Mty_signature sig1, Mty_signature sig2 ->
    signatures ~loc env ~mark cxt subst sig1 sig2
  | Mty_functor (Unit, res1), Mty_functor (Unit, res2) ->
    begin match modtypes ~loc env ~mark (Body Unit :: cxt) subst res1 res2 with
  ...
  | Mty_functor ((Named (param1, arg1) as arg), res1),
    ...
    let cc_arg =
      modtypes ~loc env ~mark:(negate_mark mark) (Arg arg :: cxt) Subst.identity
        arg2' arg1
    in
    let (env, subst) =
  ...
  | Mty_for_hole, _mty | _mty, Mty_for_hole -> Tcoerce_none
...
and signature_components ~loc old_env ~mark env cxt subst paired =
  ...
  | (Sig_value (id1, valdecl1, _), Sig_value (_id2, valdecl2, _), pos) :: rem ->
    let cc = value_descriptions ~loc env ~mark cxt subst id1 valdecl1 valdecl2
    in
  ...
  | (Sig_module (id1, pres1, mty1, _, _),
    ...
    begin match pres1, pres2, mty1.md_type with
  ...
  | (Sig_modtype (id1, info1, _), Sig_modtype (_id2, info2, _), _pos) :: rem ->
    modtype_infos ~loc env ~mark cxt subst id1 info1 info2; comps_rec rem
...
module Illegal_permutation = struct
  ...
  let rec transposition_under path =
    function
...
let rec context ppf =
  ...
  | Modtype id :: rem ->
    fprintf ppf "@[<2>module type %a =@ %a@]" Printtyp.ident id context_mty rem
  | Body x :: rem ->
...
let include_err env ppf =
  ...
  | Type_declarations (id, d1, d2, err) ->
    fprintf ppf "@[<v>@[<hv>%s:@;<1 2>%a@ %s@;<1 2>%a@]%a%a@]"
      ...
      (Printtyp.tree_of_type_declaration id d1 Trec_first) "is not included in"
      !Oprint.out_sig_item (Printtyp.tree_of_type_declaration id d2 Trec_first)
  ...
  | Extension_constructors (id, x1, x2, err) ->
    fprintf ppf "@[<v>@[<hv>%s:@;<1 2>%a@ %s@;<1 2>%a@]@ %a%a@]"
      ...
      (Includecore.report_extension_constructor_mismatch "the first"
         "the second" "declaration") err show_locs (x1.ext_loc, x2.ext_loc)
  | Module_types (mty1, mty2) ->
    fprintf ppf
      "@[<hv 2>Modules do not match:@ \
       %a@;<1 -2>is not included in@ %a@]" !Oprint.out_module_type
      (Printtyp.tree_of_modtype mty1) !Oprint.out_module_type
      (Printtyp.tree_of_modtype mty2)
  | Modtype_infos (id, d1, d2) ->
    fprintf ppf
      "@[<hv 2>Module type declarations do not match:@ \
       %a@;<1 -2>does not match@ %a@]" !Oprint.out_sig_item
  ...
  | Class_type_declarations (id, d1, d2, reason) ->
    fprintf ppf
      "@[<hv 2>Class type declarations do not match:@ \
       %a@;<1 -2>does not match@ %a@]@ %a" !Oprint.out_sig_item
  ...
  | Class_declarations (id, d1, d2, reason) ->
    fprintf ppf
      "@[<hv 2>Class declarations do not match:@ \
       %a@;<1 -2>does not match@ %a@]@ %a" !Oprint.out_sig_item
      (Printtyp.tree_of_class_declaration id d1 Trec_first) !Oprint.out_sig_item
...
let is_big obj =
  ...
  size > 0 &&
    ((if Bytes.length !buffer < size then buffer := Bytes.create size);
     try ignore (Marshal.to_buffer !buffer 0 size obj []); false with _ -> true)
...
let report_error ppf errs =
  if errs = [] then
    ...
    let include_err' ppf (_, _, obj as err) =
      if not (is_big obj) then
        fprintf ppf "%a@ " include_err err
      else if !pe then
        (fprintf ppf "...@ "; pe := false)
    in
