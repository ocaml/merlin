...
let type_variance =
  function NoVariance -> "" | Covariant -> "+" | Contravariant -> "-"
...
=
  fun ?sep ?first ?last fu f xs ->
    ...
    and sep = match sep with Some x -> x | None -> ("@ " : _ format6)
    in
    let aux f =
      function
      | [] -> ()
...
let rec_flag f rf = match rf with Nonrecursive -> () | Recursive -> pp f "rec "

let nonrec_flag f rf =
  match rf with Nonrecursive -> pp f "nonrec " | Recursive -> ()

let direction_flag f = function Upto -> pp f "to@ " | Downto -> pp f "downto@ "
...
and core_type ctxt f x =
  if x.ptyp_attributes <> [] then
    ...
    | Ptyp_arrow (l, ct1, ct2) ->
      pp f "@[<2>%a@;->@;%a@]"
      (* FIXME remove parens later *) (type_with_label ctxt) (l, ct1)
        (core_type ctxt) ct2
    | Ptyp_alias (ct, s) ->
  ...
  if x.ptyp_attributes <> [] then
    ...
    | Ptyp_variant (l, closed, low) ->
      ...
      let type_variant_helper f x =
        ...
        | Rtag (l, _, ctl) ->
          pp f "@[<2>%a%a@;%a@]" (iter_loc string_quot) l (fun f l ->
            match l with
            | [] -> ()
      ...
      pp f "@[<2>[%a%a]@]" (fun f l ->
        ...
        | _ ->
          pp f "%s@;%a"
            ...
            | Open, _ -> ">"
            end (list type_variant_helper ~sep:"@;<1 -2>| ") l
      ) l (fun f low ->
        match low with
        | Some [] | None -> ()
...
and sugar_expr ctxt f e =
  if e.pexp_attributes <> [] then
    ...
    | Pexp_apply
      ...
      =
        let print_path ppf =
          function None -> () | Some m -> pp ppf ".%a" longident m
        in
      ...
      | Lident "!", [ e ] -> pp f "@[<hov>!%a@]" (simple_expr ctxt) e; true
...
and uncurry params e =
  match e.pexp_desc with
  | Pexp_fun (l, e0, p, e) -> uncurry ((l, e0, p) :: params) e
  | _ -> List.rev params, e
...
and expression ctxt f x =
  if x.pexp_attributes <> [] then
    ...
    | Pexp_fun (l, e0, p, e) ->
      ...
      pp f "@[<2>fun@;%a->@;%a@]" (pp_print_list (label_exp ctxt)) params
  ...
  if x.pexp_attributes <> [] then
    ...
    | Pexp_coerce (e, cto1, ct) ->
      pp f "(%a%a :> %a)" (expression ctxt) e
        (option (core_type ctxt) ~first:" : " ~last:" ") cto1
      (* no sep hint*) (core_type ctxt) ct
    ...
    | Pexp_record (l, eo) ->
      ...
      in
      pp f "@[<hv0>@[<hv2>{@;%a%a@]@;}@]"
      (* "@[<hov2>{%a%a}@]" *) (option ~last:" with@;" (simple_expr ctxt)) eo
...
and value_description ctxt f x =
  ...
  pp f "@[<hov2>%a%a@]" (core_type ctxt) x.pval_type (fun f x ->
...
and class_signature ctxt f { pcsig_self = ct; pcsig_fields = l; _ } =
  let class_type_field f x =
    ...
    | Pctf_extension e ->
      item_extension ctxt f e; item_attributes ctxt f x.pctf_attributes
  in
  pp f "@[<hv0>@[<hv2>object@[<1>%a@]@ %a@]@ end@]" (fun f ->
    function
    | { ptyp_desc = Ptyp_any; ptyp_attributes = []; _ } -> ()
    | ct -> pp f " (%a)" (core_type ctxt) ct
  ) ct (list class_type_field ~sep:"@;") l
(* call [class_signature] called by [class_signature] *)

and class_type ctxt f x =
  match x.pcty_desc with
  | Pcty_signature cs ->
    class_signature ctxt f cs; attributes ctxt f x.pcty_attributes
  | Pcty_constr (li, l) ->
    pp f "%a%a%a" (fun f l ->
      match l with
      | [] -> ()
  ...
  | Pcty_arrow (l, co, cl) ->
    pp f "@[<2>%a@;->@;%a@]"
    (* FIXME remove parens later *) (type_with_label ctxt) (l, co)
      (class_type ctxt) cl
  | Pcty_extension e -> extension ctxt f e; attributes ctxt f x.pcty_attributes
  | Pcty_open (o, e) ->
...
and class_field ctxt f x =
  ...
  | Pcf_method (s, pf, Cfk_concrete (ovf, e)) ->
    ...
    in
    pp f "@[<2>method%s %a%a@]%a" (override ovf) private_flag pf (fun f ->
      function
  ...
  | Pcf_extension e ->
    item_extension ctxt f e; item_attributes ctxt f x.pcf_attributes

and class_structure ctxt f { pcstr_self = p; pcstr_fields = l } =
  pp f "@[<hv0>@[<hv2>object%a@;%a@]@;end@]" (fun f p ->
    match p.ppat_desc with
    | Ppat_any -> ()
    | Ppat_constraint _ -> pp f " %a" (pattern ctxt) p
    | _ -> pp f " (%a)" (pattern ctxt) p
  ) p (list (class_field ctxt)) l
...
and module_type ctxt f x =
  if x.pmty_attributes <> [] then
    ...
    | Pmty_with (mt, l) ->
      let with_constraint f =
        function
        | Pwith_type (li, ({ ptype_params = ls; _ } as td)) ->
          let ls = List.map fst ls in
          pp f "type@ %a %a =@ %a"
            (list (core_type ctxt) ~sep:"," ~first:"(" ~last:")") ls
...
and module_type1 ctxt f x =
  if x.pmty_attributes <> [] then
    ...
    | Pmty_signature s ->
      pp f "@[<hv0>@[<hv2>sig@ %a@]@ end@]"
      (* "@[<hov>sig@ %a@ end@]" *) (list (signature_item ctxt)) s
...
and signature_item ctxt f x : unit =
  ...
  | Psig_modtype { pmtd_name = s; pmtd_type = md; pmtd_attributes = attrs } ->
    pp f "@[<hov2>module@ type@ %s%a@]%a" s.txt (fun f md ->
      match md with
      | None -> ()
      | Some mt -> pp_print_space f (); pp f "@ =@ %a" (module_type ctxt) mt
  ...
  | Pstr_recmodule decls ->
    ...
    | pmb :: l2 ->
      pp f "@[<hv>@[<hov2>module@ rec@ %s@ =@ %a@]%a@ %a@]"
        (Option.value pmb.pmb_name.txt ~default:"_") (module_expr ctxt)
        pmb.pmb_expr (item_attributes ctxt) pmb.pmb_attributes (fun f l2 ->
        List.iter (aux f) l2
      ) l2
    | _ -> assert false
    end
...
and type_extension ctxt f x =
  ...
  in
  pp f "@[<2>type %a%a += %a@ %a@]%a" (fun f ->
    function
    | [] -> ()
    | l -> pp f "%a@;" (list (type_param ctxt) ~first:"(" ~last:")" ~sep:",") l
  ) x.ptyext_params longident_loc x.ptyext_path private_flag x.ptyext_private
  (* Cf: #7200 *) (list ~sep:"" extension_constructor) x.ptyext_constructors
  ...
  | Variable_in_scope (loc, var) ->
    Location.errorf ~source ~loc
      "In this scoped type, variable %a \
       is reserved for the local type %s." tyvar var var
  | Other loc -> Location.errorf ~source ~loc "Syntax error"
