...
(* Type expressions for the core language *)
  ...
  let iter sub { ptyp_desc = desc; ptyp_loc = loc; ptyp_attributes = attrs } =
    ...
    | Ptyp_constr (lid, tl) -> iter_loc sub lid; List.iter (sub.typ sub) tl
    | Ptyp_object (ol, _o) -> List.iter (object_field sub) ol
    | Ptyp_class (lid, tl) -> iter_loc sub lid; List.iter (sub.typ sub) tl
    | Ptyp_alias (t, _) -> sub.typ sub t
    | Ptyp_variant (rl, _b, _ll) -> List.iter (row_field sub) rl
    | Ptyp_poly (_, t) -> sub.typ sub t
    | Ptyp_package (lid, l) ->
      iter_loc sub lid; List.iter (iter_tuple (iter_loc sub) (sub.typ sub)) l
  ...
  =
    ...
    sub.location sub ptype_loc; sub.attributes sub ptype_attributes
  ...
  =
    ...
    sub.location sub ptyext_loc; sub.attributes sub ptyext_attributes
  
  let iter_type_exception
      sub { ptyexn_constructor; ptyexn_loc; ptyexn_attributes }
  =
    sub.extension_constructor sub ptyexn_constructor;
    sub.location sub ptyexn_loc; sub.attributes sub ptyexn_attributes
  ...
  =
    ...
    sub.location sub pext_loc; sub.attributes sub pext_attributes
end
...
(* Type expressions for the class language *)
  let iter sub { pcty_loc = loc; pcty_desc = desc; pcty_attributes = attrs } =
    ...
    sub.attributes sub attrs;
    match desc with
    | Pcty_constr (lid, tys) -> iter_loc sub lid; List.iter (sub.typ sub) tys
    ...
    | Pcty_open (o, e) -> sub.open_description sub o; sub.class_type sub e
  ...
  let iter_signature sub { pcsig_self; pcsig_fields } =
    sub.typ sub pcsig_self; List.iter (sub.class_type_field sub) pcsig_fields
end
...
(* Type expressions for the module language *)
  ...
  let iter_with_constraint sub =
    ...
    | Pwith_typesubst (lid, d) -> iter_loc sub lid; sub.type_declaration sub d
  ...
  let iter_signature_item sub { psig_desc = desc; psig_loc = loc } =
    ...
    | Psig_extension (x, attrs) -> sub.attributes sub attrs; sub.extension sub x
...
(* Value expressions for the module language *)
  let iter sub { pmod_loc = loc; pmod_desc = desc; pmod_attributes = attrs } =
    ...
    | Pmod_constraint (m, mty) -> sub.module_expr sub m; sub.module_type sub mty
  ...
  let iter_structure_item sub { pstr_loc = loc; pstr_desc = desc } =
    ...
    | Pstr_extension (x, attrs) -> sub.attributes sub attrs; sub.extension sub x
...
(* Value expressions for the core language *)
  let iter sub { pexp_loc = loc; pexp_desc = desc; pexp_attributes = attrs } =
    ...
    | Pexp_fun (_lab, def, p, e) ->
      iter_opt (sub.expr sub) def; sub.pat sub p; sub.expr sub e
    | Pexp_function pel -> sub.cases sub pel
    | Pexp_apply (e, l) -> sub.expr sub e; List.iter (iter_snd (sub.expr sub)) l
    ...
    | Pexp_construct (lid, arg) -> iter_loc sub lid; iter_opt (sub.expr sub) arg
    ...
    | Pexp_letop { let_; ands; body } ->
      ...
      List.iter (sub.binding_op sub) ands; sub.expr sub body
    | Pexp_extension x -> sub.extension sub x
    | Pexp_unreachable | Pexp_hole -> ()
  
  let iter_binding_op sub { pbop_op; pbop_pat; pbop_exp; pbop_loc } =
    iter_loc sub pbop_op;
    sub.pat sub pbop_pat; sub.expr sub pbop_exp; sub.location sub pbop_loc
end
...
(* Patterns *)
  let iter sub { ppat_desc = desc; ppat_loc = loc; ppat_attributes = attrs } =
    ...
    | Ppat_construct (l, p) -> iter_loc sub l; iter_opt (sub.pat sub) p
...
(* Value expressions for the class language *)
  let iter sub { pcl_loc = loc; pcl_desc = desc; pcl_attributes = attrs } =
    ...
    sub.attributes sub attrs;
    match desc with
    | Pcl_constr (lid, tys) -> iter_loc sub lid; List.iter (sub.typ sub) tys
    ...
    | Pcl_constraint (ce, ct) -> sub.class_expr sub ce; sub.class_type sub ct
    | Pcl_extension x -> sub.extension sub x
    | Pcl_open (o, e) -> sub.open_description sub o; sub.class_expr sub e
...
let default_iterator =
  {
    ...
    value_description =
      (fun
       ...
       ->
         ...
         this.location this pval_loc; this.attributes this pval_attributes);
    pat = P.iter;
    expr = E.iter;
    ...
    include_description =
      (fun this { pincl_mod; pincl_attributes; pincl_loc } ->
         ...
         this.location this pincl_loc; this.attributes this pincl_attributes);
    include_declaration =
      (fun this { pincl_mod; pincl_attributes; pincl_loc } ->
