...
let restore_ppx_context payload =
  ...
  let field name payload =
    ...
    | "include_dirs" ->
        Clflags.set.contents.Clflags.include_dirs :=
          get_list get_string payload
    | "load_path" ->
        Config.load_path :=
          Misc.Path_list.of_string_list_ref (ref (get_list get_string payload))
    | "open_modules" ->
        Clflags.set.contents.Clflags.open_modules <-
          get_list get_string payload
    | "for_package" ->
  ...
  { txt = "ocaml.ppx.context"; loc = Location.none },
    Parsetree.PStr [Str.eval (
      Exp.record ([
        ...
        lid "for_package",  make_option make_string None;
        lid "debug",        make_bool false;
      ]) None)]
