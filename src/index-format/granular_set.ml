...
module Make(Ord: OrderedType) = struct
    ...
    let height t = match fetch t with
        Empty -> 0
      | Node {h;_} -> h
      ...
      if hl > hr + 2 then begin
        ...
        | Node{l=ll; v=lv; r=lr; _} ->
            if height ll >= height lr then
              ...
              match fetch lr with
                Empty -> invalid_arg "Set.bal"
              | Node{l=lrl; v=lrv; r=lrr; _}-> create
                  (create ll lv lrl)
                  lrv
                  (create lrr v r)
            end
        ...
        | Node{l=rl; v=rv; r=rr; _} ->
            if height rr >= height rl then
              ...
              match fetch rl with
                Empty -> invalid_arg "Set.bal"
              | Node{l=rll; v=rlv; r=rlr; _} -> create
                  (create l v rll)
                  rlv
                  (create rlr rv rr)
            end
    ...
    let rec add x t : t = match fetch t with
      | Empty -> link (Node{l= link Empty; v=x; r= link Empty; h=1})
      | Node{l; v; r; _} as t ->
          let c = Ord.compare x v in
          if c = 0 then link t else
          if c < 0 then
            let ll = add x l in
    ...
    let rec min_elt t = match fetch t with
        Empty -> raise Not_found
      | Node{l; v; _} when fetch l = Empty -> v
      | Node{l; _} -> min_elt l

    let rec remove_min_elt t = match fetch t with
    ...
    let is_empty t = match fetch t with
      | Empty -> true | _ -> false

    let rec mem x t = match fetch t with
        Empty -> false
      | Node{l; v; r; _} ->
          let c = Ord.compare x v in
          c = 0 || mem x (if c < 0 then l else r)

    let rec remove x t = match fetch t with
        Empty -> link Empty
      | (Node{l; v; r; _} as t) ->
          let c = Ord.compare x v in
    ...
    let rec add_min_element x t = match fetch t with
      | Empty -> singleton x
      | Node {l; v; r; _} ->
        bal (add_min_element x l) v r

    let rec add_max_element x t = match fetch t with
      | Empty -> singleton x
      | Node {l; v; r; _} ->
    ...
    let rec max_elt t = match fetch t with
        Empty -> raise Not_found
    ...
    let rec split x t = match fetch t with
      ...
      | Node{l; v; r; _} ->
          ...
          if c = 0 then (l, true, r)
            ...
            let (ll, pres, rl) =
              split x l in (ll, pres, join rl v r)
          else
            let (lr, pres, rr) =
              split x r in (join l v lr, pres, rr)
    ...
    let rec union t1 t2 =
      ...
      | (Node{l=l1; v=v1; r=r1; h=h1}, Node{l=l2; v=v2; r=r2; h=h2}) ->
          if h1 >= h2 then
            if h2 = 1 then add v2 t1 else begin
              let (l2, _, r2) = split v1 t2 in
              join (union l1 l2) v1 (union r1 r2)
            end
          else
            if h1 = 1 then add v1 t2 else begin
              let (l1, _, r1) = split v2 t1 in
              join (union l1 l2) v2 (union r1 r2)
            end

    let rec filter p t = match fetch t with
        Empty -> link Empty
      | (Node{l; v; r; _}) as t ->
          let l' = filter p l in
          let pv = p v in
          let r' = filter p r in
          if pv then
            if l==l' && r==r' then link t else join l' v r'
          else concat l' r'

    let rec cardinal t = match fetch t with
        Empty -> 0
      | Node{l; r; _} -> cardinal l + 1 + cardinal r

    let rec elements_aux accu t = match fetch t with
        Empty -> accu
      | Node{l; v; r; _} -> elements_aux
        (v :: elements_aux accu r) l

    let elements s =
      elements_aux [] s

    let try_join l v r =
      if (fetch l = Empty || Ord.compare (max_elt l) v < 0)
      && (fetch r = Empty || Ord.compare v (min_elt r) < 0)
      then join l v r
      else union l (add v r)

    let rec map f t = match fetch t with
      | Empty -> link Empty
      | Node{l; v; r; _} as t ->
         let l' = map f l in
         let v' = f v in
         let r' = map f r in
         if l == l' && v == v' && r == r' then link t
         else try_join l' v' r'

    let rec iter f t = match fetch t with
        Empty -> ()
    ...
    let rec schema iter f m =
      ...
      match tree with
      | Empty -> ()
      | Node {l; v; r; _} -> schema iter f l ; f v; schema iter f r
end
