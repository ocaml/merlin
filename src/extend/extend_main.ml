...
module Reader = struct
  ...
  module Make (V : R.V0) = struct
    ...
    let exec =
      function
      | Req_load buf -> buffer := Some (V.load buf); Res_loaded
      ...
      | Req_print_outcome trees ->
        let print t =
          V.print_outcome Format.str_formatter t; Format.flush_str_formatter ()
        in
...
(* Postpone messages until ready *)
  let (send, set_ready) =
    ...
    let set_ready () =
      ...
      postponed := []; List.iter really_send postponed'
    in
    let send msg =
...
module Handshake = struct
  ...
  let negotiate (capabilities : P.capabilities) =
    ...
    | _ -> prerr_endline "Unexpected value after handshake."; exit 1
...
let extension_main ?reader desc =
  ...
  let reader =
    ...
    | Some (module R : R.V0) -> let module M = Reader.Make(R)  in M.exec
  in
  let respond f =
    match f () with
  ...
  let rec loop () =
    ...
    match input_value stdin with
    | exception End_of_file -> exit 0
    | P.Start_communication -> prerr_endline "Unexpected message."; exit 2
    | P.Reader_request request ->
      respond (fun () -> P.Reader_response (reader request)); loop ()
  in
  loop ()
