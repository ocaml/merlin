...
let is_operator =
  let open Raw_parser in function
    ...
    | BANG | PLUS | PLUSDOT | MINUS | MINUSDOT | STAR | EQUAL | LESS | GREATER
    | OR | BARBAR | AMPERSAND | AMPERAMPER | COLONEQUAL | PLUSEQ | PERCENT
      -> true
    | _ -> false

let is_ident =
  let open Raw_parser in function
    | UIDENT _ | LIDENT _ -> true
    | _ -> false
