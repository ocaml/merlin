...
module Make
    ...
    (Recovery :
      ...
      end) (Dump : sig val symbol : unit -> Parser.xsymbol -> string end)
= struct
  type 'a candidate =
    {
      line : int;
      ...
      | Some (Parser.Element (state, _, pos, _)) ->
        ...
        if depth = 0 then
          ...
          let col' =
            ...
            | None -> max_int
            | Some env ->
              begin match Parser.top env with
              | None -> max_int
  ...
  let attempt r token =
    ...
    let rec aux =
      ...
      | [] -> `Fail
      | x :: xs ->
        begin match feed_token ~allow_reduction:true token x.env with
        | `Fail ->
    ...
    let rec aux acc env =
      ...
      | Some (Parser.Element (state, _, _startp, endp)) ->
        ...
        let rec eval (env : a Parser.env) : Recovery.action -> a Parser.env =
          function
          | Recovery.Abort -> log ~title:"eval Abort" ""; raise Not_found
          | Recovery.R prod ->
