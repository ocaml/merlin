...
module Analyser = Lrgrep_runtime.Interpreter(Parse_errors.Table_error_message)(Parser_raw.MenhirInterpreter)


let log_env env =
  let rec states env =
    string_of_int (MenhirInterpreter.current_state_number env) ::
...
let explain env (unexpected, startp, endp) popped shifted =
  ...
  in
  let return item =
    { analyser; item; unclosed = !unclosed; location; popped; shifted; unexpected }
  in
...
let to_error { analyser; item; unclosed; location; popped; shifted; unexpected = _ } =
  let msg =
  match analyser with
  | msg :: _ -> msg
  | [] ->
