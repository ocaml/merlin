...
let opening (type a) : a terminal -> string option =
  function
...
let opening_st st =
  match incoming_symbol st with T term -> opening term | _ -> None

let closing (type a) : a terminal -> bool =
  function
  | T_END -> true
  | T_RPAREN -> true
  | T_RBRACKET -> true
...
let closing_st st =
  match incoming_symbol st with T term -> closing term | _ -> false
...
let explain env (unexpected, startp, endp) popped shifted =
  ...
  let rec process env =
    ...
    | Some (Element (st, _, startp, endp)) ->
      ...
      begin match Parser_explain.named_item_at (number st) with
...
let to_error { item; unclosed; location; popped; shifted; unexpected = _ } =
  let inside =
    match item with None -> "" | Some (name, _) -> " inside `" ^ name ^ "'"
  in
  let after =
    match unclosed with None -> "" | Some (name, _) -> " after unclosed " ^ name
  in
