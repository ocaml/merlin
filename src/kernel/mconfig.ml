...
let dump_ocaml x =
  `Assoc
    [ "include_dirs", `List (List.map ~f:Json.string x.include_dirs);
      "no_std_include", `Bool x.no_std_include; "unsafe", `Bool x.unsafe;
      "classic", `Bool x.classic; "principal", `Bool x.principal;
...
let cwd = ref None
let unsafe_get_cwd () = match !cwd with None -> assert false | Some cwd -> cwd
    ...
    [ "build_path", `List (List.map ~f:Json.string x.build_path);
      ...
      `List
        (List.map ~f:(fun (impl, intf) ->
         ...
         ) x.suffixes); "stdlib", Json.option Json.string x.stdlib;
      "reader", `List (List.map ~f:Json.string x.reader);
      "protocol",
      begin match x.protocol with
      | `Json -> `String "json"
      | `Sexp -> `String "sexp"
      end; "log_file", Json.option Json.string x.log_file;
      ...
      "assoc_suffixes",
      `List
        (List.map ~f:(fun (suffix, reader) ->
           `Assoc [ "extension", `String suffix; "reader", `String reader ]
...
let dump_query x =
  `Assoc
    [ "filename", `String x.filename; "directory", `String x.directory;
      "printer_width", `Int x.printer_width; "verbosity", `Int x.verbosity ]

type t = { ocaml : ocaml; merlin : merlin; query : query }

let dump x =
  `Assoc
    [ "ocaml", dump_ocaml x.ocaml; "merlin", dump_merlin x.merlin;
...
let stdlib =
  let env =
    ...
    with
    | Not_found -> (try Some (Sys.getenv "CAMLLIB") with Not_found -> None)
  in
  fun config ->
...
let normalize_step t =
  ...
  if merlin.flags_to_apply <> [] then
    ...
    let t =
      List.fold_left ~f:(fun t { workdir; workval } ->
        fst
...
let rec normalize t =
  if is_normalized t then
    (log ~title:"normalize" "%a" Logger.json (fun () -> dump t); t)
  else
...
let merlin_flags =
  [
    ...
    Marg.param "extension" (fun extension merlin ->
      ...
      | Some _ -> { merlin with  extensions = extension :: merlin.extensions }
    ),
    ...
    "<path> Change path of ocaml standard library";(* Legacy support for janestreet. Ignored. To be removed soon. *)
   "-attributes-allowed", Marg.unit_ignore, " DEPRECATED" ]
  ...
  [
    ...
    Marg.param "integer" (fun verbosity query ->
      let verbosity =
        ...
        with
        | _ -> invalid_arg "argument should be an integer"
      in
...
let ocaml_ignored_flags =
  [ "-a"; "-absname"; "-alias-deps"; "-annot"; "-app-funct"; "-bin-annot"; "-c";
    "-compact"; "-compat-32"; "-config"; "-custom"; "-dalloc"; "-dclambda";
    "-dcmm"; "-dcombine"; "-dcse"; "-dflambda"; "-dflambda-no-invariants";
    "-dflambda-verbose"; "-dinstr"; "-dinterf"; "-dlambda"; "-dlinear"; "-dlive";
    "-dparsetree"; "-dprefer"; "-drawclambda"; "-drawflambda"; "-drawlambda";
    "-dreload"; "-dscheduling"; "-dsel"; "-dsource"; "-dspill"; "-dsplit";
    "-dstartup"; "-dtimings"; "-dtypedtree"; "-dtypes"; "-dump-pass"; "-fno-PIC";
    "-fPIC"; "-g"; "-i"; "-inlining-report"; "-keep-docs"; "-keep-docs";
    "-keep-locs"; "-linkall"; "-make_runtime"; "-make-runtime"; "-modern";
    "-no-alias-deps"; "-noassert"; "-noautolink"; "-no-check-prims";
    "-nodynlink"; "-no-float-const-prop"; "-no-keep-locs"; "-no-principal";
    "-no-rectypes"; "-no-strict-formats"; "-no-strict-sequence";
    "-no-unbox-free-vars-of-clos"; "-no-unbox-specialised-args"; "-O2"; "-O3";
...
let ocaml_ignored_parametrized_flags =
  [ "-cc"; "-cclib"; "-ccopt"; "-color"; "-dflambda-let"; "-dllib"; "-dllpath";
    ...
    "-inline"; "-inline-prim-cost"; "-inline-toplevel"; "-intf"; "-intf_suffix";
    "-intf-suffix"; "-o"; "-rounds"; "-runtime-variant";
...
let ocaml_flags =
  [
    ...
    Printf.sprintf " Make strings mutable (default: %B)"
      (not Config.safe_string);
    "-safe-string",
    Marg.unit (fun ocaml -> { ocaml with  unsafe_string = false }),
    Printf.sprintf " Make strings immutable (default: %B)" Config.safe_string;
    "-nopervasives",
...
let parse_arguments ~wd ~warning local_spec args t local =
  let_ref cwd (Some wd) @@
    fun () -> Marg.parse_all ~warning arguments_table local_spec args t local
...
let () =
  ...
  in
  List.iter ~f:(add (fun x -> x.ocaml) (fun x ocaml -> { x with  ocaml }))
    ocaml_flags;
  List.iter ~f:(add (fun x -> x.merlin) (fun x merlin -> { x with  merlin }))
    merlin_flags;
  List.iter ~f:(add (fun x -> x.query) (fun x query -> { x with  query }))
    query_flags;
...
let document_arguments oc =
  let print_doc flags =
    List.iter ~f:(fun (name, _flag, doc) ->
  ...
  output_string oc
    "Flags accepted by ocamlc and ocamlopt but not affecting merlin will be ignored.\n"
...
let build_path config =
  ...
  let result =
    if config.merlin.exclude_query_dir then
      dirs
    else
      config.query.directory :: dirs
  in
