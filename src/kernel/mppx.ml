...
let change_directory dir =
  ...
  | () -> true
  | exception exn ->
    log ~title:"changing directory" "change_directory %S failed with %a" dir
      Logger.exn exn;
    false
...
let rewrite cfg parsetree =
  ...
  let restore () =
    if not (change_directory prev_dir) then ignore (change_directory "/")
  in
  (* add include path attribute to the parsetree *)
  with_include_dir (Mconfig.build_path cfg) @@
    fun () ->
      match
        Pparse.apply_rewriters ~restore:false ~ppx ~tool_name:"merlin" parsetree
      with
      | parsetree -> restore (); cfg, parsetree
      | exception exn ->
        ...
        Msupport.raise_error exn; restore (); cfg, parsetree
