...
let return_and_cache status = cache := Some status; status
...
let compatible_prefix result_items tree_items =
  let rec aux acc =
    ...
    | ritem :: ritems, pitem :: pitems
      when
        Btype.is_valid ritem.part_snapshot &&
...
let rec type_structure caught env =
  function
  | parsetree_item :: rest ->
    let (items, _, part_env) =
      Typemod.merlin_type_structure env [ parsetree_item ]
    in
    let typedtree_items = items.Typedtree.str_items, items.Typedtree.str_type in
    let item =
      {
        parsetree_item;
        typedtree_items;
...
let rec type_signature caught env =
  function
  | parsetree_item :: rest ->
...
let type_interface config caught parsetree =
  ...
  return_and_cache (env0, snap0, `Interface (List.rev_append prefix suffix))

let run config parsetree =
  (if not (Env.check_state_consistency ()) then
     (Mocaml.flush_caches (); Local_store.reset ()));
  Mocaml.setup_config config;
  let caught = ref [] in
  Msupport.catch_errors Mconfig.(config.ocaml.warnings) caught @@
    fun () ->
      Typecore.reset_delayed_checks ();
      let (initial_env, initial_snapshot, typedtree) =
        match parsetree with
        | `Implementation parsetree ->
          type_implementation config caught parsetree
...
let get_errors t =
  ...
  Typecore.reset_delayed_checks (); !caught

let get_typedtree t =
  let split_items l =
    let (typd, typs) = List.split (List.map ~f:(fun x -> x.typedtree_items) l)
    in
...
let node_at ?(skip_recovered=false) t pos_cursor =
  ...
  let rec select =
    ...
    | (_, _) :: ((_, node') :: _ as ancestors)
      when Mbrowse.is_recovered node' ->
      select ancestors
    | l -> l
  in
  ...
  | path ->
    log ~title:"node_at" "Deepest before %s" (Mbrowse.print () path); path
