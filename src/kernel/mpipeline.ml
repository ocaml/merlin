  ...
  lazy
    (let start = Misc.time_spent () in
     ...
     let update () =
       ...
       time_shift := time_shift0 +. delta; r := !r +. delta -. shift
     in
  ...
  *)
  ...
  let get config =
    ...
    | state ->
      ...
      log ~title "found entry for this configuration"; state
    | exception Not_found ->
      log ~title "nothing cached for this configuration";
      let state = Mocaml.new_state () in
      cache := (key, state) :: List.take_n 5 !cache; state
end
...
let with_pipeline t f =
  Mocaml.with_state t.state @@
    fun () -> Mreader.with_ambient_reader t.config (input_source t) f

let get_lexing_pos t pos =
  Msource.get_lexing_pos (input_source t) ~filename:(Mconfig.filename t.config)
    pos
...
let process
    ?state ?(pp_time=ref 0.0) ?(reader_time=ref 0.0) ?(ppx_time=ref 0.0)
    ?(typer_time=ref 0.0) ?(error_time=ref 0.0) ?for_completion config
    raw_source
=
  let state = match state with None -> Cache.get config | Some state -> state in
  let source =
    timed_lazy pp_time
      (lazy
         begin match Mconfig.(config.ocaml.pp) with
         | None -> raw_source
         | Some { workdir; workval } ->
           let source = Msource.text raw_source in
           let source =
             Pparse.apply_pp ~workdir ~filename:Mconfig.(config.query.filename)
               ~source ~pp:workval
           in
           Msource.make source
         end)
    ...
    timed_lazy reader_time
      (lazy
         (let (lazy source) = source in
          ...
          let config = Mreader.apply_directives config result.Mreader.parsetree
          in
    ...
    timed_lazy typer_time
      (lazy
         (let (lazy { Ppx.config; parsetree; _ }) = ppx in
          ...
          let errors = timed_lazy error_time (lazy (Mtyper.get_errors result))
          in
          { Typer.errors; result }))
  in
  {
...
let for_completion
    position
    { config; state; raw_source; pp_time; reader_time; ppx_time; typer_time;
      error_time; _ }
=
  process
    config
    raw_source
    ~for_completion:position
    ~state
    ~pp_time
    ~reader_time
    ~ppx_time
...
let timing_information t =
  [ "pp", !(t.pp_time); "reader", !(t.reader_time); "ppx", !(t.ppx_time);
    "typer", !(t.typer_time); "error", !(t.error_time) ]
