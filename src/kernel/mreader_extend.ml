...
let stop t =
  if t.stopped then
    ...
    (log ~title:"stop" "%a" print t;
     t.stopped <- true; Extend_driver.stop t.driver)

let stop_finalise t =
  if not t.stopped then
    (log ~title:"stop_finalise" "leaked process %s" t.name; stop t)
...
let load_source t config source =
  ...
  | Res_loaded -> Some t
  | _ -> Extend_driver.stop t.driver; incorrect_behavior "load_source" t; None
...
let start name args config source =
  ...
  Gc.finalise stop_finalise process; load_source process config source

let parsetree =
  function Signature sg -> `Interface sg | Structure str -> `Implementation str

let parse ?for_completion t =
  log ~title:"parse" "?for_completion:%a %a"
    (Option.print Msource.print_position) for_completion print t;
  assert (not t.stopped);
  match
    Extend_driver.reader t.driver
      begin match for_completion with
      | None -> Req_parse
      | Some pos ->
        let pos =
          Msource.get_lexing_pos t.source ~filename:(Mconfig.filename t.config)
            pos
        in
  ...
  | _ -> incorrect_behavior "parse" t; None
...
let reconstruct_identifier pos t =
  ...
  | _ -> incorrect_behavior "reconstruct_identifier" t; None
...
let print_pretty tree t =
  ...
  | _ -> incorrect_behavior "pretty_print" t; None
...
let print_outcomes ts t =
  ...
  | [] -> Some []
  | ts ->
    begin match Extend_driver.reader t.driver (Req_print_outcome ts) with
    | Res_print_outcome ts -> Some ts
    | _ -> incorrect_behavior "print_batch_outcome" t; None
    end
