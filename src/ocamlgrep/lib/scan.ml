...
(* Process a single cmt file: read it, digest-check the source, run
   the matcher, and emit the resulting events. *)
let process_one_cmt (paths : Paths.t) handle_event expr cmt_path =
  ...
  | { Cmt_format.cmt_sourcefile = Some source;
    ...
    if not (Sys.file_exists pp_source) then ()
      ...
      | exception Match.Cannot_parse_type exn ->
        failwith
          (Format.asprintf "%s: could not parse type: %a." cmt_path
...
let incremental_search
  ...
  in
  List.iter (process_one_cmt paths handle_event expr) cmt_files

let search paths cmt_files query =
  let events = ref [] in
  let handle_event ev = events := ev :: !events in
  incremental_search paths cmt_files handle_event query;
  List.rev !events
