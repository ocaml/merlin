...
let param_ignore = fun x -> param "string" (fun _ x -> x) x
...
let parse_all ~warning global_spec local_spec =
  let rec normal_parsing args global local =
    ...
    | None ->
      ...
      | arg :: args ->
        warning ("unknown flag " ^ arg); resume_parsing args global local
      | [] -> global, local
      end
