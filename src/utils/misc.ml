...
(* File functions *)
let remove_file filename = try Sys.remove filename with Sys_error _msg -> ()
      ...
      if Sys.file_exists dir && Sys.is_directory dir then
        ...
        (fun filename ->
           ...
           | exception Not_found ->
             ...
             Hashtbl.add cache filename exists; exists)
      else
        (fun _ -> false)
  end)
...
let find_in_path path name =
  canonicalize_filename
    (if not (Filename.is_implicit name) then
       (if
          exact_file_exists ~dirname:(Filename.dirname name)
       ...
       List.find_map path ~f:(fun dirname ->
         if exact_file_exists ~dirname ~basename:name then
           Some (Filename.concat dirname name)
         else
           None
       ))
...
let find_in_path_uncap ?(fallback="") path name =
  ...
  canonicalize_filename
    (let uname = String.uncapitalize name in
     ...
     List.find_map path ~f:(fun dirname ->
       if exact_file_exists ~dirname ~basename:uname then
         Some (Filename.concat dirname uname)
       else if exact_file_exists ~dirname ~basename:name then
         Some (Filename.concat dirname name)
       else if
         has_fallback && exact_file_exists ~dirname ~basename:ufallback
       then
         Some (Filename.concat dirname ufallback)
       else if
         has_fallback && exact_file_exists ~dirname ~basename:fallback
       then
         Some (Filename.concat dirname fallback)
       else
         None
     ))
...
let rev_split_string cond s =
  let rec split1 res i =
    if i >= String.length s then
      res
    else if cond s.[i] then
      split1 res (i + 1)
    else
      split2 res i (i + 1)
  and split2 res i j =
    if j >= String.length s then
      String.sub s ~pos:i ~len:(j - i) :: res
    else if cond s.[j] then
...
let rev_string_split ~on s = rev_split_string ((=) on) s

let get_ref r =
  let v = !r in
  r := []; v

let set_or_ignore f opt x = match f x with None -> () | Some y -> opt := Some y
let fst3 (x, _, _) = x
let snd3 (_, x, _) = x
let thd3 (_, _, x) = x
let fst4 (x, _, _, _) = x
...
module LongString = struct
  ...
  let create str_size =
    ...
    tbl.(tbl_size - 1) <- Bytes.create (str_size mod Sys.max_string_length); tbl
  ...
  let input_bytes ic len =
    ...
    Array.iter (fun str -> really_input ic str 0 (Bytes.length str)) tbl; tbl
end
...
let file_contents filename =
  ...
  try
    ...
    let rec loop () =
      ...
      | 0 -> ()
      | n -> Buffer.add_subbytes buf str 0 n; loop ()
    in
...
let spellcheck env name =
  let cutoff =
    match String.length name with 1 | 2 -> 0 | 3 | 4 -> 1 | 5 | 6 -> 2 | _ -> 3
  in
...
let did_you_mean ppf get_choices =
  ...
  | choices ->
    ...
    Format.fprintf ppf "@\nHint: Did you mean %s%s%s?@?"
      (String.concat ~sep:", " rest) (if rest = [] then "" else " or ") last
...
let modules_in_path ~ext path =
  ...
  List.fold_left ~init:[] path ~f:(fun results dir ->
    try
      Array.fold_left (fun results file ->
