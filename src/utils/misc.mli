...
(** [try_finally work ~always ~exceptionally] is designed to run code in [work]
    ...
    [always], that must be run after any execution of the function (typically,
    freeing system resources), and [exceptionally], that should be run only if
    [work] or [always] failed with an exception (typically, undoing user-visible
    state changes that would only make sense if the function completes
...
(** Returns a more precise measurement of resources usage than
    Sys.times/Unix.times. Both user and kernel cpu time is accounted. *)
...
(** Return the name of the OCaml module matching a basename (filename without
    directory). Remove the extension and capitalize *)
...
(** [ordinal_suffix n] is the appropriate suffix to append to the numeral [n] as
    an ordinal number: [1] -> ["st"], [2] -> ["nd"], [3] -> ["rd"], [4] ->
    ["th"], and so on. Handles larger numbers (e.g., [42] -> ["nd"]) and the
    numbers 11--13 (which all get ["th"]) correctly. *)
