...
let escaped str =
  ...
  for i = 0 to len - 1 do
    match str.[i] with '\\' | '"' -> incr extra_chars | _ -> ()
  done;
    ...
    while !i < len do
      ...
      | '\\' ->
        ...
        | 'x' ->
          ...
          Buffer.add_char buf (Char.chr (c0 * 16 lor c1)); i := !i + 2
        | '0' .. '9' ->
          let c0 = Char.code str.[!i + 1] in
          let c1 = Char.code str.[!i + 2] in
...
let rec of_list = function [] -> nil | a :: tl -> Cons (a, of_list tl)

let rec tell_sexp tell =
  function
  | Cons (a, b) -> tell "("; tell_sexp tell a; tell_cons tell b
  | Sym s -> tell s
...
and tell_cons tell =
  ...
  | Sym "nil" -> tell ")"
  | Cons (a, b) -> tell " "; tell_sexp tell a; tell_cons tell b
  | sexp -> tell " . "; tell_sexp tell sexp; tell ")"
...
let read_sexp getch =
  ...
  and read_num getch c =
    ...
    let rec aux () =
      match getch () with
      | c when c >= '0' && c <= '9' -> Buffer.add_char buf c; aux ()
      | ('.' | 'e' | 'E') as c ->
        is_float := true; Buffer.add_char buf c; aux ()
      | c ->
  ...
  and read_string getch =
    ...
    let rec aux () =
      ...
      | '\\' -> Buffer.add_char buf '\\'; Buffer.add_char buf (getch ()); aux ()
  ...
  and read_sym getch next =
    ...
    let rec aux next =
      match match next with Some c -> c | None -> getch () with
      | ('\'' | '-' | ':' | '_') as c -> Buffer.add_char buf c; aux None
      | c when is_alphanum c -> Buffer.add_char buf c; aux None
...
let to_string sexp =
  ...
  to_buf sexp buf; Buffer.contents buf
    ...
    | None ->
      ...
      | '\000' ->
        ...
        if read = 0 then
          ...
          (getch := getch_of_subbytes buffer 0 read; !getch ())
      | c -> c
      end
  in
  fun () ->
